...
In order to match entities from a pre-created [Amazon Rekognition Collection](https://docs.aws.amazon.com/rekognition/latest/dg/collections.html), make sure these two properties,`collectionId` and `maxEntities`, are set in your `amplifyconfiguration.json` file. `collectionId` should be the  name of your collection that you created either with the CLI or the SDK and `maxEntities` should be a number greater than `0` or less than or equal to `50` which is the max number of entities Rekognition can detect from a collection. If there is no `collectionId` set in the `amplifyconfiguration.json` file or `maxEntities` is set to be `0`, then this call will just detect entities in general with facial features, landmarks, etc. Bounding boxes for entities are returned as ratios so make sure if you would like to place the bounding box of your entity on an image that you multiple the x by the width of the image, the y by the height of the image, and both height and width ratios by the image's respective height and width.
