...
To enable DataStore to use multiple authorization types based on the model's `@auth` rules, set configure the "auth mode strategy" when initializing DataStore:
