...
Note: In order for the Angular compiler to correctly pickup the `aws-exports.js` file, you'll need it to be in typescript format.  So make a copy of the `aws-exports.js` file but with a `ts` extension like so:
...
Amplify.configure(awsamplify.default);
```
