...
For client side authentication flow, there are three different ways you can use:

1. `USER_SRP_AUTH`: The `USER_SRP_AUTH` flow is used by default and utilizes the SRP protocol (Secure Remote Password) to encrypt the password on the client before it's sent to the back-end. It is a more secure way of sending user credentials over the network and is, therefore, the recommended approach.
...
One of the reason to use this authentication flow is that it would allow you to migrate users to AWS Cognito.
...
Here is the sample of how to create a CAPTCHA challenge with the Lambda Triggers.

The following Lambda creates a CAPTCHA as a challenge to the user which is the `Define Auth Challenge Lambda Trigger`. The URL for the CAPTCHA image and  the expected answer is added to the private challenge parameters:
...
This example defines a custom challenge which is a `Create Auth Challenge Lambda Trigger`:
...
This Lambda is used to verify a challenge answer which is a `Verify Auth Challenge Response Lambda Trigger`:
