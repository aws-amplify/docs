...
As you can notice in the sample schema file above, the schema has a `@model` directive. This tells Amplify CLI that the related types should be stored in an Amazon DynamoDB table. When you create or update your backend with *push* command, the CLI will automatically create the necessary data sources for you, behind the scenes. The transform libraries can greatly simplify the process of bootstrapping highly scalable, serverless GraphQL APIs on AWS. To use transformers, you will annotate your *schema.graphql* with GraphQL directives that will augment the behavior of the generated API. 

The following directives are available to be used with AWS AppSync when defining your schema:  

| Directive | Description |
| --- | --- |
| @model on Object | Used for storing types in Amazon DynamoDB. |
| @auth on Object | Used to define different authorization strategies. | 
| @connection on Field | Used for specifying relationships between @model object types. |
| @searchable on Object | Used for streaming the data of an @model object type to Amazon Elasticsearch Service. |
| @versioned one Object | Used to add object versioning and conflict detection to a @model. | 

But you may also write your own transformers to build your own reproducible patterns. To learn more about the GraphQL Transform libraries see [GraphQL Transform Documentation](https://aws-amplify.github.io/docs/cli/graphql?sdk=js).
...
When creating APIs, you will make changes to the other files and directories in the *amplify/backend/api/YOUR-API-NAME/* directory but should not manually change anything in the *build* directory because your changes will overwritten the next time you run `amplify push` or `amplify api gql-compile`. Here is an overview of the API directory:
...
##### Create custom resolvers & other resources
