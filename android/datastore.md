...
Open your project `build.gradle` and add `mavenCentral()` as a repository, `classpath 'com.amplifyframework:amplify-tools-gradle-plugin-beta:0.1.0'` as a dependency, and a plugin of `'com.amplifyframework.amplifytools'` as in the example below:
...
Gradle tasks will generate model files to be used in your API calls and provision a backend for you.
...
If you do not wish to use the above Gradle task you can do this manually by first installing the Amplify CLI:
...
In your application's `onCreate` or similar lifecycle location initialize Amplify:

```java
AWSDataStorePlugin dataStorePlugin = AWSDataStorePlugin.getInstance(AmplifyModelProvider.getInstance());
...
Amplify.DataStore.query(Post.class, new ResultListener<Iterator<Post>>() {
        ...
        public void onResult(Iterator<Post> result) {
            while (result.hasNext()) {
                ...
                Log.i("Rating: ", String.valueOf(post.getRating()));
                Log.i("Status: ", post.getStatus().toString());
            }
        }
...
When constructing predicates, static QueryField instances such as `Post.RATING` does not own any information about which model the field belongs in. In order to avoid any ambiguity between equal field names of different models, it is recommended to construct a custom instance of QueryField in the form of  `QueryField.field("{model-name}.{field-name}")` (i.e. `QueryField.field("Post.rating")`).
{: .callout .callout--info}

When using multiple conditions, they can be chained by using the following fields: `and | or | not`

```java
Amplify.DataStore.query(Post.class, Post.RATING.gt(4).and(Post.STATUS.eq(PostStatus.ACTIVE)), new ResultListener<Iterator<Post>>() {
        @Override
        public void onResult(Iterator<Post> result) {
            while (result.hasNext()) {
                Post post = result.next();
...
Models in DataStore are immutable. To update a record you must use the `.copyOfBuilder()` function to apply updates to the item's fields rather than mutating the instance directly:
...
`AWSDataStorePlugin.observe()` method takes dependency on RxJava and returns an instance of `io.reactivex.Observable<DataStoreItemChange<? extends Model>>` when a model class is observed. You will need to add `implementation 'io.reactivex.rxjava2:rxandroid:2.1.1'` to your `build.gradle`.
{: .callout .callout--info}
...
DataStore can connect to an existing AppSync backend that has been deployed from another JavaScript project or even it was originally deployed by iOS or Android. In these workflows it is best to work with the CLI directly by running an `amplify pull` command from your terminal and then generating models, by using the Gradle Task selecting **modelgen** or with `amplify codegen models` using the Amplify CLI.
...
```java
Amplify.DataStore.query(Comment.class, Post.ID.eq("123"), new ResultListener<Iterator<Comment>>() {
        /* callback implementation */
    }
);
```
...
1. Contain configuration for API category with EXACTLY ONE GraphQL endpoint, and
2. Contain configuration for Data Store category to set "syncMode" to "api"
...
Neither category needs to be configured for just local data store without any cloud sync.
