...
3. Provision the backend with `amplify push`. Here is an example:
```perl
...
* `classpath 'com.amplifyframework:amplify-tools-gradle-plugin-beta:0.1.0'` as a dependency
...
2. In the `build.gradle` for the app, enable model generation for a non sync-able endpoint with the following:
...
3. Build the project. When you edit the schema under `amplify/backend/api/<APINAME>/schema.graphql` and build, it will regenerate the Model files.
