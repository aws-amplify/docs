...
2. In the `build.gradle` for the app, enable model generation for a non-syncable endpoint with the following:
...
With the Blog model generated, you can create an instance of this object and pass it to your GraphQL operation. Note you should not specify an ID - we will create one for you. Also, if you manually type out the builder method, you'll notice that it guides you into entering all required fields first before you'll see the optional fields and build() method appear in autosuggest.
