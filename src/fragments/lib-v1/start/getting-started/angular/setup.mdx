...
### Strictly typing `amplifyconfiguration.json`

If you have TypeScript [strict mode](https://www.typescriptlang.org/tsconfig/#strict) on and see the error

```
Could not find a declaration file for module './amplifyconfiguration.json'. 'amplifyconfiguration.json.js' implicitly has an 'any' type.
```

Create a `amplifyconfiguration.json.d.ts` file on the same level as amplifyconfiguration.json with the following content:
