...
In-App Messaging is currently in developer preview. The library APIs are supported for JavaScript applications in general, however the prebuilt UI components are only available for React Native at this time.
...
In-App Messaging also integrates seamlessly with [Amplify Analytics](https://docs.amplify.aws/lib-v1/analytics/getting-started/q/platform/js/). Once you’ve set up an In-App Messaging campaign (through [Amazon Pinpoint](https://aws.amazon.com/pinpoint/)), simply sync messages to your user’s device and recorded analytics events will automatically start working as trigger events to display in-app messages to your users.

![In-App-Messaging](/images/lib-v1/in-app-messaging/preview-console-and-simulator.png)