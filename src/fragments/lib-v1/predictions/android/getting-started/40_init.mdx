...
Note that because the predictions category requires auth, you will need to either configure [guest access](/[platform]/prev/build-a-backend/auth/enable-guest-access/) or [sign in a user](/[platform]/prev/build-a-backend/auth/enable-sign-in/) before using features in the predictions category.
