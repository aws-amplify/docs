`copy` method copies an existing file to a different key and returns a `{key: 'destKey'}` object upon
success.
...
To copy a file across different access levels, you'll need to explicitly provide the access level if it's 'private' or 'protected' (Defaults to 'guest').
