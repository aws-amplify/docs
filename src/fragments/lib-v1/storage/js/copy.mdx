...
To copy a file, you need to provide a source key in `source` and a destination key in `destination`. `accessLevel` can be 'guest' | 'private' | 'protected' but defaults to 'guest'.
...
console.log(
	await list({ path: 'copied/', options: { accessLevel: 'private' } })
); // []
