...
**[Amplify GraphQL client](/gen1/[platform]/prev/build-a-backend/graphqlapi/query-data/)** - a light weight option if you're looking for a simple way to leverage GraphQL features and do not need the offline capabilities or caching. If you need those features, please look at [Amplify DataStore](/gen1/[platform]/prev/build-a-backend/more-features/datastore/set-up-datastore/).

**[Amplify DataStore](/gen1/[platform]/prev/build-a-backend/more-features/datastore/set-up-datastore/)** - makes it easy to build apps that need to support offline and low-latency scenarios. DataStore also makes working with distributed, cross-user data just as simple as working with local-only data by providing a programming model for leveraging shared and distributed data without writing additional code.
