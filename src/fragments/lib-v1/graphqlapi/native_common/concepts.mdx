...
Amplify offers three SDK options for AppSync.

__[Amplify GraphQL client](/lib-v1/graphqlapi/query-data)__ - a light weight option if you're looking for a simple way to leverage GraphQL features and do not need the offline capabilities or caching. If you need those features, please look at [Amplify DataStore](/lib-v1/datastore/getting-started).

__[Amplify DataStore](/lib-v1/datastore/getting-started)__ - makes it easy to build apps that need to support offline and low-latency scenarios. DataStore also makes working with distributed, cross-user data just as simple as working with local-only data by providing a programming model for leveraging shared and distributed data without writing additional code.
