> Prerequisite: [Install, configure and init an Amplify project](/gen1/[platform]/tools/cli/start/set-up-cli/) with Amplify CLI
...
Because the `Todo` type was decorated with an `@model` directive of the [GraphQL Transform](/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/) library, the CLI created the additional schema and resolvers for queries, mutations, and subscriptions as well as a DynamoDB table to hold the Todos.
...
import js0 from '/src/fragments/lib-v1/graphqlapi/js/js-configure.mdx';
...
import reactnative0 from '/src/fragments/lib-v1/graphqlapi/js/react-native-configure.mdx';
...
// Subscribe to creation of Todo
const sub = API.graphql(graphqlOperation(onCreateTodo)).subscribe({
  next: (payload) => {
    const createdTodo = payload.value.data?.onCreateTodo;
    console.log(createdTodo);
  }
...
| --- | --- |
| [@model](/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/) on Object | Store objects in DynamoDB and configure CRUD resolvers. |
| [@auth](/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/) on Object | Define authorization strategies for your API. |
| [@hasOne, @hasMany, @belongsTo, @manyToMany](/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/) on Field | Specify relationships between @model object types. |
| [@searchable](/gen1/[platform]/build-a-backend/graphqlapi/search-and-result-aggregations/) on Object | Stream data of an @model object type to the Amazon OpenSearch Service. |
| [@primaryKey and @index](/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/) on Object | Index your data with keys. |
| [@function](/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#lambda-function-resolver) on Field | Connect Lambda resolvers to your API. |
| [@predictions](/gen1/[platform]/build-a-backend/graphqlapi/connect-machine-learning-services/) on Field | Connect machine learning services. |
| [@http](/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#http-resolver) on Field | Configure HTTP resolvers within your API. |

You may also write your own transformers to implement reproducible patterns that you find useful.

### Mocking and Local Testing

Amplify supports running a local mock server for testing your application with AWS AppSync, including debugging of resolvers, before pushing to the cloud. Please see the [CLI Toolchain documentation](/gen1/[platform]/tools/cli/usage/mock/) for more details.
...
The Amplify CLI will create an `amplify/backend/api` folder that will hold the existing GraphQL schema, resolvers, and additional configuration around the API. To learn more about how the CLI manages this configuration, check out the documentation [here](/gen1/[platform]/build-a-backend/graphqlapi/). To learn how to configure custom GraphQL resolvers, check out the documentation [here](/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#override-amplify-generated-resolvers).
