...
Although DataStore presents a distinct API, its cloud synchronization functionality relies on the underlying [API category](/[platform]/prev/build-a-backend/graphqlapi/set-up-data/). Therefore, you will still be required to incorporate the API plugin when working with DataStore.
