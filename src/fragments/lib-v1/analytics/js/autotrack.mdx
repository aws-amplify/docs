...
You can track the session both in a web app or a React Native app by using Analytics. A web session can be defined in different ways. To keep it simple you define that the web session is active when the page is not hidden and inactive when the page is hidden. 
...
Note: Amplify doesn't capture the location automatically. Instead you can add the location information, in the default config when you [configure Analytics](https://docs.amplify.aws/lib-v1/analytics/getting-started/q/platform/js#set-up-existing-analytics-backend) or while [updating the end point](https://docs.amplify.aws/lib-v1/analytics/getting-started/q/platform/js#update-endpoint).
