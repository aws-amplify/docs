...
This code imports the MapLibre GL JS library and CSS, one of the popular options for map rendering we recommend for use with Amplify Geo. In the HTML body you create a `<div>` element with an id of 'map' that will be the map's container. Finally in the script section you setup some Amplify configuration that is required for Amplify Geo to understand what Amplify AWS resources have been created.
...
In this step you will show you how to add code to display a map in your application.
