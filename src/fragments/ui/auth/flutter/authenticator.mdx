...
This is done by providing an `authenticatorBuilder` method to the Authenticator. If you are familiar with widgets like `StreamBuilder`, `FutureBuilder`, or `LayoutBuilder`, this may feel familiar. `authenticatorBuilder` accepts a builder method that takes arguments for the current `BuildContext` and `AuthenticatorState`, which is a interactive representation of the state being managed by the authenticator.
