...
The example above made authentication a requirement for the entire application. The Authenticator can also be used to protect a subset of routes, and leave other routes as unauthenticated routes. 
...
In addition to customizing form fields and theming, you can also fully customize one or more of the screens for the authenticator. This can be used for simple use cases, such as adding a logo to the sign up and sign in screen, as well as far more complex use cases.

This is done by providing a builder to the `authenticatorBuilder` value of the Authenticator. If you are familiar with widgets like `StreamBuilder`, `FutureBuilder`, or `LayoutBuilder`, this may feel familiar. `authenticatorBuilder` is a builder method that takes arguments for the current `BuildContext` and `AuthenticatorState`, which is a representation of the state being managed by the authenticator.
...
This example uses `SignInForm()` and `SignUpForm()` widgets from the amplify_authenticator package. In addition to the available prebuilt forms, form fields (ex: `SignInFormField.username()` or `SignUpFormField.email()`) and form buttons (ex: `SignInButton()`) are also available. All of the prebuilt widgets are fully integrated into the authenticator state. 
