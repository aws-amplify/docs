...
This example uses the `SignInForm()` and `SignUpForm()` widgets from the amplify_authenticator package. In addition to the available prebuilt forms, form fields (ex: `SignInFormField.username()` or `SignUpFormField.email()`) and form buttons (ex: `SignInButton()`) are also available. All of the prebuilt widgets are fully integrated into the authenticator's state. 
