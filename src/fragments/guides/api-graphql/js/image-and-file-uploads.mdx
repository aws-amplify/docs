...
To build this API, you need the following:
...
The first thing you will want to do is create the authentication service. To do so, you'll initialize an Amplify project and add authentication.
...
Next, you'll create the storage service (Amazon S3):
...
Next you will be creating a GraphQL API with a type that has an image field. This image can only be accessed by someone using your app. If someone tries to fetch this image directly, they will not be able to view it.
...
Now that the backend is created, how can you interact with it to save and read products and images from it?

Here is the code that you could use to save a product and image to the API:
...
async function saveProduct() {
  ...
  await client.graphql({ query: createProduct, variables: { input: product }});
}
```

Here is the code that you could use to query a product and corresponding image from the API:
...
async function getProductById () {
  const product = await client.graphql({ query: getProduct, variables: { id: "12345" }});
...
Here is the code that you could use to query an array of products and images from the API:
...
async function listProductsWithImages () {
  const productData = await client.graphql({ query: listProducts });
