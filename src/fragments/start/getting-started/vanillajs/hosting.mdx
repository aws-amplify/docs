...
If you get an "AccessDenied" error within an XML document, this maybe because the application's root directory was not initialized properly. Ensure that [`DistributionDir`](/cli/usage/headless/#--frontend) is set to the correct directory in `amplify/.config/project-config.json` and then re-run `amplify publish`
