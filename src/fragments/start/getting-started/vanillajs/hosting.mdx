...
Note: To remove the hosting resources locally from the backend run `amplify remove hosting` command. On your next `amplify push` the provisioned hosting resources will get removed from the cloud. Now, in order to observe the Amplify project resources run `amplify status` command.