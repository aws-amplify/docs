...
async function createNewTodo() {
  ...
  return await client.graphql(graphqlOperation(createTodo, { input: todo }));
}
...
+  client.graphql(graphqlOperation(listTodos)).then((evt) => {
...
+client.graphql(graphqlOperation(onCreateTodo)).subscribe({
+  next: (evt) => {
