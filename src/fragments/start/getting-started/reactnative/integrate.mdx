...
#### Home.js
Next, we will create a Home component for our app which will implement most of the CRUD functionality for our app. Create a new file `Home.js` in the `src` folder. Here, we have total four components defined :

1. **Header**: Simple Header component with Title.
2. **AddTodoModal**: Used for displaying a Modal when a new Todo needs to be added.
...
If you restart your app now, you should see that newly added todos will be automatically start showing up on the list. The items look like they can be check off and marked as completed, but when pressed, they don’t seem to do anything right now. Let’s learn how to update existing data.
...
Deleting an existing data entry is even easier than updating one since you don’t need to copy the instance to delete it. It may not be the best user experience to do this but, for this app, we will trigger deletion of a todo item off of a long press of the item.
