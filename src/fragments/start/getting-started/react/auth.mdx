...
In addition to the `Authenticator`, you can build custom authentication flows using the `Auth` class from the JS library. The `Auth` class can be used in replacement of the `Authenticator`, except in the case of using [override function calls](https://ui.docs.amplify.aws/react/components/authenticator#override-function-calls)
