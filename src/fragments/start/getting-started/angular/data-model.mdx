...
Next, create an `restuarants` Angular component.

```
ng generate component restaurants
```

This will create `<app-restaurants>` that you will use to implement create and list APIs. First, update `src/app/app.component.html` to use the new Angular component:

```html
<app-restaurants></app-restaurants>
```

In your `src/app/restaurants/restaurants/restaurants.component.ts` file, add data to your database with a mutation by using the `API.service` file which was generated when you ran `amplify add api`:
