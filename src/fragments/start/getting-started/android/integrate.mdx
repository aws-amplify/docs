...
- Extend _MyAmplifyApp_ from _android.app.Application_ by adding `extends Application` to your class
...
- Extend _MyAmplifyApp_ from _android.app.Application_ by adding `: Application()` to your class
...
We just reached a _very cool_ checkpoint. We have a fully featured CRUD application that saves and retrieves data in the local device, which means the app **works without an AWS account and even without internet connection**.
