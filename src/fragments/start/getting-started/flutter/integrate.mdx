...
Go ahead and run your code now on an iOS or Android device and you should see an app with a progress indicator and a floating action button but not much else.
...
Lastly update the `dispose` function as follows to cancel the subscription when the UI is disposed.
...
A key difference between hot reload and hot restart is that hot restart disposes the current state, while hot reload does not.
