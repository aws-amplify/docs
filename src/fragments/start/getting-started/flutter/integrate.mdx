...
Go ahead and run your code now on any device and you should see an app with some titles and a floating action button but not much else.
...
in the Amplify Authenticator, which will provide a pre-built authentication flow with no setup required.
...
To that, we'll use Amplify's so-called model helpers which provide methods we can call to quickly
create GraphQL requests from our model types. Here we're using the `ModelQueries.list` helper to
retrieve all the budget entries and then passing the GraphQL request to the API category which
will automatically transform the response into a list of `BudgetEntry` types.
