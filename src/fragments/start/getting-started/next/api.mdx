...
export async function getServerSideProps({ req, params }) {
  const SSR = withSSRContext({ req });
  const { data } = await SSR.client.graphql({
    query: getPost,
    variables: {
      id: params.id
    }
...
- **getServerSideProps** - This will only run on the server-side and allows for fetching of data as we make the request. Here, you use params.id to query for that specific post with client.graphql and await the response.
...
For each request (`req`) on the server, you create a copy of Amplify (called `SSR` here from `withSSRContext({ req })`) that scopes credentials, data, and storage to _just one_ request. `client.graphql` queries for a list of posts, and returns them as the `posts` prop for the `Home` component.

- **handleCreatePost** – This function is called when a logged-in user submits your "New Post" form. `client.graphql` is called to create the new post's `title` and `content`. Once created, you redirect to `/posts/${post.id}`.
