...
- **handleCreatePost** – This function is called when a logged-in user submits our "New Post" form. `API.graphql` is called to create the new post's `title` and `content`. Once created, you redirect to `/posts/${post.id}`. Notice that you explicitly set `authMode` to `AMAZON_COGNITO_USER_POOLS`. This is because our `schema.graphql` explicitly requires an authorized user to create/delete/update our **Post** model. Based on our configuration when you ran `amplify add api`, this value is defaulting to `API_KEY`.
...
- **handleDelete** – This function is called when the "Delete post" button is clicked. Notice that you explicitly set `authMode` to `AMAZON_COGNITO_USER_POOLS`. This is because our `schema.graphql` explicitly requires an authorized user to create/delete/update our **Post** model. Based on our configuration when you ran `amplify add api`, this value is defaulting to `API_KEY`.
...
- **handleCreatePost** – This function is called when a logged-in user submits our "New Post" form. `API.graphql` is called to create the new post's `title` and `content`. Once created, you redirect to `/posts/${post.id}`.
