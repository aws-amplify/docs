---
lastUpdated: '2021-10-01T21:22:07.000Z'
---
...
- Install [Amplify CLI](/cli) version 4.22.0 or later by running:
...
1. Fill in the following for your project:
  ...
  * Select the **Next** button
  ...
  After selecting **Next**, **select where you would like to save your project**, and then select **Create**.
...
1. Enter the Amplify iOS GitHub repo URL (`https://github.com/aws-amplify/amplify-ios`) into the search bar and hit **Enter**. Wait for the result to load. You'll see the Amplify iOS repository rules for which version of Amplify you want Swift Package Manager to install. Choose the dependency rule **Up to Next Major Version**, as it will use the latest compatible version of the dependency that can be detected from the `main` branch, then click **Add Package**.

    ![Search for repo](/images/project-setup/20_5_search-amplify-repo.png)

1. Lastly, choose which of the libraries you want added to your project. For this tutorial, select **Amplify**, **AWSAPIPlugin**, and **AWSDataStorePlugin**, then click **Add Package**.
