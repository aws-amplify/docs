...
Given these requirements, you'll need to be able to query the API for a list of todos and also provide a way to send updates to the API to create, update, and delete todos. In GraphQL you would use a `type` to define that entity, like so:
...
Because you're using Amplify, you can use the GraphQL Schema Definition Language (SDL) and custom Amplify directives to define your backend requirements for your API. The GraphQL Transform library then converts your SDL definition into a set of fully descriptive AWS CloudFormation templates that implement your data model.
...
The `@model` directive lets Amplify know you intend for this type to have data that needs to be stored. This will create a DynamoDB table for us and make all GraphQL operations available in the API.
...
To create a new todo in the database, use the `client.graphql()` operation with the `createTodo` mutation and pass in the data you'd like to write.
