...
Now that we have our authentication service deployed to AWS, it's time to add authentication to our Vue app. Creating the login flow can be quite difficult and time consuming to get right. Luckily Amplify Framework has an authentication UI component we can use that will provide the entire authentication flow for us, using our configuration specified in our **aws-exports.js** file.
...
You can also [customize](https://ui.docs.amplify.aws/components/authenticator?platform=vue#customization) this component to add or remove fields, update styling, or other configurations.
