...
The data collected by the library is limited to adding a user agent to each request made to the AWS service. The user-agent header is included with information about the Amplify library version, operating system name and version. AWS collects this data to generate metrics related to our library usage. This information is not linked to the user’s identity and not used for tracking purposes as defined by Apple’s [App Tracking Transparency framework](https://developer.apple.com/documentation/apptrackingtransparency).
