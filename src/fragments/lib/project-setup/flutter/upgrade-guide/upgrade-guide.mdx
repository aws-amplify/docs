...
- `fetchAuthSession` had several changes. See the example below for how to migrate.
  - `Amplify.Auth.getPlugin()` has been added to return plugin specific sub types. Casting to `CognitoAuthSession` is no longer needed when using this.
  - `getAWSCredentials` has been deprecated. AWS credentials will always be included in the response.
  - `fetchAuthSession` will no longer throw exceptions. Instead, indivdual getters of type `AuthResult` will throw if the underlying operation fails.
  ...
  - `CognitoAuthSession.credentials`, `CognitoAuthSession.userPoolTokens`, `CognitoAuthSession.`, and ``CognitoAuthSession.` are deprecated and will be removed in a future release.
...
- Auth hub events are now have a specific sub type, `AuthHubEvent` and contain a `type` that is an enum. See the example below for how to migrate.
