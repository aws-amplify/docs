...
Before you can add an existing AWS resource to an Android application, the application must have the Amplify libraries installed. For detailed instructions, see [Install Amplify Libraries](/gen1/[platform]/start/project-setup/create-application/#n2-install-amplify-libraries).
...
* See the [API (GraphQL) category](/gen1/[platform]/build-a-backend/graphqlapi/existing-resources/) to use existing AWS AppSync resources.
...
* See the [Authentication category](/gen1/[platform]/build-a-backend/auth/set-up-auth/#prerequisites) to use existing Amazon Cognito resources.
