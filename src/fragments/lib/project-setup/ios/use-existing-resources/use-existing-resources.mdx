...
Before you can add an existing AWS resource to an iOS application, the application must have the Amplify libraries installed. For detailed instructions, see [Install Amplify Libraries](/[platform]/start/project-setup/create-application/#n2-install-amplify-libraries).
...
- See the [API (GraphQL) category](/[platform]/build-a-backend/graphqlapi/) to use existing AWS AppSync resources.
...
- See the [Authentication category](/[platform]/build-a-backend/auth/existing-resources/) to use existing Amazon Cognito resources.
- See the [Geo (Developer Preview) category](/[platform]/build-a-backend/more-features/geo/existing-resources/) to use existing Amazon Location Service resources.
