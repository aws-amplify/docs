...
{/_ TODO Rewrite We do not show that there is a provider for Appsync - is that something we should be highlighting? \\ \*/} {/_ Do we have CLI support for PubSub? \\ \*/}
...
<Callout>
  Only JSON serializable message payloads are currently supported for MQTT
  providers within PubSub. If you are attempting to use message payloads that
  are non-JSON serializable, consider transforming the payload into a format
  that aligns with the input type expected by
