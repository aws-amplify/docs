...
Learn how to install and configure [Amplify Command Line Interface (CLI)](https://docs.amplify.aws/cli/start/install/) , a unified toolchain to create AWS cloud services for your app.
...
You will need to update `aws-exports.js` manually to configure In-App Messaging. Add the configuration for In-App Messaging to the exported `awsmobile` variable with both the Pinpoint project ID (created when Analytics was deployed) as well as the AWS region to which it was deployed. You can find your Project ID by navigating to the AWS console, and searching for Amazon Pinpoint under the All projects section.
...
4. On the **Visual editor** tab, select the following:
   1. Service: **Pinpoint**
   2. Actions: **mobiletargeting:GetInAppMessages** (manually added, may be unrecognized)

      - Click on “add actions“
      ![In-App-Messaging](/images/lib/in-app-messaging/attach-a-policy.png)
      - Enter **GetInAppMessages** into the popup.
      ![In-App-Messaging](/images/lib/in-app-messaging/mobiletargeting-policy-attach.png)
      
   3. Resources: **arn:aws:mobiletargeting:\*:<account-id\>:apps/<pinpoint-project-id\>\***
