...
Now your application is set up with Amplify. To interact with Amplify In-App Messaging APIs, you need to first initialize In-App Messaging by calling the `initializeInAppMessaging` API by directly importing it from the `in-app-messaging` sub-path. This is required to be called as early as possible in the app lifecycle.
