...
You can use the JWT token provided by the Authentication API to authenticate against API Gateway directly when using a [custom authorizer](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-integrate-with-cognito.html).
...
You need to provide Authorization token to authenticate against API Gateway when using token-based Lambda Authorizer
