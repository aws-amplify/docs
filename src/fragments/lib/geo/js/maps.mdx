...
<BlockSwitcher>
  ...
  **Note:** There must be a `div` with an `id="map"` on the DOM before making the call to `createMap` in this way.
  ...
  In React, you will want to call `createMap` inside of a `useEffect` hook or `componentDidMount` call and use a `ref` instead of an `id` to access the DOM element for the map.
