...
In order to help users find their query quickly the geocoder search box will show search term suggestions as well as geocode results while typing that the user can select from. Suggestions may be points of interest or addresses based on partial or misspelled free-form text.

<Callout>

While `geocode` results will be shown in the suggestions box while a user is typing, markers for those locations will not be shown until a user selects a search term by pressing `enter` or selecting a search term in the dropdown.

</Callout>

This behavior may be disabled by setting `autocomplete` to `false` in the options sent to `createAmplifyGeocoder()`.
...
If you would like to customize this behavior, you can pass your own `processResults` function to full take control of the functionality.
