...
While `geocode` results will be shown in the suggestions box while a user is typing, markers for those locations will not be shown until a user selects a search term by pressing `enter` or selecting an item in the dropdown.
