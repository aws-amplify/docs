...
```bash
npm install maplibre-gl maplibre-gl-js-amplify aws-amplify @aws-amplify/ui-react-geo
```

To use `withAuthenticator` as in the example below, install the necessary dependency with the following command:
```bash
...
<BlockSwitcher>
  ...
  **Note:** When using the existing [maps implementation](/lib/geo/maps) you can add the Geofence control to an existing map
