...
If you are using a different mapping library or have a need for a more programmatic approach to managing geofences, the `@aws-amplify/geo` package provides methods for managing geofences, but not geofence collections.
...
The return from `saveGeofences` is a Promise that resolves to an object with both successes and errors for geofences that were successfully created or not.
...
try {
  ...
  // error handling from logic and validation issues within `createGeofences`
  throw error;
}
...
`geoGeofence` is used to get a single geofence object from a collection.
...
- `geofenceId` - the `geofenceId` of the geofence to get.
