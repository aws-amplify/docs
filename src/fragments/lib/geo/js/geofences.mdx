...
```bash
npm install -S maplibre-gl@1 maplibre-gl-js-amplify@geo aws-amplify@geo
```

> **Note:** Make sure that `maplibre-gl-js-amplify` version `1.2.5-geo` or above and `@aws-amplify/geo` version `1.1.11-geo.11` or above are installed.
...
> **Notes:** To use Geofence Controls the user will need to be authenticated with the administrative Cognito user associated with the Geofence Collection you created above. An easy way to add this to a React application for instance is to use the [Amplify Authenticator](https://ui.docs.amplify.aws/components/authenticator)
