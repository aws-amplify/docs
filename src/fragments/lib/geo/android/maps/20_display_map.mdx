...
2. Declare the view instance variable at top-level of the activity:
...
Replace `MapLibreView` with `AmplifyMapView` as needed.

3. Interact with the map at Activity's `onCreate`
...
protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    // You must either do in your Application as per the Getting Started
    // instructions or do it here, at the activity level
...
The map needs to be loaded in order to interact with it. You can use either `getMapAsync` that is called when the map is ready or `getStyle` that is called when both the map and its style are ready. Some APIs, like the `SymbolManager` requires the style to also be loaded so you can draw markers and other symbols on the map, that's when `getStyle` comes in handy.
...
The `cameraPosition` moves the camera do the passed configuration without any animation. If animation is needed, refer to the `map.animateCamera` instead. See the [official reference](https://docs.maptiler.com/maplibre-gl-native-android/com.mapbox.mapboxsdk.camera/) for more details.

### Add a markers to your map
...
The `AmplifyMapView` comes with a default search field, place markers, visualization modes (map or list) and map controls. It can be used to easily embed a place picker into any app.
...
```kotlin
mapView.onPlaceSelect { place, symbol ->
    // place is an instance of AmazonLocationPlace
    // symbol is an instance of Symbol from MapLibre
...
The `MapLibreView` has quite a few configuration parameters not present in their official guides yet. Check the [MapboxGLAndroidSDK/attrs.xml source](https://github.com/maplibre/maplibre-gl-native/blob/master/platform/android/MapboxGLAndroidSDK/src/main/res/values/attrs.xml) for reference.

Last but not least, check the [official MapView API reference](https://docs.maptiler.com/maplibre-gl-native-android/com.mapbox.mapboxsdk.maps/#mapview) for the available public API documentation.
