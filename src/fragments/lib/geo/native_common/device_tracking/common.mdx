You can use Amplify Geo to collect and send location data of users to [Amazon Location Service](https://aws.amazon.com/location/).  Tracking capabilities is configurable to provide a balance and flexibility between power efficiency and location accuracy. Location data can also be retrieved and deleted at any point.  This can be helpful in building asset and delivery tracking features to help business understand the current and historical locations of their products, personnel, and infrastructure.  See [Common use cases for using Amazon Location Service](https://docs.aws.amazon.com/location/latest/developerguide/common-usecases.html)
...
* **User-provided**: A device ID provided by you.  You are respsonbile for any removing personally identifierable information (PII) or discernable pattern from your provided device ID.  Amplify does not guarantee or prevent collisions with other device IDs. It is up to you to determine if this device ID is consistent across app sessions and devices.
