...
* **User-provided**: A device ID provided by you.  You are responsible for any removing personally identifiable information (PII) or discernable pattern from your provided device ID.  Amplify does not guarantee or prevent collisions with other device IDs. It is up to you to determine if this device ID is consistent across app sessions and devices.
