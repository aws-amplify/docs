...
* **User-provided**: A device ID provided by you.  You are responsible for removing any personally identifiable information (PII) or discernible pattern from your provided device ID.  You are responsible for following best security practices.  Amplify does not guarantee or prevent collisions with other device IDs. It is up to you to determine if this device ID is consistent across app sessions and devices.  We recommend using `Geo.Device.tiedToUser()` instead.
