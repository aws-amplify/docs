<Callout>

**Note:** Amplify Geo is in developer preview and is not intended to be used in production environments. Please reach out to us for any feedback and/or issues [here](https://github.com/aws-amplify/amplify-android/issues)

</Callout>

Amplify Geo provides APIs and map UI components for mobile app development such that you can add maps to your app in just a few lines of code.
Amplify Geo APIs are powered by [Amazon Location Service](https://aws.amazon.com/location/) and the map UI components from [MapLibre](https://maplibre.org/) are already integrated with the Geo APIs. You can quickly get started using [Amplify CLI](/cli/geo/maps) to provision your map resources.

Follow this guide to get started with Amplify Geo through the Amplify CLI.

**Note:** If you want to use existing Amazon Location Service resources [follow this guide](/lib/geo/existing-resources) instead.

## Prerequisites

import android0 from "/src/fragments/lib/geo/android/getting_started/10_pre_req.mdx";

<Fragments fragments={{android: android0}} />

## Provisioning resources through CLI

import android3 from "/src/fragments/lib/geo/android/getting_started/20_cli_resources.mdx";

<Fragments fragments={{android: android3}} />

## Install Amplify Libraries

import android6 from "/src/fragments/lib/geo/android/getting_started/30_install_lib.mdx";

<Fragments fragments={{android: android6}} />

## Initialize Amplify Geo

import android9 from "/src/fragments/lib/geo/android/getting_started/40_init_geo.mdx";

<Fragments fragments={{android: android9}} />
