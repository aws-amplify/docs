...
Passing the `resumable: true` option to the Put API returns an object with the following methods: `pause` , `resume` , `abort`.
...
const duplicateUpload = await Storage.put(file.name, file, {
  resumable: true,
});

// Caveat: Original uploads that were initiated over one hour ago will be cleared and duplicate uploads will begin from the beginning.
```
