...
* files are with `private` access level
* list result doesn't include files under any sub path
* list with pagination, each page contain 50 items
...
Pagination is enabled by default. The default `pageSize` is `1000` if it has not been defined in the `S3ListOptions`. You can also list all files under a given path without pagination.
