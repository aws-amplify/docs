...
### Upload with Flutter file picker plugins
...
safePrint('Uploaded data with metadata: ${uploadDataResult.uploadedItem.metadata}');
...
safePrint('Uploaded file with metadata: ${uploadFileResult.uploadedItem.metadata}');
```
...
Future<void> uploadFile(String path) {
  try {
    uploadOperation = Amplify.Storage.uploadFile(
