...
```dart
// `S3DataPayload.dataUrl()` throws exception on invalid data url string,
// and extracts `contentType` value from the data url string.
...
The `Amplify.Storage.getProperties` is available in developer preview allowing retrieve metadata without downloading the file.
...
Calls to `Amplify.Storage.uploadFile` return a reference to the operation that is actually performing the upload.
...
The operation control APIs are only available on `S3UploadOperation` in developer preview. A `S3UploadOperation` is able to be paused and resumable when uploading a file that is larger than 5MB.
...
Amplify will automatically perform a S3 multipart upload for files that are larger than 5MB. For more information about S3's multipart upload, see [Uploading and copying objects using multipart upload](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html).
