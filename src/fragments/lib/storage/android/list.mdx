You can list all of the objects uploaded under a given prefix by setting the pageSize. If the page size is set lower than the total file size available,
...
try {
    ...
    Log.i("AmplifyApplication", "next token: ${result.nextToken}")
