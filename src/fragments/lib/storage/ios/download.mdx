...
```swift
let downloadTask = try await Amplify.Storage.downloadData(key: "myKey")
Task {
...
let downloadTask = try await Amplify.Storage.downloadFile(key: "myKey",
...
```swift
let url = try await Amplify.Storage.getURL(key: "myKey")
print("Completed: \(url)")
```
