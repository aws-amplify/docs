...
You can get the current user's ID from the Amplify Auth category as shown per the Auth category documentation. Be sure to have it before you set it below.
...
Future<void> addAnalyticsWithLocation(
  ...
  properties.addStringProperty('phoneNumber', phoneNumber);
  properties.addIntProperty('age', age);
