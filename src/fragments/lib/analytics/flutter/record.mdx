...
Future<void> registerGlobalProperties() async {
  ...
  properties.addBoolProperty('boolKey', true);
  properties.addDoubleProperty('doubleKey', 10);
  properties.addIntProperty('intKey', 10);
  properties.addStringProperty('stringKey', 'stringValue');
...
Future<void> unregisterGlobalProperties() async {
  await Amplify.Analytics.unregisterGlobalProperties(
    propertyNames: ['boolKey', 'doubleKey', 'intKey', 'stringKey'],
  );
}
```
