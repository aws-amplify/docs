...
AWS AppSync SDK for iOS will be in Maintenance for 12 months i.e until September 1, 2024, after which no new updates will be made. This may result in the APIs going out of sync with the services and result in breaking production apps.
...
If you were using complex objects in the AWS AppSync SDK for iOS (Maintenance mode), then we recommend you to follow our [Working with Files](/[platform]/build-a-backend/graphqlapi/customize-client-libraries/) guide. This guide details the recommended path to store file metadata in your GraphQL API records and use Amplify Storage to store blob data in Amazon S3.
