...
Generate the Swift models for the app with `amplify codegen models`.

Next, add the following dependencies from Amplify (`https://github.com/aws-amplify/amplify-swift.git`):
- AWSAPIPlugin 
...
The API record's `id` is used as the file key for Storage to ensure uniqueness so that multiple API records will not be associated with the same file key unintentionally.
...
To associate a new or different file with the record, update the existing record with the file key. The following example uploads the file using Storage, updates the record with the file's key. If an image is already associated with the record, this will update the record with the new image.
...
// Set the association to nil and update it
song.coverArtKey = nil
let updateResult = try await Amplify.API.mutate(request: .update(currentSong))
guard case .success(let updatedSong) = updateResult else {
    print("Failed with error: ", result)
    return
}
...
guard let coverArtKey = song.coverArtKey else {
    ...
    let result = try await Amplify.API.mutate(request: .delete(currentSong))
...
```swift
//
//  workingWithFilesApp.swift
//  workingWithFiles
//
//  Created by Law, Michael on 7/27/23.
//
...
struct SongView: View {
        ...
        NavigationView {
            ZStack {
                ...
                .sheet(isPresented: $isImagePickerPresented) {
                    ImagePicker(selectedImage: $vm.currentImage)
                }
                VStack {
                    IsLoadingView()
                }
    ...
    func OpenImagePickerButton() -> some View {
        Button("Select \(vm.currentImage != nil ? "a new ": "" )song album cover") {
    ...
    func SongNameTextField() -> some View {
        TextField("\(vm.currentSong != nil ? "Update": "Enter") song name", text: $songName)
    ...
    func CreateOrUpdateSongButton() -> some View {
        if vm.currentSong == nil, let image = vm.currentImage {
            Button("Save") {
                Task {
                    try? await vm.createSong(name: songName,
                                             artCover: image)
                }
            }
            .buttonStyle(TappedButtonStyle())
            .disabled(vm.isLoading)
        } else if vm.currentSong != nil, let image = vm.currentImage {
            Button("Update") {
                Task {
                    try? await vm.updateArtCover(artCover: image)
                }
            }
            .buttonStyle(TappedButtonStyle())
            .disabled(vm.isLoading)
    ...
    func AdditionalOperations() -> some View {
        if vm.currentSong != nil {
            VStack {
                Button("Refresh") {
                    Task {
                        try? await vm.refreshSongAndArtCover()
                    }
                }.buttonStyle(TappedButtonStyle())
                Button("Remove association from song") {
                    Task {
                        try? await vm.removeImageAssociationFromSong()
                    }
                }.buttonStyle(TappedButtonStyle())
                Button("Remove association and delete image") {
                    Task {
                        try? await vm.removeImageAssociationAndDeleteImage()
                    }
                }.buttonStyle(TappedButtonStyle())
                Button("Delete song and art cover") {
                    Task {
                        try? await vm.deleteSongAndArtCover()
                    }
                    songName = ""
                }.buttonStyle(TappedButtonStyle())
            }.disabled(vm.isLoading)
...
struct PhotoAlbumView: View {
    ...
    @StateObject var vm = PhotoAlbumViewModel()
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                .padding()
                .sheet(isPresented: $isImagePickerPresented) {
                    MultiImagePicker(selectedImages: $vm.currentImages)
                }
    ...
    func DisplayImages() -> some View {
        ...
        ScrollView(.horizontal) {
            HStack {
                ForEach($vm.currentImages, id: \.self) { image in
    ...
    func OpenImagePickerButton() -> some View {
        ...
        Button("Select \(!vm.currentImages.isEmpty ? "new " : "")photo album images") {
            isImagePickerPresented.toggle()
        }.buttonStyle(TappedButtonStyle())
    }
    ...
    func PhotoAlbumNameTextField() -> some View {
        TextField("\(vm.currentAlbum != nil ? "Update": "Enter") album name", text: $albumName)
            .textFieldStyle(RoundedBorderTextFieldStyle())
            .multilineTextAlignment(.center)
    }
    ...
    func CreateOrUpdateAlbumButton() -> some View {
        if vm.currentAlbum == nil, !vm.currentImages.isEmpty {
            Button("Save") {
                Task {
                    try? await vm.createPhotoAlbum(name: albumName,
                                                   photos: vm.currentImages)
                }
            }
            .buttonStyle(TappedButtonStyle())
            .disabled(vm.isLoading)
        } else if vm.currentAlbum != nil {
            ...
            if let lastImage = lastImage {
                ...
                Button("Replace last image in album with above") {
                    Task {
                        try? await vm.replaceLastImage(lastImage)
                        self.lastImage = nil
                        try? await vm.refreshAlbumAndPhotos()
                    }
                }
                .buttonStyle(TappedButtonStyle())
                .disabled(vm.isLoading)
                Button("Append above image to album") {
                    Task {
                        try? await vm.addAdditionalPhotos(lastImage)
                        self.lastImage = nil
                        try? await vm.refreshAlbumAndPhotos()
                    }
                }
        ...
        if vm.currentAlbum != nil {
            VStack {
                ...
                Button("Remove associations from album") {
                    Task {
                        try? await vm.removeStorageAssociationsFromAlbum()
                        try? await vm.refreshAlbumAndPhotos()
                    }
                }.buttonStyle(TappedButtonStyle())
                Button("Remove association and delete photos") {
                    Task {
                        try? await vm.removeStorageAssociationsAndDeletePhotos()
                        try? await vm.refreshAlbumAndPhotos()
                    }
                }.buttonStyle(TappedButtonStyle())
                Button("Delete album and images") {
                    Task {
                        try? await vm.deleteAlbumAndPhotos()
                    }
                    albumName = ""
                }.buttonStyle(TappedButtonStyle())
            }.disabled(vm.isLoading)
