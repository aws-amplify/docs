...
For more details on TanStack Query, including requirements, supported browsers, how query keys work, and advanced usage, see the [TanStack Query documentation](https://tanstack.com/query/latest/docs/react/overview). 
For more information on how optimistic updates with TanStack Query work, see the [TanStack Optimistic UI Documentation](https://tanstack.com/query/latest/docs/react/guides/optimistic-updates).
...
TanStack Query Devtools are not required, but are a useful resource for debugging and understanding how TanStack works under the hood. For more information on the TanStack Query Devtools, visit [the docs](https://tanstack.com/query/latest/docs/react/devtools)
...
```js
// TODO - add JS example after approval of TS code snippet
```
...
## Optimistic UI loading and error states
...
For more in depth on the usage of these hooks, see the [TanStack documentation](https://tanstack.com/query/latest/docs/react/guides/mutations).

Below is an example of using these states to render a loading indicator while a mutation is in progress, and an error message if the mutation fails. For more examples, see the [Complete Example](#complete-example) below.
