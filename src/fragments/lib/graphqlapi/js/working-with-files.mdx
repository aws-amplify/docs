...
It is important to understand when these mismatches can occur in your app, and to add meaningful error handling around these edges. For the purposes of these docs, this guide does not include exhaustive error handling, real-time subscriptions, re-querying of outdated records, or attempts to retry failed operations. However, these are all important considerations for a production-level application.
