...
It should be noted that the following examples are not meant to serve as exhaustive documentation for the full functionality of either Storage or the GraphQL API categories, but instead demonstrate best practices with regards to managing associations between the two. Refer to the relevant documentation for more information on the full capabilities of each category.
...
You may want to add multiple files to a single record, such as a photo album with multiple images. For a full working example of multi-file CRUD, see the [Complete Example](#complete-example) below.
...
Multi-file CRUD operations use the same patterns as when working with a single file, with the exception that we are now working with a list of image keys, as opposed to a single image key.
