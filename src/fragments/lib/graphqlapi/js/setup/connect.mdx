...
In this guide you will learn how to connect an existing data stack to your application. This job includes understanding the required client configuration and how to connect the data stack. We will also review functions to create, update, delete, and read your data using the Amplify GraphQL client. Finally, we will look at how to subscribe to real-time events to watch for mutations in your data.
...
==EXPANDER SECTION to provide a deeper dive on authentication types and their differences including API key access, Cognito, IAM, and OIDC. May be an option for a table since a comparison.==

### Recap
==Recap of job completed and a tie-in to real-world use and application of the topic discussed.==
...
==Task forward section with steps to install and configure with an explanation on the flow differences between the Amplify API vs their existing API.==

#### Troubleshooting
==EXPANDER SECTION to provide tips on how to avoid common errors such as: not changing the default auth rules, CLI set up correctly, and troubleshooting and AWS account with the CLI . Each error should have a header with descriptive title to be included on a primary troubleshooting guide as well.==

### Recap
==Recap of job completed and a tie-in to real-world use and application of the topic discussed.==

## Create, update, and delete application data
In this job you will execute mutations to create, update, and delete application data. We will also show you how to cancel these requests. 

> **Before you begin, you will need:**
> - Your application connected to the API

### Execute mutations to create, update, and delete application data
...
==Steps to execute with the Amplify GraphQL client.==
...
==Brief explanation on how canceling mutations works, what happens in the back end, and how to execute the command.==
...
==EXPANDER SECTION that provides and overview of the cost of specific operations (e.g. list vs get, queries and scans) as well as limitations for specific functions (e.g. sorting, open search), items, and filters.==

==Steps to list or get data. This is then followed by explanation how to filter data by variable and steps to filter data.==
...
==EXPANDER SECTION that provides more detail on how our pagination options differ and the benefits of our options.==

### Cancel read requests
==Brief explanation on how canceling read queries works, what happens in the back end, and how to execute the command.==
...
==Explanation how to unsubscribe and when to for reducing redundancy with steps for these actions.==

### Recap
==Recap of job completed and a tie-in to real-world use and application of the topic discussed.==

## Conclusion
Congratulations! You have finished the **Connect a data stack** guide. In this guide, you installed and configured Amplify libraries; used GraphQL mutations for app data requests; filtered app data in Amplify; and set up, filtered, and cancelled subscriptions for real-time events.
