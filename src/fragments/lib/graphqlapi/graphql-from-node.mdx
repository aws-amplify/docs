...
Create a Lambda function with `amplify add function` and make sure to **give access** to your GraphQL API when prompted for in the `amplify add function` flow. Granting access to your API will create the necessary `GRAPHQLAPI` environment variables (below).
