...
## Custom Request Inputs using Encodable Objects

You can create custom query inputs that uses Encodable data structures as the filter.  The filtering object are encoded to JSON object as part of the request.
...
extension GraphQLRequest {
    static func createToDo(filter: CreateTodoInput) throws -> GraphQLRequest<TODO> {
      let operationName = "createTodo"
      let document = """
      query createTodo($id: ID!) {
        \(operationName)(id: $id) {
          id
          name
          description
      ...
      """
      // turn input into data
      guard let data = try? JSONEncoder().encode(filter) else { 
        throw APIError("Failed to encode filter")
      }
      // turn data into `Any` JSON object
