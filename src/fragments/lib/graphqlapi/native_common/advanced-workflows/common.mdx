...
You can custom query inputs that uses Encodable data structures as a filter.  The filtering object are encoded to JSON object as part of the request.
...
struct ProductRequestFilter: Encodable
{
  let name: String
  let brand: String
}
...
extension GraphQLRequest {
    static func getProducts(filter: ProductRequestFilter) throws -> GraphQLRequest<JSONValue> {
      let operationName = "getProducts"
        let document = """
        query ($filter: productRequestFilter!) {
