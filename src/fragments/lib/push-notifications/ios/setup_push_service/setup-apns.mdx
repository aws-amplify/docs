...
_If you already have an ID assigned to your app, you can skip this step. You can use an existing app ID provided it doesn't contain a wildcard character ("\*")._
...
- In the **Registering an App ID** pane, for **Description**, type a custom name for your app ID that makes it easy to recognize later.
...
- Click **Continue**. In the **Confirm your App ID** pane, check that all values were entered correctly. The identifier should match your app ID and bundle ID.
- Click **Register** to register the new app ID.

### Step 2: Create an APNs SSL Certificate

You will create and download the certificate from your Apple Developer account. Then, you will install the certificate in Keychain Access and export it as a `.p12` file.

_If you already have an SSL certificate for your app, you can skip this step._
...
- In the **Generate your certificate** pane, click **Choose File**, and then select the CSR file you created and click **Continue**. 
...
- In **Certificates**, find the certificate you just added. The certificate is named "Apple Push Services:`com.my.app.id`", where `com.my.app.id` is the app ID for which the certificate was created.
...
_If you already have a registered device, you can skip this step._
...
_If you already have an iOS distribution certificate, you can skip this step._
