...
- **Define Auth Challenge**: This trigger defines the sequential series of challenges that the user will need to complete in the flow. These challenges can be custom challenges, as well as traditional SRP_A verification.
...
The Amplify CLI can help you setup the AWS Lambda triggers for you custom authentication flow. In the terminal, navigate to your project, run `amplify add auth`, and choose the following options:
...
The boilerplate for Create Auth Challenge opens in your selected code editor. Enter the following code to this file:
...
The `createAuthChallenge` function is doing two things:

1. Setting '1234' as the expected reply for the custom challenge. This is added to the `privateChallengeParameters` object, and is kept internal to the custom authentication flow.

2. Appending a key/value pair to the `publicChallengeParameters` object, which is returned to your application and can be used to provide instructions to your user.

<Callout>
  This challenge is expecting a simple, arbitrary value for each authentication
  attempt. This is not a secure authentication mechanism and is included for
  demonstration purposes only. For production-ready applications, you would use
  this challenge to create a more secure authentication mechanism, such as using
  SES to deliver a randomly generated code.
</Callout>
