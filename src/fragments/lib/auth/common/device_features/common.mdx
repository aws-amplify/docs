...
Additionally, device tracking is not supported when the User Pool has a configuration to sign-in with `email/phone_number` or `alias` and when the
`signIn` API passes `USER_PASSWORD_AUTH` as the `authFlowType`.
