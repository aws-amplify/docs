...
Refer [SMS sandbox](/lib/auth/sms_flows#sandbox-mode).
...
Sign up users normally with the chosen `Username` type and password. `"phone_number"` will be required in the `userAttributes` if SMS MFA is ON, or manually enabled for the user
...
## Setting up user preferred MFA option
