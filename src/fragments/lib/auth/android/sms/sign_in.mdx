...
Amplify.Auth.signIn(
    ...
    "password",
    result -> {
      if (result.nextStep.getSignInStep() == AuthSignInStep.CONFIRM_SIGN_IN_WITH_SMS_MFA_CODE) {
      String destination = result.nextStep.getCodeDeliveryDetails().getDestination();
        Log.d("SignIn", "SMS code sent to "+ destination);
        Log.d("SignIn", "Additional Info" + result.nextStep.getAdditionalInfo());
