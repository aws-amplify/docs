...
```kotlin
fun customChallenge(response: String) {
    Amplify.Auth.confirmSignIn(
        response,
        { Log.i("AuthQuickstart", "Confirm Sign in succeeded") },
        { Log.e("AuthQuickstart", "Confirm Sign in failed", it) },
    )
}
```
...
You will know the sign in flow is complete if you see the following in your Logcat window:
