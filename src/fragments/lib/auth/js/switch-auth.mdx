...
When using Custom Auth Flows, you can setup a Password or Passwordless Sign In. This decision will determine how your Custom Auth Lambda functions are defined.
...
Here is the sample for creating a CAPTCHA challenge with a custom auth flow.
...
<Block name="Password Sign In">

Password Sign In will need to account for the SRP_A and Password verifier challenges when checking for the session length.
...
Password Sign In will need to account for the `SRP_A` and `PASSWORD_VERIFIER` challenges.
