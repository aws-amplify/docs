...
In order to catch errors when using `Auth.federatedSignIn()` you can use the [Hub](/gen1/[platform]/build-a-backend/utilities/hub/) eventing system provided by Amplify to listen for the `signIn_failure` event.
