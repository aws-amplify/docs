...
userEffect(() => {
  async function createCode() { 
    try {
      const codeSetUp = await Auth.setupTOTP(props.user);
      setCodeTOTP(codeSetUp)
    } catch (error) {
      console.log(error)
    }
  };
  if (props.user?.challengeName !== 'SOFTWARE_TOKEN_MFA') {
    createCode();
  }
}, [props.user])
...
const handleCode = async (event: any) => {
  ...
  try {
    if (props.user?.challengeName !== 'SOFTWARE_TOKEN_MFA') {
      await Auth.verifyTotpToken(props.user, code)
    } else {
      await Auth.confirmSignIn(props.user, code, 'SOFTWARE_TOKEN_MFA')
    }
