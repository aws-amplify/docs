...
// This can also be used if user is already signed in for testing/validation
...
// If signing in with MFA is enabled, use confirmSignIn method to 
// send a new MFA code to be used with Authenticator. 
...
) { const loggedUser = await Auth.confirmSignIn( user, challengeAnswer )}
