...
You can use your own token provider across the library, for example, to use it with AppSync when OIDC is the Auth mode. You must supply the token provider to Amplify via the `Amplify.configure` method call. Below, you can see sample code of how such a custom provider can be built to achieve the use case.
