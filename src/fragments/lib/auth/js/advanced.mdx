...
You can federate into Cognito identity pool by creating your own custom credentials provider. The custom credentials provider you write may accept the login details such as the domain name, token and user information from the third party federation sign in process (Google, Facebook, Apple, Amazon) and exchange them for credentials with Cognito. You must provide the custom credentials provider to Amplify via the `Amplify.configure` method call. Below, you can see a rough psuedo code of how such a custom provider can be built to achieve the use case. 
...
// Note: The custom provider class must implement AWSCredentialsAndIdentityIdProvider
class CustomCredentialsProvider implements AWSCredentialsAndIdentityIdProvider {
  federatedLogin?: {
    domain: string,
    token: string | undefined
  };
  setToken(login?: typeof this.federatedLogin) { // custom method to set the login information
  ...
  async getCredentialsAndIdentityId(
    ...
    try {
      ...
      // You can add in some validation to check if the token is avaialable before proceeding
      ...
      const credentials: AWSCredentialsAndIdentityId = {
        credentials: {
...
```
Now that the custom credentials provider is built and given to `Amplify.configure`, let's look at how you can use the custom credentials provider to finish federation into Cognito identity pool.
      ...
      if (response.status === 'connected') {
        ...
        fb.login(
          response => {
            ...
            customCredentialsProvider.setToken({
              domain: "graph.facebook.com",
              token: response.authResponse.accessToken,
            });
            const fetchSessionResult = await fetchAuthSession(); // will return the AWS credentials
