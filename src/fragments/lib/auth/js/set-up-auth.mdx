...
- A front-end app
- If you are using Amplify resources you need to [install and configure](/lib/project-setup/prereq/q/platform/js/) the Amplify CLI and have an existing Amplify project or create a new one with `amplify init`

## Set up and connect backend resources

Setting up and connecting your backend resources includes understanding how these resources are managed and created. We will also guide you on the paths to integrating Amplify Auth, using different password formats, and the use of additional authentication security methods, such as Multi-factor authentication.

### Decide how to create and manage your backend resources

You can create and manage your Authentication resources with Amplify by using the Amplify CLI, Amplify Studio, or create them outside of Amplify with tools such as CDK and CloudFormation. The path we recommend is via the Amplify CLI, which allows you to create new Amazon Cognito resources or import existing ones. However, you can also use the Amplify Studio console to configure authentication or use existing resources and directly connect them to your application using the Amplify Libraries. These tools will help you with resource provisioning, IAM policies and roles for resources, deployments, local testing, continuous integration and deployment (CI/CD), and environment management.

With Amplify Auth, you can use a username and password as an authentication method, create a fully custom authentication flow, or use a social provider to validate the identity of the user (known as Federation).
...
Before you implement Auth on your front-end application, you will want to evaluate the options of using the Authenticator connected UI component, using the Amplify Library APIs, or Cognito Hosted UI for your use case. We recommend using the Authenticator since it creates a sign in and registration experience for your with few lines of code. You can also use Authenticator together with Amplify Libraries methods to extend capabilities when needed.
...
| Constraints | Dependent on Amplify CLI for provisioning resources. | Building the UI and managing the authentication state is custom built. | Limited ability to manage the look and feel of the Hosted UI page from Cognito. |
...
Before you begin, you should have finished [*Set up and connected backend resources with a configured Auth category*](#set-up-and-connect-backend-resources).
