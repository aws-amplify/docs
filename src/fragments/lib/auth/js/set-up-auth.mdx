...
2. Social provider federation which shares data with third parties. 
3. Multi-factor authentication which adds a layer of security at sign-in but may also add friction for your users.
...
3. Enable One-Time-Passwords for MFA flows to improve user verification.
...
- Password strength: You can set minimum length and character types to fortify user passwords.
...
You can require the end-user to verify their contact information before their account is activated. This verification is usually set up when configuring the Cognito User Pool. Only email and phone can be verified. Email verification asks the end-user to click a link sent to their email address provided during sign-up, or enter a code that is sent to the users email. Phone verification is done by asking the end-user to enter the code sent to them in a SMS text message. You do have the option to require both verification methods.
...
There are three ways to add authentication capabilities to your application: Authenticator UI component, Amplify Libraries, or Hosted UI. Select from the options below to complete the steps to set up a simple password/username authentication for your application.
...
By default, each user that sign-up remains in the unconfirmed status, until verify with a confirmation code that was set to their email or phonenumber. You can confirm the sign-up after retrieving a confirmation code from the user as shown here.
