...
export async function signUp(signUpParams: SignUpParams) {
  try {
    const { user } = await Auth.signUp({
      ...
      attributes: {
        email: signUpParams.email, // optional
        phone_number: signUpParams.phoneNumber, // optional - E.164 number convention
...
interface ResendConfCodeParams {
  username: string;
}

export async function resendConfirmationCode(resendConfCodeParams: ResendConfCodeParams) {
  try {
    await Auth.resendSignUp(resendConfCodeParams.username);
...
export async function confirmSignUp(confirmSignUpParams: ConfirmSignUpParams) {
  try {
    await Auth.confirmSignUp(confirmSignUpParams.username, confirmSignUpParams.code);
  } catch (error) {
...
export async function signIn(signInParams: SignInParams) {
  try {
    const user = await Auth.signIn(signInParams.username, signInParams.password);
  } catch (error) {
