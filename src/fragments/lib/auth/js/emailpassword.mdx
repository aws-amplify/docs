...
After a user has finished signup, they can proceed to sign in.  When signing in with user name and password, you will pass in the username and the password to the signIn method of the Auth class. 
...
async function signInWithCustomChallenge() {
  try {
    ...
    if (user.challengeName === `CUSTOM_CHALLENGE`) {
      const loggedUser = await Auth.confirmSignIn(
        user, // Return object from Auth.signIn()
        customChallengeAnswer, // Can be a code, string, CAPTCHA, etc.
        `CUSTOM_CHALLENGE`
      );
      console.log(loggedUser)
    } else { 
...
New users can be created from the [Cognito console](https://console.aws.amazon.com/cognito/home) for your app, requiring the user to input a new password and required attributes (optional) when signing in. If the user object returned by `Auth.signIn()` contains the `NEW_PASSWORD_REQUIRED` for the `challengeParam`, you can submit the data using the `Auth.completeNewPassword()` method as well.
