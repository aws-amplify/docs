...
When Amplify adds a new enumeration value (e.g., a new enum class entry or sealed class subtype in Kotlin, or a new enum value in Swift), it will publish a new minor version of the Amplify Library. Plugins that switch over enumeration values should include default handlers (an else branch in Kotlin or a default statement in Swift) to ensure that they are not impacted by new enumeration values. 
...
import iosContinueSignInWithMFASelection from "src/fragments/lib/auth/ios/signin_next_steps/90_continue_mfa_selection.mdx";

<Fragments fragments={{ios: iosContinueSignInWithMFASelection}} />

### Continue signin with MFA Setup
