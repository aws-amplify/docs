...
Amplify JS can be used with Next.js' App Router (Next.js v13.4+) with a few important changes to how you configure and use the library, namely:

- Amplify JS must be explicitly configured in both your client-side and server-side code (with `ssr` enabled)
- An appropriate request object will need to be prepared for use with `withSSRContext` when performing operations on the server that require authentication

Example of a Next.js App Router Server Component that uses Amplify JS:
