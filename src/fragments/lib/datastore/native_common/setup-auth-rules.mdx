...
* Owners can create and delete; other signed-in users can update and read (read includes query and subscription).
```graphql
type YourModel
  @model
  @auth(
...
* Users belonging to the "Admin" group can create and delete, other signed users can update and read (read includes query and subscription).
```graphql
type YourModel
  @model
  @auth(
