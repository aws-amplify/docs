...
### Many-to-many with Custom Primary Keys:

[TODO: explain CPK diff] A schema using custom primary keys with a `many-to-many` relationship is converted into a `hasMany` / `belongsTo` relationship with a join table.
...
### Examples using Custom Primary Key:
