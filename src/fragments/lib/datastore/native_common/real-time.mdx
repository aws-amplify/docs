...
`observeQuery` still reacts dynamically to updates of data after the sync process is complete, sending a singular snapshot for every change occuring to the observed Model (just as `observe` does) as long as the subscription is maintained.
