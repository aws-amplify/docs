...
- `errorHandler` - adds a handler function to be applied when the DataStore encounters an unrecoverable error in one of its ongoing background operations (model synchronization).
- `conflictHandler` - adds a handler that will be invoked whenever there is a conflict between two model instances, one in the local store, and one from the remote server, as received from a sync operation.
...
- `syncInterval` - sets the duration of time after which delta syncs will not be preferred over base syncs. Time unit is default as minute.
- `doSyncRetry` - sets the retry enabled on datastore sync.
