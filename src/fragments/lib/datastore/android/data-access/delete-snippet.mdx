...
Deletion can also be achieved by passing an instance and a predicate into `Amplify.DataStore.delete()`. The data will only be deleted if it meets the specific conditions.
Below, we delete posts with a `rating` greater than `4`:
