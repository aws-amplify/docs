...
As of v6 of Amplify, you will now import the functional API’s directly from the `aws-amplify/analytics` path as shown below. Red lines of code are v5 and Green ones are v6:
...
+ });
```
Additionally, you can identify an user with Analytics with this API, 
...
```
Further, you can stream analytics data to kinesis. To configure Kinesis with Amplify, refer to the [Installation and Cofiguration](https://docs.amplify.aws/react/build-a-backend/more-features/analytics/streaming-data/#installation-and-configuration) section of the docmentation. This is an example on how to stream and flush as needed, 
...
flushEvents();
```
Finally, let's look at the difference in how to personlaize recomendations,
...
As of v6 of Amplify, you will now import the functional API’s directly from the `aws-amplify/in-app-messaging` path as shown below. Red lines of code are v5 and Green ones are v6:
...
| Difference | v5 | v6 |
| ---- | ---- | ----------- |
| API input | userId, userInfo | userId, userProfile, options |
| `attributes` property | `attributes` under `userInfo` | `customProperties` under `userProfile` |
...
```
Finally we look at the differences in responding to interactions events. 
