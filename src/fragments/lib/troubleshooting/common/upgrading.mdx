...
<BlockSwitcher>
  <Block name="Web">
    ...
    You can also upgrade by making sure the version of `aws-amplify` in your package.json file is `latest` or `^6.0.1` and re-installing dependencies by running `npm install`.
  ...
  <Block name="React Native">
    In v6, you will no longer need the `amazon-cognito-identity-js` package, so that dependency should be removed from your package.json.
