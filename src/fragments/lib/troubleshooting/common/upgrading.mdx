...
If you would like to use Geo, Predictions, PubSub, or Interactions you will need to install those packages separately. (see [category-specific migration instructions](migration-guide/#4-upgrade-category-usage))
...
* [API (GraphQL) - Configure the default authorization mode](/[platform]/build-a-backend/graphqlapi/connect-to-api/#configure-the-default-authorization-mode)
...
* [Analytics - Storing analytics data (Kinesis Firehose)](/[platform]/build-a-backend/more-features/analytics/storing-data/#installation-and-configuration)
* [Analytics - Personalized recommendations](/[platform]/build-a-backend/more-features/analytics/personalize-recommendations/#installation-and-configuration)
...
<Callout>DataStore is no longer supported in an SSR context: if you are using DataStore within your SSR project, you will need to migrate to the API category.</Callout>

<Accordion title='Auth' headingLevel='4'>
...
<Accordion title='Analytics' headingLevel='4'>
...
<Accordion title='API (Rest)' headingLevel='4'>
...
For a deeper look at how the REST API functionality in V6, check out our [API (REST) category documentation](http://lib/restapi/getting-started/q/platform/js/).

</Accordion>

<Accordion title='In App Messaging' headingLevel='4'>
...
<Callout>Make sure that the @aws-amplify/interactions package has the same version number as the aws-amplify package in your package.json file.</Callout>
...
<Accordion title='Predictions' headingLevel='4'>
...
<Callout>Make sure that the `@aws-amplify/predictions` package has the same version number as the `aws-amplify` package in your `package.json` file.</Callout>
...
<Accordion title='PubSub' headingLevel='4'>
...
For a deeper look at how the PubSub functionality in V6, check out our [PubSub category documentation](http://lib/pubsub/subunsub/q/platform/js/).

</Accordion>

<Accordion title='Storage' headingLevel='4'>
...
// Set up Amplify Storage with Manual configuration
Amplify.configure({
  Auth: {
...
For a deeper look at how the Storage functionality in V6, check out our [Storage category documentation](http://lib/storage/getting-started/q/platform/js/).

</Accordion>

<Accordion title='Utilities' headingLevel='4'>
...
<Accordion title='Server-side Rendering' headingLevel='4'>
