...
+ });
```
Additionally, you can identify an user with Analytics with `identifyUser` API, 
...
```
Further, you can stream analytics data to Kinesis. To configure Kinesis with Amplify, refer to the [Installation and Configuration](https://docs.amplify.aws/react/build-a-backend/more-features/analytics/streaming-data/#installation-and-configuration) section of the documentation. This is an example on how to stream data and flush events as needed, 
...
<Accordion title='Push Notifications' headingLevel='4'>
...
For a deeper look at Push Notifications functionality in v6, check out our [Push Notifications category documentation](/[platform]/build-a-backend/push-notifications/).
