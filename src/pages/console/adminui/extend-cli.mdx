...
This message occurs if the Amplify project has an `override.ts` file present in the resources.

To resolve this error.

Remove the `override.ts` file and run `amplify push` or continue utilizing Amplify CLI to build and push your resources.
 
While you may not be able to run a deploy on Amplify Studio, resources such a UI Builder and Manage should not be effected by this.
