...
This behavior is observed when running a Amplify CLI headless pull command in the terminal, which opens a new tab in the browser with a Amplify Studio login screen. On logging into the Amplify Studio application we observe the terminal prompt is in a loading state.

To resolve this issue,

1. Exit from the Amplify CLI prompt
1. If present, remove the empty folders created in your present directory (Ex: `amplify` or `amplify-backup`) from the previous pull
1. Rerun the Amplify CLI headless pull command
...
This message occurs when running a Amplify CLI headless pull command in the terminal and allowing access permission to the CLI. 

To resolve this issue,

1. Delete the `.config.json` file present at the location `<home-dir>/.amplify/admin/`
1. If present, remove the empty folders created in your present directory (Ex: `amplify` or `amplify-backup`) from the previous pull
1. Rerun the Amplify CLI headless pull command in the terminal and allow access on the prompt in the browser.

<Callout>

Note, the content of `<home-dir>/.amplify/config.json` is utilized to store JWTs to vend temporary AWS credentials and should __NOT__ be shared.
