...
### Nesting Collections
Slots within collections allow you to render additional nested collections. For example, use nested collections to generate a collection of posts, each with its own collection of comments.  
...
By nesting the `CommentViewCollection` within the `comment` slot in the `AmpligramCollection`, and using the `overrideItems` prop, we can render each Post with its related Comments.
