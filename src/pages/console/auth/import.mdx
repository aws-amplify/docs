...
Importted Amazon Cognito resource is particularly useful if you’re trying to do the following:
...
It is best practice to create atleast two app clients with the following conditions:
...
[Learn more about getting started with User Pools](https://docs.aws.amazon.com/cognito/latest/developerguide/getting-started-with-cognito-user-pools.html) or [configuring a User Pool App Client](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-settings-client-apps.html)

We can also use Amplify CLI to [import an Cognito user pool resource](/cli/auth/import/).
...
We can also use Amplify CLI to [import an Cognito identity pool resource](/cli/auth/import/).
