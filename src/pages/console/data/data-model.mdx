...
**[DataStore](/lib/datastore/getting-started)** is an on-device storage engine that automatically synchronizes data between your mobile and web apps and your database, enabling offline use. DataStore is automatically enabled for your API when you add a **Conflict Resolution** method. 

<Callout warning>

**IMPORTANT NOTE**: Changing your Conflict Resolution setting results is a **destructive data change**, and may result in inaccessible data. It's highly recommended that this setting only be changed on GraphQL APIs with **no data**. 

</Callout>

To enable/disable Conflict Resolution, or to change your Conflict Resolution settings, take the following steps:
