...
Using DataStore and direct GraphQL require different query syntax and structure. Make sure to use the appropriate syntax for your API. 

### Destructive data changes

You can change the settings for your GraphQL API at any time via Studio or the CLI. However, turning Conflict Resolution off is a **destructive** data change. 
...
If Conflict Resolution is disabled, these fields are **permanently and irreversibly** removed from your data model. This will have no immediate impact on your app, so long as you are using [GraphQL syntax](/lib/graphqlapi/mutate-data/q/platform/js/) to interact with your data. However, if Conflict Resolution is later re-enabled, any existing data in the API will be inaccessible until the required fields are set for all existing records. 
