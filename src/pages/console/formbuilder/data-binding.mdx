export const meta = {
  title: 'Data binding and relationships',
  description:
...
### Ad hoc forms
Ad Hoc Forms are standalone React components that can be used in any React or Nextjs project, even without an AWS account. Upon submission, the input values for the form are [accessible via the `onSubmit` hook for handling](/console/formbuilder/lifecycle/#handle-form-data-submissions---onsubmit). 

You can build ad hoc forms without ever logging into AWS [using the Amplify Sandbox](https://sandbox.amplifyapp.com/ui-library). 

## Types of forms

All connected and ad hoc forms are either a **Create** form or an **Update** form. 
...
For ad hoc update forms, you can pass an object to the `initialData` prop to pre-populate the form. However, as with all ad hoc forms, you must handle the form submission in code. 

## Customizing data binding

### Connect an ad hoc form

If you want to convert an ad hoc form to a connected form, you can do so from within the form configuration menu. To configure your form's data model mapping:
1. [Log into Amplify Studio](/console/adminui/start/) and select the **UI Library** from the left-hand navigation bar
1. Select your ad hoc form and select **Configure** in the upper right-hand corner
...
**Select from an existing model**: 
  * If your form matches the data model, Studio will bind them together, converting your ad hoc form to a connected form
  * If your form doesn't match the data model, Studio will add fields to match to your form (or schema) to ensure a match

**Map data in code**: Use this option to ignore data mapping, and keep this form ad hoc
...
1. [Log into Amplify Studio](/console/adminui/start/) and select the **UI Library** from the left-hand navigation bar
1. Select your connected form and select **Configure** in the upper right-hand corner (*hint: you can configure a default form!*)
