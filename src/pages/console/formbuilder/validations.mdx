...
### Available validation rules within Studio
...
<ProductForm
  onValidate={{
    price: {
      currency: (value, validationResponse) => { // Pass validation function to match the nested object
...
<AgentContactForm
  onValidate={{
    licenseNumber: (value, validationResponse) => {
      ...
      return fetch(`http://localhost:3000/api/agent/${value}`)
        .then(response => {
          if (response.status !== 200) {
            return { // If the request failed, return a validation error
              hasError: true,
