...
Use sectional elements to divide your form up into multiple parts. This is useful to represent nested data and their designated form inputs into individual sections.
...
[Select Field](TK) and [Radio Group Field](TK) require a set of options for your users to choose from. For example, a "Status" input can only have the options "Not started", "In progress", and "Done".
...
Every form input can made to accept multiple values. The form input gets "wrapped" around by a "list input" component to allow your users to add, update, and remove items.

1. Select a form input in a form that's not connected to a data model
2. Go to **Data** 
...
**Note:** If your form is connected to data model, then form data will be saved to the cloud upon clicking **Submit**. Careful to not accidentally add unintended records into a production database.
...
Delete a form input by selecting the form input and then select **Action** > **Delete**.