...
Amplify Studio's Form Builder automatically generates cloud-connected forms as React code, either from your data model, or from scratch. You can configure validation logic, adjust theming, and customize presentation all within the console. 
...
## Option 3: Generate a form from JSON

Amplify Studio can generate custom forms from a JSON text. This is particularly useful if you are interfacing with another API - most API endpoints expect input data in JSON format, so you can quickly generate a form by pasting a sample payload. 
...
From the JSON block, Amplify Studio automatically...
