...
**Note:** An Admin can reset a user's password by going into the Cognito Userpool, selecting the user, and choosing "Reset password" under the *Actions* dropdown. This can also be done programmatically using the Cognito API Action `AdminResetUserPassword`. Learn more about [resetting a users password as an Admin](https://docs.aws.amazon.com/cognito-user-identity-pools/latest/APIReference/API_AdminResetUserPassword.html).
