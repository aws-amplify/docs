...
Note that once your User Pool has been created when using Amplify CLI to set up backend resources (via `amplify add auth`), you can only select the 'OPTIONAL' configuration for MFA when updating with (via `amplify update auth`).  Similarly, once MFA has been enabled for a User pool you cannot disable it (via `amplify update auth`).
...
You can use Short Message Service (SMS) as a second layer of authentication with MFA. When SMS is enabled, your users will get an authentication code via a text message to complete sign-in after they sign in with their username and password.
