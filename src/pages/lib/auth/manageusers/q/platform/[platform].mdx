...
Note that if you need to retrieve the latest user attributes after they have been changed, you will need to bypass the local cache like so:
...
```
The `bypassCache` parameter defaults to false, and the alternative to retrieve the latest user attributes is to have the user sign out and sign back in.
