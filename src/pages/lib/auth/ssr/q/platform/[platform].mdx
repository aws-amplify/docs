...
**Note:** When using `getServerSideProps` each request (`req`) on the server creates a copy of Amplify that scopes the credentials and the user session to *just one* request. 
