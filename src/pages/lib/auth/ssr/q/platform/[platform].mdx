...
## Use Auth with SSR with Next.js Pages Router

You can implement user authentication on both the client-side and server-side using Amplify Auth. Server-side code can access user data to render pages using Server-Side Rendering. [Server-Side data fetching in Next.js](https://nextjs.org/docs/pages/building-your-application/data-fetching/get-server-side-props) allows you to render pages on the server on each request. This helps you set up secure data on pages that can be rendered based on the state of the authenticated user.

To start working with Amplify Auth on your Server-Side pages, you will first enable SSR by updating the Amplify configuration on your app. When using the Amplify CLI, the **aws-exports.js** file is created and updated automatically for you based upon the resources you added and configured. For client-only apps, `Amplify.configure(awsExports)` is all you need. However, to enable SSR support, you will also add `ssr: true`:
...
This will give you access to the currently authenticated user with the request object. This SSR configuration will need to be done with each SSR configuration - in this case it is returning data for the authenticated user.

<Callout>

**Note:** Server-side functions that don't have a `req` object (e.g. Next.js' `getStaticProps` and `getStaticPaths`) should still use `withSSRContext()`. Please note that it is **not** possible to perform authenticated requests with Amplify when using functions that do not have a `req`.

</Callout>

### Use Auth with Next.js to securely fetch data at request time 

#### Doesn't make sense to include this section that talks more about API/GraphQL under Auth

When you need a page with data rendered server-side at request time you can use the `getServerSideProps` method with [Next.js Pages Router](https://nextjs.org/docs/pages/building-your-application/data-fetching/get-server-side-props). This will ensure that pages are pre-rendered on each request and pass any data as `props`.

When you combine this with `withSSRContext` you can also call the authenticated API route directly from `getServerSideProps` to call your data. This will allow you to limit data to authenticated users. Here is an example fetching from a To do app:
...
Next, prepare a request object for `withSSRContext` to perform server-side operations that require authentication.  Here is an example of a Next.js App Router Server Component that queries data from GraphQL (e.g. `app/posts/page.tsx`):
...
For example, you can create and manage persistent sessions in a database by interacting with Amplify Auth in a Server Action. For example, you can add an action to check for authentication:
...
#### This doesn't currently work with Amplify V5

You can also use [Middleware in Next.js](https://nextjs.org/docs/pages/building-your-application/routing/middleware) to run code before loading the page and then reuse this logic to run authentication checks across multiple pages. 
...
### Use Auth and HTTP-only cookies with Next.js

#### This also seems incorrect

HTTP-only cookies provide secure server-side session persistence that you can use with SSR in Next.js. This method of authentication will allow you to keep your permanent data separate from ephemeral encrypted sessions and maintain user session access across requests without passing tokens to the frontend client. These cookies are automatically sent on each request and can be easily purged when your user signs out.

```js
TODO
```

You can now use the Next.js App Router with authentication for SSR and add additional customizations with Server Actions, Middleware, and HTTP-only cookies.
