...
Server-Side Rendering (SSR) can help improve your search engine optimization, security, and load speed. Amplify JavaScript requires slight modifications to work more efficiently on the server-side. In this guide we will review how SSR helps with keeping your apps secure and how to implement Auth with SSR.
...
You can implement Amplify Auth on both the client-side and server-side of your application. [Server-Side Rendering in Next.js](https://nextjs.org/docs/pages/building-your-application/data-fetching/get-server-side-props) allows you to render pages on the server on each request. This helps you determine if the content should be rendered and returned based on the current user's authentication state.
...
To start working with Amplify Auth on your Server-Side pages, you will first need to enable SSR by updating the Amplify configuration on your app and then update your client-side and server-side code.

### Enable SSR support

When using the Amplify CLI, the **aws-exports.js** file is created and updated automatically for you based upon the resources you added and configured. For client-only apps, `Amplify.configure(awsExports)` is all you need. However, to enable SSR support, you will also add `ssr: true`:
...
### Initialize Amplify Auth on client and server-side
...
For client-side code rendered in the browser, you will use the top-level imports:
...
But when on the server, you will use Auth from `withSSRContext()` with the `getServerSideProps` function:
...
You will need to run `Amplify.configure({ ...awsExports, ssr: true })` in **both** the client-side and server-side code. In order to use Amplify with Next.js App Router, you **must** run `Amplify.configure()` in both Client and Server Components. The `ssr` option **must** be enabled.
...
Then you can prepare a request object for `withSSRContext` to perform server-side operations that require authentication.  Here is an example of a Next.js App Router Server Component that checks for the current authenticated user:
...
export default async function Page() {
  // Construct a req object and prepare an SSR enabled version of Amplify
  const req = {
    headers: {
