...
This method can be used to check if a user is signed. It throws an error if the user is not authenticated. 
...
You can update the storage mechanism to choose where and how tokens are persisted in your application.
The default option is `localStorage`. Additionally, you can import the `sessionStorage`, `sharedInMemoryStorage` or `CookieStorage` options as well. 
