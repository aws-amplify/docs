...
__Note:__ You do not need to specify the `createdAt` field. Amplify will automatically populate this field for you.
...
__Note:__ Join table records must be deleted before deleting the associated records. For example, for a many-to-many relationship between Posts and Tags, delete the PostTags join record before deleting a Post or Tag.
...
const todo = await client.graphql<GraphQLQuery<CreateTodoMutation>>({
  query: mutations.createTodo,
  variables: {
    input: {
      id: 'some_id',
