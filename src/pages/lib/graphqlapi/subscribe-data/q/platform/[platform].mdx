...
In this guide, we outline the benefits of enabling real-time data integrations and how to set up and filter these subscriptions. We will also cover how to unsubscribe from subscriptions when needed.
...
Subscriptions is a GraphQL feature that allows the server to send data to its clients when a specific event happens. For example, you can subscribe to an event when a new record is created, updated, or deleted via the API. You can enable real-time data integration in your app with a subscription.
...
## Filter real-time subscriptions server-side
...
const sub = API.graphql<GraphQLSubscription<OnCreateTodoSubscription>>(
  { query: subscriptions.onCreateTodo, variables }
).subscribe({
  next: ({ provider, value }) => console.log({ provider, value }),
  error: error => console.warn(error)
});
...
Congratulations! You have finished the **Subscribe to real-time events** guide. In this guide, you set up subscriptions for real-time events and understand how to filter and cancel these subscriptions when needed.
