...
Amplify CLI auto-generates both `list` and `get` queries in your GraphQL API for any `@model` type. A `list` will get you several items, such as Todo items, and does not require you to pass an identifier for a specific record. This is best suited for getting an overview or summary of items or you can enhance the `list` operation with filters by specific criteria across a list of items. When you want to query a single entry by an identifier, you would use `get` to retrieve a specific Todo item.

<Callout>
__Note:__ The cost structure of your underlying data source can impact the cost to run some queries. For example, the `list` operation uses DynamoDB "scan operations" which can use more read request units than the `get` operation. You will want to review the associated costs for these operations for your data source. In our example case here we are using Amazon DynamoDB and you can [learn more about how DynamoDB pricing is calculated](https://aws.amazon.com/dynamodb/pricing/) on their site.
...
<Accordion title='Troubleshoot unauthorized error' headingLevel='4' eyebrow='Troubleshooting'>
