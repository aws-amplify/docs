...
<InlineFilter filters={["js", "react-native", "ios", "flutter", "android"]}>

Authorization rules operate on the deny-by-default principle which means that if an authorization rule is not specifically configured, it is denied. You can open up access to your models through auth rules for queries, mutations, and subscriptions. Updating these rules enables you to allow broader access for non-sensitive data while implementing the principle of least privilege for access to sensitive data.

In this guide we will review ways you can further customize auth rules including applying rules to the global, model, and field levels, customizing your authorization strategy, using rules with other auth providers, and combining multiple auth rules. We will also show how you can use the auth rules access control matrix for visualizing your policy design.
...
Review the example below for a definition of the `@auth` directive:
```graphql
...
**Note:** In the *Connect your app code to the API* guide we covered how to, "Configure the default authorization mode." If you are looking to implement the default authorization mode via an API Key, an Amazon Cognito User Pool, AWS IAM, OpenID Connect (OIDC), or Lambda Authorizer, [please review the guide](/lib/graphqlapi/connect#configure-the-amplify-library).

</Callout>

Before you begin, you will need:

* An [application connected to the API](/lib/graphqlapi/connect)

</InlineFilter>

<InlineFilter filters={["ios", "flutter", "android"]}>

<Callout>

**Note:** See the *[Configure authorization modes](/lib/graphqlapi/authz)* guide to review how to implement the default authorization mode via an API Key, an Amazon Cognito User Pool, AWS IAM, OpenID Connect (OIDC), or Lambda Authorizer.

</Callout>

Before you begin, you will need:

* An [application connected to the API](/lib/graphqlapi/getting-started)

</InlineFilter>

## Review auth rule levels
...
To help you get started, there is a global authorization rule defined when you create a new GraphQL schema. For production environments, you should remove the global authorization rule and apply rules on each model instead.
...
The global authorization rule is applied to every data model in the GraphQL schema. The rule in this case is `{ allow: public }` which allows anyone to create, read, update, and delete data.
...
A model authorization rule sets access for an entire model type in your GraphQL schema. In the example below, each signed-in user (also known as "owner") of a Todo can create, read, update, and delete their own Todos:
...
Amplify also allows you to restrict the allowed operations, combine multiple authorization rules, and apply fine-grained field-level authorization. In the example below, everyone (`public`) can read every Todo but authenticated users (`owner`) can create, read, update, and delete their own Todos:
...
- Any signed in user has read access to the list of employee `name` and `email` fields
...
You should now be able to apply rules at the appropriate level for your use case.

## Understand authorization strategies and providers
...
| Recommended for a production environment's public data access. Public data access is granted for unauthenticated users or devices using AWS IAM controls. | [`public`](#public-data-access) | `iam` |
| Per user data access. Access is restricted to the "owner" of a record. Leverages the `amplify add auth` Cognito user pool by default. | [`owner`](#per-user-or-owner-based-data-access) | `userPools` / `oidc` |
...
**Note:** See the *[Connect your app code to the API](/lib/graphqlapi/connect#configure-the-amplify-library)* guide to review how to implement the default authorization mode via an API Key, an Amazon Cognito User Pool, AWS IAM, OpenID Connect (OIDC), or Lambda Authorizer.
...
**Note:** Do not set `ownerField` to your `@primaryKey` field or `id` field if no primary key is specified. If you want to query by the `ownerField`, use an `@index` on that `ownerField` to create a secondary index.
...
In the example below, upon record creation, the `authors` list is populated with the creator of the record. The creator can then update the `authors` field with additional users. Any user listed in the `authors` field can access the record.
...
**Note:** If you want to restrict a record's access to a specific user, see [Per-user / owner-based data access](#per-user-or-owner-based-data-access) above. `private` authorization applies the authorization rule to **every** signed-in user to access.
...
If you have a connected child model that allows `private` level access, any user authorized to fetch it from the parent model can also read the connected child model. For example, in the relationship below, the owner of a `Todo` record can query all the tasks connected to it, since the `Task` model allows `private` read access.
...
You can also use OpenID Connect with `private` authorization. See [Use OpenID Connect as an authorization provider](#use-oidc-as-an-authorization-provider).
...
**Dynamic group authorization**: When you want to restrict access to a set of user groups, you can use dynamic group authorization. Each record contains an attribute specifying what Cognito groups should be able to access it. Use the `groupsField` argument to specify which attribute in the underlying data store holds this group information. To specify that a single group should have access, use a field of type `String`. To specify that multiple groups should have access, use a field of type `[String]`:
...
By default, `group` authorization leverages Amazon Cognito user pool groups but you can also use OpenID Connect with `group` authorization. See [Use OpenID Connect as an authorization provider](#use-oidc-as-an-authorization-provider).
...
<Accordion title='Grant Lambda function access to GraphQL API' headingLevel='4' eyebrow='Learn more'>

Lambda functions' IAM execution role do not immediately grant access to Amplify's GraphQL API because the API operates on a "deny-by-default"-basis. Access needs to be explicitly granted. Depending on how your function is deployed, the workflows slightly differ.
...
Refer to the [sample code](/lib/graphqlapi/graphql-from-nodejs#iam-authorization) to learn how to sign the request to call the GraphQL API using IAM authorization.
...
To configure a Lambda function as the authorization mode, set the `lambdaConfig` in the CDK construct. Use the `ttl` to designate the token expiry time.
...
You can now apply an authorization strategy with the related providers.

## Use OIDC as an authorization provider

As seen above, `private`, `owner`, and `group` authorization can be configured with an OpenID Connect (OIDC) authorization mode. 

First, add `provider: oidc` to the authorization rule.
...
Then you can implement into your app code. The sample code below highlights the supported authorization strategies with an `oidc` authorization provider.
...
You can now use the OIDC provider for your authorization rules when needed.

## Combine multiple auth rules

There are several ways you can combine authorization rules to define your auth strategy, provider, and operations at the global, model, and field levels. Because auth rules are set up with deny by default, you can be strategic in how you open up and layer this access. 

When combining multiple authorization rules, they are "logically OR"-ed. In the example below, any user (signed in or not, verified by IAM) is allowed to read all posts and owners are allowed to create, read, update, and delete their own posts.
...
As you customize your auth rules it can be helpful to visualize and validate relationships between your authorization mechanisms via the access control matrix. You can map who can perform specific operations and see this based on your provider and authorization strategy.
...
Congratulations! You have finished the **Customize your auth rules** guide. In this guide, you learned about the different levels and strategies for authorization and then reviewed individual examples as well as an example of how to combine them.

## Next steps

Our recommended next step is to explore more customizations you can create for your data:
