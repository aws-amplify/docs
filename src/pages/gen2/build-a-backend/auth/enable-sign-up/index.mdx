...
You can also utilize the prebuilt and managed Cognito hosted UI to have your users sign up and sign in to your application. The benefit of this includes not having to build out the login UI, however note that customizing the UI is also limited. Additionally, understand that the login process will involve the user being first redirected from your app to the Hosted UI, then after completing the authentication, getting redirected back to you app.

Initiating this flow only requires calling the `signInWithRedirect()` API without passing any parameters.
...
**Note:** V6 of `aws-amplify` has in-built support for using the in-app browser for React-Native apps. However for older versions, you will need to setup a custom `urlOpener` that utilizes a plugin such as [`react-native-inappbrowser-reborn`](https://www.npmjs.com/package/react-native-inappbrowser-reborn) to use the in app browser instead of opening the default browser application on the users phone.
