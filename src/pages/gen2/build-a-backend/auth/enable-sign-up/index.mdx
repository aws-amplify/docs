...
NOTE: V6 of `aws-amplify` has in-built support for using the in-app browser for React-Native apps. However for older versions, you will need to setup a custom `urlOpener` that utilizes a plugin such as `react-native-inappbrowser-reborn` to use the in app browser instead of opening the default browser application on teh users phone.
