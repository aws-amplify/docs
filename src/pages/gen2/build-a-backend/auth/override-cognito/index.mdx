...
TODO Update below after confirming above TODO on override as noted in line 1
...
Apply all the overrides in the `override(...)` function. For example, you can update the temporary password validity days for your Cognito user pool as illustrated in the example below.
...
Removing or adding an attribute on a Cognito userpool schema including default attributes (such as `email`) will cause errors such as `Invalid AttributeDataType input`, consider using the provided `AttributeDataType enum` as CloudFormation interprets this as a schema change. 
...
Apply all the overrides in the `override(...)` function. For example, you can add a path to the lambda execution role that facilitates the user pool group to role mapping as illustrated in the example below.
...
Apply all the overrides in the `override(...)` function. For example, you can add social providers to your Cognito user pool as illustrated in the example below.
