...
Create a custom real-time subscription for any mutation. This is typically needed to accomplish PubSub use cases or real-time notifications for your custom mutation. 
...
## (Optionally) Add subscription filters
...
If you want to customize the filters, you can further configure the subscription handler. For this example, we'll allow a customer to pass in a `namePrefix` parameter that'll allow the end users to only receive channel events in channels that start with the `namePrefix`.
