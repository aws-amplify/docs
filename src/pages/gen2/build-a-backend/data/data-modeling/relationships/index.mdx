...
Retrieving an entity's related data can be expensive, so Amplify Data offers a way to specify lazy or eager loading of relationships. This is done through "custom selection sets." With eager loading, related data is retrieved immediately when the entity is loaded. With lazy loading, related data is only retrieved when explicitly requested. Custom selection sets give you control over when and how an entity's relationships are loaded and represented within your app code..

 ## Types of relationships

|Relationship|Description|Example|
|------------|--------|--|
|`a.hasMany()`|Creates a one-directional, one-to-many relationship between two models. | A **Team** has many **Member**s. This allows you to query all the comments from the post record.|
|`a.manyToMany()`| Creates a bidirectional relationship. Must be defined on both related models. Many-to-many relationships are only supported for DynamoDB data sources. | A **Post** has many **Tag**s and a **Tag** has many **Post**s.|
|`a.hasOne()`|Creates a one-directional, one-to-one relationship between two models. | A **Customer** has one **Cart**. This allows you to query the active shopping cart from the customer record.|
|`a.belongsTo()`|Makes a "has one" or "has many" relationship bidirectional so you can read from both sides of the relationship. | A **Team** has many **Member**s and a **Member** belongs to a **Team**. This allows you to reference the team from the member record and vice versa.|
...
```ts
const { data: postWithTags } = await client.models.Post.get({
  id: window.prompt("ID") ?? ""
}, {
  selectionSet: ["tags.*", "tags.tag.*"]
})

const tags = postWithTags.tags.map(postTag => postTag.tag)
```
...
const cart = customerWithCart.activeCart;
```
