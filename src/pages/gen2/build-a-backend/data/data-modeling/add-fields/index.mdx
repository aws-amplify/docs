...
Amplify Data supports all AWS AppSync scalar types as field types. The following scalar types are available for every field:

|Field type|Description|TypeScript validation|GraphQL Scalar Type|
|-|-|-|-|
|`a.id()`|A unique identifier for an object. This scalar is serialized like a String but isn't meant to be human-readable. If not specified on create operations, a ULID (Universally Unique Lexicographically Sortable Identifier) will be generated.|`string`|ID|
...
|`a.json()`|A JSON string. Any valid JSON construct is automatically parsed and loaded in the resolver code as maps, lists, or scalar values, rather than as the literal input strings. Unquoted strings or otherwise invalid JSON result in a GraphQL validation error.|`any`|AWSJSON|
...
```ts
function PostConfig() {
  const enums = await client.enums.PrivacySetting.values()
  return <select>
