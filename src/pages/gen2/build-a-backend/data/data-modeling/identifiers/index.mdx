...
Identifiers are defined using the `.identifier()` method on a model definition. Usage of the `.identifier()` method is optional; when it's not present, the model will automatically have a field called `id` of type `ID` that is autofilled.
...
- Single-field identifier with a consumer-provided value (type: `id` or `string`)
- Composite identifier with a consumer-provided value (type: `id` or `string`)
