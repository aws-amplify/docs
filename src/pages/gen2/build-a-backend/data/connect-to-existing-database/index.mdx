...
// highlight-next-line
const modifiedRds = rdsSchema.setAuthorization((models) => [
  models.todo.authorization([a.allow.public()]),
  models.todo.fields.content.authorization([a.allow.private()]),
]);
...
// highlight-start
const modifiedRds = rdsSchema.relationships((models) => [
...
const combinedSchema = a.schema.combine([modifiedRds, ddbSchema])
...
const modifiedRds = rdsSchema.addMutation({
...
// Add custom mutations or queries that execute SQL statements
// highlight-start
const modifiedRds = rdsSchema.addQueries({
  searchTodoBasedOnGeolocation: a.query()
    .arguments({
      lat: a.float(),
      long: a.float(),
...
- Auto-enabled debug mode on sandbox 🔥
