...
```bash
npx amplify sandbox secret add MYSQL_CONNECTION_STRING
...
```bash
npx amplify generate schema-from-database --connection-string MYSQL_CONNECTION_STRING --out amplify/data/schema.rds.ts
```
