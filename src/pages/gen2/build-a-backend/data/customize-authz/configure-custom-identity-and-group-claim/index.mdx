...
To use custom claims specify `identityClaim` or `groupClaim` as appropriate. In this example, the `identityClaim` is specified and the record owner will check against this `user_id` claim. Similarly, if the `user_groups` claim contains a "Moderator" string then access will be granted.
