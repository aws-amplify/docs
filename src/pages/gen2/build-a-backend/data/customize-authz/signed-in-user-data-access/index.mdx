...
In your application, you can perform CRUD operations against the model using `client.models.<ModelName>` with the `iam` auth mode. Note in this instance the user must be authenticated for the client to sign the request using the Amplify-generated `AuthRole` IAM role.
