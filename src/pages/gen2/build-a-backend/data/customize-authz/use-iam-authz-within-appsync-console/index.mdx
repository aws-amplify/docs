export const meta = {
  ...
  description: "The AppSync console's IAM authorization only works with Amplify-generated IAM roles, so you need to explicitly allowlist IAM users or roles by name to grant them access. To allow external AWS resources or IAM roles access through CDK, add their names to the `adminRoles` property to grant them full read/write access based on their IAM policy rather than `@auth` rules."
};
...
export const data = defineData({
  schema,
  authorizationModes: {
    // Pass in the IAM Role names to grant full read/write access to the API for models via IAM authorization.
