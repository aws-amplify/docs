export const meta = {
  ...
  description: 'Learn how to query application data.'
};
...
A **custom selection set** allows consumers to specify on a per-call basis the fields the consumer wishes to retrieve, this is possible for all operations that return data (CRUDL + observeQuery). The desired fields are specified in a strongly-typed way (discoverable via intellisense) with a "dot notation".
...
When working with TypeScript you often need to specify the type of your data models for type generics. For example, when working with `useState` from React, you need to provide a type in TypeScript to ensure type-safety for the rest of your component code that uses the returned state. Use the `Schema["MODEL_NAME"]` pattern to get consumable TypeScript types for the shapes of the data model return values coming from the backend API.
