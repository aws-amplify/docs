...
Auth has several events that can trigger functions to perform custom signup/signin flows and other tasks. Here we're going to configure a "preSignUp" trigger, but configuring any trigger is the same process.

In our auth definition we can add the following.
...
Note: The `PreSignUpTriggerHandler` type comes from the [@types/aws-lambda](https://www.npmjs.com/package/@types/aws-lambda) NPM package. This package has types for all kinds of lambda event payloads that are useful for writing lambda functions.
...
Functions can be used in your data model to fetch data from sources not natively supported by the data experience or to compute values on the fly. To use a function in your data model, modify `amplify/data/resource.ts` with the following.
...
const schema = a.schema({
  ...
  // defines a query with an input of { content: string} and an output of the same shape
  echo: a
    .query()
    .arguments({ content: a.string() })
    .returns(a.ref('EchoResponse'))
    .authorization([a.allow.public()])
    .function('echo') // this tells `defineData` to use the "echo" function (defined below) when resolving the value of this entity
});
...
export const data = defineData({
  ...
  functions: {
    // each function that is used as a resolver must be included here
    echo: defineFunction({
      entry: './echo-resolver.ts'
    })
  }
});
```

Now define the resolver handler.

```
// amplify/data/echo-resolver.ts
...
This function will echo back the content that it received in the request. You can now run `npx amplify sandbox` and try out the query in the AppSync console.

For more information on custom data resolvers, see [Custom business logic](/gen2/build-a-backend/data/custom-business-logic/)
