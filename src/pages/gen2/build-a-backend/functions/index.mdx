...
export const myDemoFunction = defineFunction({
  environment: {
    ENV_VAR_NAME: 'myValueHere'
  }
});
```

Any environment variable specified here will be available to the function at runtime.
...
By default, functions will time out after 3 seconds. This can be configured to any number of seconds up to 15 minutes.
...
By default, functions have 128 MB of memory allocated to them. This can be configured up to 10240 MB. Note that this can increase the cost of executing the function. For more pricing information see [here](https://aws.amazon.com/lambda/pricing/).

```ts
// amplify/functions/my-demo-function/resource.ts
export const myDemoFunction = defineFunction({
  memoryMB: 512 // allocate 512 MB of memory to the function.
});
```

### `runtime`

Currently, only Node runtimes are supported by `defineFunction`. However, you can change the Node version that is used by the function. The default is the latest Node LTS version that is supported by AWS Lambda (currently Node 20).
