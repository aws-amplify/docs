...
<SubCommandHeading parentCommand="amplify-graphql-client-code">Options</SubCommandHeading>

The available parameters for `amplify generate graphql-client-code` are:
...
### Workflow 1 - Generate GraphQL client code using the Amplify App ID. 
For customers who know their AppSync API ID, they can pass in a `--app-id` and `--branch` parameters to the generate command. 

```bash
npx amplify generate graphql-client-code --app-id xxxxxx	--branch staging
```

### Workflow 2 - Generate GraphQL client code for a branch that is connected in the Amplify service
...
### Workflow 3 - Generate codegen for CDK app using a joint "AmplifyBackendStack" construct
...
export class HelloCdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    new Backend(this, "my-amplify-backend", { /* ... */ });
  }
}
```
...
```   
### Workflow 4 - Generate codegen in specfic language and format
