...
You can set up you backend deployments using the following steps:
...
3. Now update your custom-pipeline build settings to include the `curl` command to trigger a frontend build after the `pipeline-deploy` succeeds. Using the same Amazon CodeCatalyst example above, this looks like the following:
...
4. This should trigger a build in your Amplify app. Amplify CI will build first and generate the `amplifyconfiguration.json` for the branch and then build, deploy, and host the frontend.
