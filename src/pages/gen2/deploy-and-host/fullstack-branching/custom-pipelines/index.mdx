export const meta = {
  ...
  description: 'Learn about how to setup fullstack CI/CD in a custom pipeline.'
};
...
While building with Amplify CI/CD offers benefits such as zero-config setup, fullstack previews, centralized secrets management, Amplify Gen 2 makes it easy to integrate fullstack CI/CD into your custom pipelines (e.g. AWS CodePipeline, Amazon CodeCatalyst, GitHub Actions etc.).
...
```
5. Trigger a `git push` to your branch. Your build logs should show that there is a AWS CloudFormation deployment underway.
...
3. Now update your custom-pipeline build settings to include the `curl` command to trigger a frontend build after the `pipeline-deploy` succeeds. Using the same Amazon CodeCatalyst example above:
...
4. This should trigger a build in your Amplify app. Amplify CI will build first generate the `amplifyconfiguration.json` for the branch and then build, deploy, and host the frontend.
