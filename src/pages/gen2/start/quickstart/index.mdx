...
We've removed the to shorten the code below; however, you can chose to keep them for assistance building your app's connection to frontend resources.
...
Then, you'll see the `defineData` function, which has our schema and authorization configuration passed in as arguments. We have an `API_KEY` set up in order to enable public access. Let's change our `defaultAuthorizationMode` to `userPool` instead so that the default is to use user authentication.
