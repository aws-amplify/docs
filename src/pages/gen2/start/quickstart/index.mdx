...
3. In the Git provider screen, choose _Option 2: Start with an existing app_. Connect the repository you just deployed and pick a branch. Review all of your settings to ensure everything is set up correctly and choose _Save and deploy_ to deploy your web app.

<Callout warning>

Next.js 14 apps require a [minimum node version](https://nextjs.org/blog/next-14#other-changes) of 18.17 at build time.
...
If you have an existing Gen 2 application, you can modify the build image by navigating to _Hosting_ -> _Environment Variables_ and create a new environment variable `_CUSTOM_IMAGE` with the value `amplify:al2023`, then redeploy the build. 

</Callout>

5. Review all of your settings to ensure everything is set up correctly. Choose _Save and deploy_ to deploy your web app to the AWS global content delivery network (CDN).

<video autoPlay={true} muted={true} loop={true} width="100%">
  <source src="/images/gen2/getting-started/console1.mp4" />
</video>

6. That's it! Your app backend and frontend will now take ~5 minutes to build and deploy.
