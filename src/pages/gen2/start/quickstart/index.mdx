...
The `resource.ts` files are where you _define_  backend resources. If you open up the `/amplify/data/resource.ts` file in your text editor, you'll see some generated code already there:
...
Let's add an authorization rule so that signed in users can create new to dos.

```js
.authorization([a.allow.owner(), a.allow.private().to(['create'])]),
```
