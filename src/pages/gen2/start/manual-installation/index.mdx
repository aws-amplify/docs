...
First, if your frontend framework of choice hasn't already, create your project's `package.json` with `npm init -y`. Then, install the Amplify dependencies for building a backend:
...
You can also update an existing frontend app. To upgrade existing Amplify Code-first DX (Gen 2) projects, use your Node.js package manager (for example, `npm`) to update relevant backend packages:
