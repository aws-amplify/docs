...
The easiest way to get started with AWS Amplify is through [npm](https://npmjs.com) with [`create-amplify`](https://www.npmjs.com/package/create-amplify). However, it may be preferable to start from scratch, either with a brand new directory or an existing frontend app. This guide will walk you through how to initialize your project, install dependencies, and author your first backend.
...
Or, you can create a file local in the Amplify backend directory, `amplify/package.json`:
...
You can also update an existing frontend app. To upgrade existing Amplify (Gen 2) projects, use your Node.js package manager (for example, `npm`) to update relevant backend packages:
