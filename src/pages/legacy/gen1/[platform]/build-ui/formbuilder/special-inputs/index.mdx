...
If you have a [Connected Form](/legacy/gen1/[platform]/build-ui/formbuilder/data-binding/#connected-forms) bound to a data model with [relationships](/legacy/gen1/[platform]/tools/console/data/relationships/), Form Builder uses a special field to handle submission. The **Autocomplete** field will allow your form submitters to select one or more records from a related data model. These connected autocomplete fields have some special configuration settings.

Form builder now supports data binding to **GraphQL APIs without Conflict Resolution**. [Learn more about GraphQL and Conflict Resolution here](/legacy/gen1/[platform]/tools/console/data/data-model/#datastore-and-graphql).
...
1. [Log into your Studio application](/legacy/gen1/[platform]/tools/console/adminui/start/#logging-in-and-creating-an-app)
...
When you are satisfied with your form configuration and style, [your forms can be generated in your local project](/legacy/gen1/[platform]/build-ui/formbuilder/#render-react-form-in-your-app).
...
[**File Uploader**](https://ui.docs.amplify.aws/react/connected-components/storage/fileuploader) fields allow your forms to accept file uploads, which are stored in an Amazon S3 bucket connected to your Amplify app. After uploading, that file's S3 key is stored in your data model, allowing for systematic retrieval using the [Amplify JS library](/legacy/gen1/[platform]/build-a-backend/storage/download/).

<Video src="/images/console/formbuilder/fileuploader.mp4" description="Video - File Uploader" />

### Prerequisites

In order to use the File Uploader field, your Amplify app must meet some prerequisites:

- You must have an [Amplify app with Studio enabled](/legacy/gen1/[platform]/tools/console/adminui/start/)
- You must have Amplify Authentication, [either through Studio](/legacy/gen1/[platform]/tools/console/auth/) or [through the CLI](/legacy/gen1/[platform]/tools/cli/commands/#auth/)
- You must have added Amplify Storage to your app, either [through Studio](/legacy/gen1/[platform]/tools/console/storage/file-storage/) or [through the CLI](/legacy/gen1/[platform]/build-a-backend/storage/configure-storage/)

### How it works

The File Uploader input in Form Builder will allow the form submitter to select from files on their local device and upload them to an S3 bucket. File Uploader automatically connects to your S3 bucket added as part of Amplify Storage, and uses the same [File level access concepts](#file-level-access), defaulting to `private`.
...
If your File Uploader field is connected to a field in your data model, the S3 key will be written to your database via your GraphQL API. If the field is not connected to a data model, you can [handle the S3 key manually with the `onSubmit` hook](/legacy/gen1/[platform]/build-ui/formbuilder/lifecycle/#handle-form-data-submissions---onsubmit).

### File Uploader with connected forms

If a File Uploader field is bound to a data model as part of a [connected form](/legacy/gen1/[platform]/build-ui/formbuilder/data-binding/#connected-forms), a record is created or updated in the bound data model when the form is submitted.

When a File Uploader input field is added to a connected form, the S3 key of the uploaded file is written to a `String` field in the connected data model.

_Prerequisites:_

- You must have a GraphQL API, deployed either [through Studio](/legacy/gen1/[platform]/tools/console/data/data-model/) or [through the CLI](/legacy/gen1/[platform]/build-a-backend/graphqlapi/).
- You must have at least one field in your data model with a type of `String`
- You must have a connected form. This can be a [default form](/legacy/gen1/[platform]/build-ui/formbuilder/#getting-started), or a [custom connected form](/legacy/gen1/[platform]/build-ui/formbuilder/data-binding/#connected-forms)

_Add File Uploader to a connected form..._

1. [Log into your Studio app](/legacy/gen1/[platform]/tools/console/adminui/start/) and select the UI Library
2. Select a connected form and select **Configure** in the upper right-hand corner
3. Select the `string` field you would like to convert to a File Uploader field
4. In the **Display** section of the **Input settings** sidebar, select the **Type** dropdown and choose `StorageField`

![screenshot of a text field being converted to a file uploader field](/images/console/formbuilder/storage1.png)

<Callout info>

Only `String` type fields can be converted to a `StorageField` type. If the **Type** dropdown is missing `StorageField`, [check your data model](/legacy/gen1/[platform]/tools/console/data/data-model/)

</Callout>

### File Uploader with unconnected forms

_Add a File Uploader field to an unconnected form..._

1. [Log into your Studio app](/legacy/gen1/[platform]/tools/console/adminui/start/) and select the UI Library
2. Navigate to an unconnected form, or [create a new blank form](/legacy/gen1/[platform]/build-ui/formbuilder/#getting-started)
...
With all unconnected forms, the form submission must be configured manually. Use the `onSubmit` hook and the `fields` object to [handle submission in code](/legacy/gen1/[platform]/build-ui/formbuilder/lifecycle/#handle-form-data-submissions---onsubmit).

### Configuring File Uploader

File Uploader has several unique configurations that can be applied in the right-hand toolbar.

- **File level access**: determines which users can access the uploaded file. File Uploader will default to Private.
...
File Uploader uses the same [File level access concepts](/legacy/gen1/[platform]/build-a-backend/storage/configure-access/) as the Storage category, defaulting to `private`. If you select `private` or `protected` file level access, all files uploaded will have `{user_identity_id}` in the file path. This restricts Read and Write access, and ensures identical files uploaded by different users have unique paths.
