...
Connected Forms are bound to a model in your app's [data schema](/legacy/gen1/[platform]/tools/console/data/data-model/). Whenever a connected form is submitted, a record is automatically created or updated in the bound data model, with some or all of the form's input fields mapping to fields in the data model.

Connected forms automatically work with any Amplify GraphQL API , [with or without DataStore](/legacy/gen1/[platform]/tools/console/data/data-model/#datastore-and-graphql), and no `onSubmit` handling is required.

## Unconnected forms

Unconnected Forms are standalone React components that can be used in any React or Nextjs project, even without an AWS account. Upon submission, the input values for the form are [accessible via the `onSubmit` hook for handling](/legacy/gen1/[platform]/build-ui/formbuilder/lifecycle/#handle-form-data-submissions---onsubmit).
...
1. [Log into Amplify Studio](/legacy/gen1/[platform]/tools/console/adminui/start/) and select the **UI Library** from the left-hand navigation bar
...
**Create new data model**: Studio will use your form to [generate a brand-new data model in your schema](/legacy/gen1/[platform]/tools/console/data/data-model/)
