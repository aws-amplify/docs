...
To interface with your data while Conflict Resolution is enabled, you will likely be using [DataStore syntax](/legacy/gen1/[platform]/build-a-backend/more-features/datastore/manipulate-data/). However, if Conflict Resolution is disabled, any data interactions must be updated to use [GraphQL syntax](/legacy/gen1/[platform]/build-a-backend/graphqlapi/mutate-data/) instead.

**Inaccessible data**

When a GraphQL API is created with Conflict Resolution enabled, [several hidden fields](/legacy/gen1/[platform]/build-a-backend/more-features/datastore/how-it-works/#conflict-resolution) are automatically added to each of your data models - these fields are required for querying and mutating data, and if any records have these values set to `null`, those records will be inaccessible.

If Conflict Resolution is disabled, these fields will no longer be populated when records are created or modified. This will have no immediate impact on your app, so long as you are using [GraphQL syntax](/legacy/gen1/[platform]/build-a-backend/graphqlapi/mutate-data/) to interact with your data. However, if Conflict Resolution is later re-enabled, **any existing data in the API will be inaccessible until the required fields are set for all existing records**.
...
1. Navigate to **[Amplify Studio](/legacy/gen1/[platform]/tools/console/adminui/start/#log-into-the-amplify-console) > Data**
...
- Sorting by column in [Data Manager](/legacy/gen1/[platform]/tools/console/data/content-management/) is not supported
<InlineFilter filters={['javascript', 'react', 'nextjs']}>
- [Figma-to-Code Collections](/legacy/gen1/[platform]/build-ui/uibuilder/collections/) do not support Pagination, and have limited support for Search
- [Forms with relationships](/legacy/gen1/[platform]/build-ui/formbuilder/special-inputs/) have some limitations when searching Autocomplete fields
</InlineFilter>
...
- [`@default` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/#assign-default-values-for-fields)
- [`@index` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/#configure-a-secondary-index)
- [`@function` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#lambda-function-resolver)
- [`@http` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#http-resolver)
- [`@predictions` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/connect-machine-learning-services/)
- [`@primaryKey` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/#configure-a-primary-key)
- [`@searchable` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/search-and-result-aggregations/)
- [`@mapsTo` directive](/legacy/gen1/[platform]/build-a-backend/graphqlapi/schema-evolution/)
- [`@auth` `groupClaim` property](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#user-group-based-data-access)
- [`@auth` `groupsField` property](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#field-level-authorization-rules)
- [`@auth` custom `groupClaim`](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#configure-custom-identity-and-group-claims)
- [`@auth` custom `identityClaim`](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#configure-custom-identity-and-group-claims)
- [`@auth` field-level rules](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#field-level-authorization-rules)
- [OIDC and Function providers](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#using-oidc-authorization-provider)
- [OwnerField as Array type](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#multi-user-data-access)
- [`Query` type](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#create-a-custom-query-or-mutation)
- [`Mutation` type](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#create-a-custom-query-or-mutation)
- [`Subscription` type](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#create-a-custom-query-or-mutation)
...
1. [Pull your Amplify project](/legacy/gen1/[platform]/tools/console/adminui/extend-cli/#to-configure-the-amplify-cli-for-use-without-an-aws-account) to a local directory
