...
<MigrationAlert isLegacy url={'/legacy/gen1/[platform]/tools/cli/graphqlapi/'} />
...
Prior to version 2.0 of the CLI, `@auth` rules did not apply to subscriptions. Instead you were required to either turn them off or use [Custom Resolvers](/legacy/gen1/[platform]/tools/cli-legacy/overwrite-customize-resolvers/#custom-resolvers) to manually add authorization checks. In the latest versions `@auth` protections have been added to subscriptions, however this can introduce different behavior into existing applications: First, `owner` is now a required argument for Owner-based authorization, as shown below. Second, the selection set will set `null` on fields when mutations are invoked if per-field `@auth` is set on that field. [Read more here](#per-field-with-subscriptions). If you wish to keep the previous behavior set `level: public` on your model as defined below.
...
Passing in the current user can be done dynamically in your code by using [Auth.currentAuthenticatedUser()](/legacy/gen1/[platform]/build-a-backend/auth/manage-user-session/#retrieve-a-user-session) in JavaScript.

</InlineFilter>

<InlineFilter filters={["android"]}>

Passing in the current user can be done dynamically in your code by using [AWSMobileClient.getInstance().getUsername()](/legacy/gen1/[platform]/sdk/auth/working-with-api/#utility-properties) in Android.

</InlineFilter>

<InlineFilter filters={["swift"]}>

Passing in the current user can be done dynamically in your code by using [AWSMobileClient.default().username](/legacy/gen1/[platform]/sdk/auth/working-with-api/#utility-properties) in iOS.
