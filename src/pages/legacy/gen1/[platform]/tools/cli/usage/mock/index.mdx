...
It is highly recommended that you complete the **[Getting Started](/legacy/gen1/[platform]/start/getting-started/)** section of Amplify setup before using local mocking.
...
- [@auth](/legacy/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/)
- [@primaryKey, @index](/legacy/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/)
- [@hasOne, @hasMany, @belongsTo, @manyToMany](/legacy/gen1/[platform]/build-a-backend/graphqlapi/data-modeling/)
- [@function](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#lambda-function-resolver)
- [@searchable](/legacy/gen1/[platform]/build-a-backend/graphqlapi/search-and-result-aggregations/)

### Mocking the Lambda triggers on `@model` types

If you have DynamoDB Lambda triggers set up on `@model` types in your GraphQL schema, by following steps [listed here](/legacy/gen1/[platform]/tools/cli/usage/lambda-triggers/#as-a-part-of-the-graphql-api-types-with-model-annotation), then you can test those Lambda triggers locally via `amplify mock` or `amplify mock api`.
...
The environment variables that are listed below in [Function mock environment variables](/legacy/gen1/[platform]/tools/cli/usage/mock/#function-mock-environment-variables), are available for each invocation of the Lambda trigger.
...
If you use the `@searchable` directive on `@model` types in your GraphQL schema, then you can test the search GraphQL queries that are generated for you locally via `amplify mock` or `amplify mock api`. To learn more about the search queries, refer to [this guide](/legacy/gen1/[platform]/build-a-backend/graphqlapi/search-and-result-aggregations/).
...
A GraphQL Lambda resolver connected to your schema using [@function](/legacy/gen1/[platform]/build-a-backend/graphqlapi/custom-business-logic/#lambda-function-resolver) can be mocked using [`amplify mock api`](#api-mocking-setup). For example, if you have a function named **quoteOfTheDay** and a schema like:
