...
export const meta = {
  ...
  route: "/legacy/[platform]/build-a-backend/data/customize-authz",
  platforms: [
    'android',
    'angular',
    'flutter',
...
|--------------------------|--------------|----------------|
| [Public data access where users or devices are anonymous. Anyone with the AppSync API key is granted access.](/legacy/[platform]/build-a-backend/data/customize-authz/public-data-access) | `publicApiKey` | `apiKey` |
...
| [Per user data access. Access is restricted to the "owner" of a record. Leverages `amplify/auth/resource.ts` Cognito user pool by default.](/legacy/[platform]/build-a-backend/data/customize-authz/per-user-per-owner-data-access) | `owner`/`ownerDefinedIn`/`ownersDefinedIn` | `userPool` / `oidc` |
| [Any signed-in data access. Unlike owner-based access, **any** signed-in user has access.](/legacy/[platform]/build-a-backend/data/customize-authz/signed-in-user-data-access) | `authenticated` | `userPool` / `oidc` / `identityPool` |
| [Per user group data access. A specific or dynamically configured group of users has access.](/legacy/[platform]/build-a-backend/data/customize-authz/user-group-based-data-access) | `group`/`groupDefinedIn`/`groups`/`groupsDefinedIn` | `userPool` / `oidc` |
| [Define your own custom authorization rule within a serverless function.](/legacy/[platform]/build-a-backend/data/customize-authz/custom-data-access-patterns) | `custom` | `lambda` |
...
All Amplify Gen 2 projects enable IAM authorization for data access. This ensures that the Amplify console's [data manager](/legacy/[platform]/build-a-backend/data/manage-with-amplify-console/) will be able to access your API. It also allows you to authorize other administrative or machine-to-machine access using your own IAM policies. See the [AWS AppSync Developer Guide](https://docs.aws.amazon.com/appsync/latest/devguide/security_iam_service-with-iam.html) for details on how AWS AppSync works with IAM.
