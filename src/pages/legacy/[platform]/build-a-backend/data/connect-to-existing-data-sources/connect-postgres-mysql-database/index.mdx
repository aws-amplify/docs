...
When deploying your app to production, you need to [add the PEM-encoded public CA certificate as a secret](/legacy/[platform]/deploy-and-host/fullstack-branching/secrets-and-vars/#set-secrets). Make sure to add the certificate with the same secret name you used in the sandbox environment. For example, we used `CUSTOM_SSL_CERT` above. Make sure to preserve the newlines and the `------BEGIN CERTIFICATE------` and `------END CERTIFICATE------` delimiters in the value. Finally, make sure the size of the entire value does not exceed 4KB.
...
Use the `.setAuthorization()` modifier to set model-level and field-level authorization rules for the SQL-backed data models. Review [Customize your auth rules](/legacy/[platform]/build-a-backend/data/customize-authz) for detailed authorization rule options.
...
When deploying your app to production, you need to [add the database connection string as a secret](/legacy/[platform]/deploy-and-host/fullstack-branching/secrets-and-vars/#set-secrets). Make sure to add the appropriate database connection string with the same secret name you used in the sandbox environment. For example, we used `SQL_CONNECTION_STRING` above.
