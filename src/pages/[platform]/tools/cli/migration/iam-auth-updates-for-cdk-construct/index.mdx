...
**Use case 2: Provide access to the GraphQL API from other AWS roles, such as AWS Lambda functions' execution role, or to use the GraphQL query explorer from the AWS AppSync console.** In this case, despite these IAM roles having the required IAM policies to access the GraphQL API, you needed to also supply these IAM roles into the Amplify GraphQL API CDK construct's `allowListedAdminRoles` properties. This often led to circular dependency issues that are difficult to untangle.
