...
export const meta = {
  ...
  description: 'The "amplify override api" command generates a developer-configurable "overrides" TypeScript file that provides Amplify-generated AppSync resources as CDK constructs. For example, developers can configure API settings that are not directly available in the Amplify CLI workflow, such as X-Ray tracing.',
  platforms: [
                'android',
                'angular',
                'flutter',
...
export function override(resources: AmplifyApiGraphQlResourceStackTemplate) {
  ...
  */
  ...
  resources.models["Post"].modelDDBTable.globalSecondaryIndexes[0].provisionedThroughput = {
    readCapacityUnits: 5,
    writeCapacityUnits: 5
  }; 
}
