...
export const meta = {
  ...
  platforms: ['javascript', 'react-native', 'angular', 'nextjs', 'vue']
};
...
**Note:** `Amplify.configure()` takes in two parameters - `BackendResources` and `LibraryConfiguration`. This section focuses on defining the `BackendResources` object.
