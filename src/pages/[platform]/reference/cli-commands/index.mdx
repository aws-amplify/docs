...
- `--config-out-dir` (_string_) - A path to a directory where the outputs file is written. If not provided, defaults to the working directory of the current process.
- `--config-format` (_string_) - Format in which the outputs file is written (choices: `mjs`, `json`, `json-mobile`, `ts`, `dart`).
