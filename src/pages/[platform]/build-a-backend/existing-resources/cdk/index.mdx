...
1. [Section two](/[platform]/build-a-backend/existing-resources/cdk/#build-a-flutter-app-and-connect-to-the-graphql-api) shows how to build and integrate a Flutter app with the AppSync API. Follow this if you want a cross-platform frontend.
1. [Section three](/[platform]/build-a-backend/existing-resources/cdk/#build-a-react-app-and-connect-to-the-graphql-api) explains how to create and connect a React web app to the AppSync API. Follow this if you want to build a web-only frontend or want to add a web frontend to complement the mobile experience.

The recommended approach is to complete section one to build the backend, then optionally complete section two or three, or both, to connect frontends for Flutter or React, or both. Once you're finished experimenting with these demo apps, we recommend visiting the ["Clean up resources"](/[platform]/build-a-backend/existing-resources/cdk/#clean-up-resources) section to delete the backend resources to avoid incurring unexpected costs. 
...
* Flutter and its command-line tools [installed](https://docs.flutter.dev/get-started/install) and configured (if completing the second part of this guide).
...
```bash title="lib/cdk-backend-stack.ts"
...
Now that you have built the backend API with the CDK, you can connect a frontend. Refer to section two to connect a Flutter app or section three to connect a React app, or complete both to connect frontends in each framework.

## Build a Flutter app and connect to the GraphQL API

In this section, we will connect a Flutter web app to the GraphQL API we created with the CDK. First, we will initialize a Flutter project, define models matching our schema, and use Amplify to integrate CRUD operations. Then we will add UI pages to manage to-do items with queries and mutations. Finally, we will configure Amplify with our backend details and run the app to demonstrate full functionality with our existing API.
...
Congratulations on successfully creating a new Flutter app and linking it to a preexisting GraphQL API built with AWS CDK. The next optional section will guide you through doing the same for a React web app. Alternatively, you can skip ahead to the ["Clean up resources"](/[platform]/build-a-backend/existing-resources/cdk/#clean-up-resources) section.
...
npx @aws-amplify/cli codegen add --apiId <awsAppsyncApiId> --region <awsAppsyncRegion> 
