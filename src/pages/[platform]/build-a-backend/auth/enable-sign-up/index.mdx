...
The `autoSignIn` API will automatically sign-in a user when it was previously enabled by the `signUp` API and
after any of the following cases has completed:
...
Finally you need to define the `signUpVerificationMethod` to `link` inside the `Cognito` option of your `Auth` config.
