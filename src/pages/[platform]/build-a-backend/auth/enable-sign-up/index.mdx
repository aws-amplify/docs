...
Before you begin, you should have finished the [_Create your first authentication resource_](/[platform]/build-a-backend/auth/set-up-auth/#create-your-first-authentication-resource) section of the _Set up Amplify Auth_ guide. In that guide we showed how to configure the `amplify/auth/resource.ts` file to enable your users to sign up to your application using a username and password. We will be continuing with examples of how to further customize those sign-up, sign-in, and sign-out workflows in this guide.
...
For many apps, user sign-up and sign-in with a username and password is all that is required. Once authenticated, the app can talk to an API to access and mutate data. This is the method we illustrated in the [_Set up Amplify Auth_](/[platform]/build-a-backend/auth/set-up-auth/) guide.
