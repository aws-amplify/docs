...
1. Clients sign the requests and the backend validates the signature, allowing or denying actions depending on predefined policy. The predefined rules are known as [IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies.html) policies and automatically configured by the AWS Amplify.
...
For many apps, user sign-up and sign-in is all that is required. Once authenticated the app can talk to an API to access and mutate data. In this case, you can simply create a User Pool by adding an `auth` directory under `amplify` directory and use the default values. In your application you can use `signUp` and `signIn` (or an Amplify UI component) APIs to complete this process and retrieve tokens. The Amplify client will refresh the tokens calling `fetchAuthSession` if they are no longer valid.

![Image](/images/SimpleAuthZ.png)

## Social Provider Federation

Many apps also support login with social providers such as Facebook, Google Sign-In, or Login With Amazon. The preferred way to do this is via an OAuth redirect which lets users login using their social media account and a corresponding user is created in User Pools. With this design you do not need to include an SDK for the social provider in your app. Set this up by updating the `externalProviders` property in `auth` resources file. Add the client id and secret for the selected the social provider option. Upon deployment you can use `signInWithRedirect` in your application to either show a pre-built "Hosted UI" or pass in a provider name (e.g. `signInWithRedirect({provider: 'Facebook'})`) to interface directly and build out your own UI.
