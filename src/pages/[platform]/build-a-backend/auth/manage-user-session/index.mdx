...
The `fetchAuthSession` API will auto refresh the user's session when tokens have expired and there is a valid `refreshToken`. Additionally, you can also refresh the session by calling the `fetchAuthSession` API with the `forceRefresh` flag enabled. 
...
### Refresh session with social providers
...
**Note:** refresh session with social providers will not work when the user has authenticated with a `token` oauth flow, e.g `{ responseType: 'token' }`.
