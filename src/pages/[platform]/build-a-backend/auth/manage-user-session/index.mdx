...
The `fetchAuthSession` API will automatically refresh the user's session when tokens have expired and there is a valid `refreshToken`. Additionally, you can also refresh the session by calling the `fetchAuthSession` API with the `forceRefresh` flag enabled. 
