...
AWSCognitoAuthSignInOptions options =
    AWSCognitoAuthSignInOptions
        .builder()
        .authFlowType(AuthFlowType.USER_AUTH)
        .preferredFirstFactor(AuthFactorType.PASSWORD_SRP) // or AuthFactorType.PASSWORD
        .callingActivity(callingActivity)
        .build();
Amplify.Auth.signIn(
    "username",
    ...
    options,
    result -> {
        // result.getNextStep() === 'DONE'
        handleNextSignInStep(result);
    },
...
Amplify.Auth.signIn(
    ...
    options,
    result -> {
        // result.getNextStep() === 'CONFIRM_SIGN_IN_WITH_OTP'
        handleNextSignInStep(result);
    },
    error -> Log.e("AuthQuickstart", "Failed to sign in", error)
);
...
Amplify.Auth.signIn(
    ...
    options,
    { result ->
        // result.signInStep === 'CONFIRM_SIGN_IN_WITH_OTP'
        handleNextSignInStep(result)
    },
    { error ->
        Log.e("AuthQuickstart", "Failed to sign in", error)
...
When multiple first factor options are available and none are set as preferred in InitiateAuth API call (or preferred challenge is not available), Amplify Auth continues to a new sign in step `CONTINUE_SIGN_IN_WITH_FIRST_FACTOR_SELECTION`:
