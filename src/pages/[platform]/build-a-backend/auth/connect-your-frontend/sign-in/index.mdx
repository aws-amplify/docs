...
To initiate an available first factor, the desired first factor type is passed to confirm sign in as the challenge response. The next sections will cover how to handle each challenge during the passwordless authentication flow.
