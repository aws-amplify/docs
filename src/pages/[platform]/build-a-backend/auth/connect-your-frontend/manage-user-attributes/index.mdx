...
export const UpdateUserAttributesForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const userAttributes = Object.fromEntries(formData.entries());
...
import { UserAttributeKey } from "aws-amplify/auth";
import { useUserAttributes } from "@aws-amplify/ui-react-core";
import { View, Heading, Checkbox, Button, Loader, Text } from "@aws-amplify/ui-react";
    ...
    <View>
      ...
      <View as="form" onSubmit={handleDeleteSubmit}>
        {Object.entries(data.attributes ?? {}).map(([key, value]) => (
          <View key={key} flexDirection="row" alignItems="center">
            <Checkbox name={key} />
            <Text>{key}</Text>
...
```
As with updating, it is important to not that only attributes configured as 'mutable' may be deleted. Furthermore, 'required' attributes may not be deleted because they may not be undefined.

### Verifying Attributes
```ts
Sure, here's an updated version of the code using components from the Amplify UI library:

import { UserAttributeKey } from "aws-amplify/auth";
import { useUserAttributes } from "@aws-amplify/ui-react-core";
import { View, TextField, Button, Loader, Text } from "@aws-amplify/ui-react";
...
export const UpdateAndVerifyUserAttribute = () => {
  ...
  return (
    <View>
      ...
      {data.pendingVerification.name?.codeDeliveryDetails && (
        <View as="form" onSubmit={handleVerifySubmit}>
          <Text>
            Verification Code for{" "}
            {data.pendingVerification[0].codeDeliveryDetails.destination}:
          </Text>
          <TextField type="text" name="confirmationCode" />
          <Button type="submit" isLoading={isLoading}>
            Verify {data.pendingVerification.name}
          </Button>
        </View>
      )}
    </View>
...
export const SimpleProfilePage = () => {
  const [{ isLoading, data, hasError }, handleAttributes] = useUserAttributes();
  const [isEditMode, setIsEditMode] = useState(false);
  const formRef = useRef<HTMLFormElement>(null);

  const editableAttributes: UserAttributeKey[] = [
    // Array of mutable attributes (declared in amplify/auth/resource.ts)
  ];
  ...
  if (isLoading && data.attributes) {
    return <Loader />;
  }

  if (hasError) {
    return <Text>Oh no, an error occurred.</Text>;
  }
        ...
        !isEditMode ? (
          ...
          <Flex key={key} alignItems="center">
            <TextField
              label={key}
              name={key}
              defaultValue={data.attributes?.[key] || ""}
            />
            <Button
              isLoading={isLoading}
              type="button"
    ...
    <View>
      ...
      {data.pendingVerification.name?.codeDeliveryDetails && (
        <View>
          ...
          />
          <Button
            title={`Verify ${data.pendingVerification.name}`}
...
export const SimpleProfilePage = () => {
  ...
  const handleAttributeDelete = (key: UserAttributeKey) => {
    ...
    const input = formRef.current?.findNode(`input[name="${key}"]`);
    if (input) {
      input.clear();
    }
  };
  ...
  return (
    <View>
      ...
      <TouchableOpacity onPress={() => setIsEditMode((prev) => !prev)}>
        <Text>{isEditMode ? "Turn off Edit mode" : "Turn on Edit mode"}</Text>
      </TouchableOpacity>
