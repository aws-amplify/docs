...
```ts
import { getCurrentDevice } from 'aws-amplify/auth';

const output = await getCurrentDevice();
```
...
```dart
Future<void> getCurrentUserDevice() async {
  try {
    final device = await Amplify.Auth.getCurrentDevice();
    safePrint('Device: $device');
  } on AuthException catch (e) {
