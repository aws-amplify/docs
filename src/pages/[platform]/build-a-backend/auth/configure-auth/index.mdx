...
Once your User Pool is functioning, you can enable more configurations such as federation with Facebook, Google, or Login with Amazon. You can also configure more advanced settings by selecting _Manual Configuration_.
...
You can find [more documentation on adding each social provider in the Libraries section](/[platform]/build-a-backend/auth/add-social-provider/#setup-your-auth-provider) of the docs.
...
Run `amplify import auth` or read the [guide on how to import existing Cognito resources](/[platform]/tools/cli/auth/import/).

## Troubleshooting

```
Member must satisfy regular expression pattern: arn:[\w+=/,.@-]+:[\w+=/,.@-]+:([\w+=/,.@-]*)?:[0-9]+:[\w+=/,.@-]+(:[\w+=/,.@-]+)?(:[\w+=/,.@-]+)?
```

This error may occur when modifying the [`breakCircularDependency`](/[platform]/tools/cli/reference/feature-flags/#breakCircularDependency) flag and push a resource that depends on the auth category. To mitigate the issue run `amplify update auth`, walkthrough the auth flow keeping the previous settings and run `amplify push` or `amplify push --force`.
