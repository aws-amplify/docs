...
Amplify client libraries can be configured to obtain credentials from a third party identity provider. [Learn more about the custom token providers advanced workflow](/[platform]/build-a-backend/advanced-workflows/custom-token-provider/).
