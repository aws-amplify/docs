...
Shown in the snippet above, `String` and `Number` can be assigned minimum and maximum constraints. This is useful to defer validations to the underlying service, although does not extend to validating against specific constraints such as matching against a regular expression or a range of values.
