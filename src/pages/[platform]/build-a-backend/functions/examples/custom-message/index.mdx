...
You can use `defineAuth` and `defineFunction` to create a [Cognito custom message Lambda trigger](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-lambda-custom-message.html) that sends a custom email, phone verification message, or a multi-factor authentication (MFA) code.
