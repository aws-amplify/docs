...
To get started, create a new directory and a resource file, `amplify/functions/pre-sign-up/resource.ts`. Then, define the Function with `defineFunction`:

```ts title="amplify/functions/pre-sign-up/resource.ts"
...
Next, create the corresponding handler file, `amplify/functions/pre-sign-up/handler.ts`, file with the following contents:

```ts title="amplify/functions/pre-sign-up/handler.ts"
