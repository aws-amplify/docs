...
To get started, Update the `amplify/data/resource.ts` file to define a data model for the user's profile:
...
export const handler: PostConfirmationTriggerHandler = async (event) => {
  ...
  const { errors, data: newProfile } = await client.models.Profile.create({
