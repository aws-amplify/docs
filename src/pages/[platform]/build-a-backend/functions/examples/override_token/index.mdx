...
You can use `defineAuth` and `defineFunction` to create a [Cognito Pre token generation Lambda trigger](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-lambda-pre-token-generation.html) to override the token by adding a new claim or modifying the user's group membership.
...
To get started, create a new directory and a resource file, `amplify/auth/pre-token-generation/resource.ts`. Then, define the Function with `defineFunction`:
