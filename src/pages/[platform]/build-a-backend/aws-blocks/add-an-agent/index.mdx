...
export const api = new ApiNamespace(scope, 'api', (context) => ({
  ...
  async startConversation() {
    ...
    return agent.createConversationId(user.sub);
  },

  // Send a message — the agent streams chunks to a Realtime channel
...
The `Agent` Block scopes data by an unguessable `conversationId` but does not authorize the caller on read paths. Always derive the user from `requireAuth(context)` and confirm the conversation belongs to them — as `getMessages` does above — before returning history.
...
Make sure [model access is enabled](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html) for the default Bedrock model in your account and Region.
...
Generate the native client for your platform from your backend's `blocks.spec.json` and attach your Amplify Auth session token as a `Bearer` token on each request, as described in [Connect your frontend](/[platform]/build-a-backend/aws-blocks/connect-your-frontend/). Call `startConversation` to get a conversation ID, then `sendMessage`, and subscribe to the returned Realtime channel to receive streamed chunks.
