...
- In the Resources pane, choose the method you want to test. Pick `ANY` right under `/items`.
...
- In the Method Execution pane, in the Client box, choose TEST. Choose the `GET` method. Add `limit=10` to the Query String `{items}` field.
