...
curl https://a5b4c3d2e1.execute-api.eu-west-2.amazonaws.com/dev/items
```

### POST method example

```bash title="Terminal" showLineNumbers={false} 
curl -H "Content-Type: application/json" -d '{"name":"item-1"}' https://a5b4c3d2e1.execute-api.eu-west-2.amazonaws.com/dev/items
```
...
**Warning:** Testing methods using production endpoints may result in changes to resources that cannot be undone.
...
**Warning:** Testing methods with the API Gateway console may result in changes to resources that cannot be undone.
