...
If you want to configure a public REST API, you can set an API key in Amazon API Gateway. Then, update your `amplify_outputs.json` to reference it. The value associated to the `"authorizationType"` key should be `"API_KEY"`. Also include a `"apiKey"` as a key, and set its value to whatever you configured in API Gateway.
...
If you set up the API Gateway with a custom authorization with a Cognito User pool, then you can set the `amplify_outputs.json` to use `AMAZON_COGNITO_USER_POOLS `.
...
Your `amplify_outputs.json` should contain Cognito configuration in the `awsCognitoAuthPlugin` block, including details about your Cognito user pool:
...
If you are using a 3rd party OIDC provider you will need to configure it and manage the details of token refreshes yourself. Update the `amplify_outputs.json` file and code snippet as follows:
