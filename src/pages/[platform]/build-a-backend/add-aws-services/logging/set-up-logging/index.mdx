...
First create the file `amplify/custom/RemoteLoggingConstraintsConstruct.resource.ts` with the following:

```ts title="RemoteLoggingConstraintsConstruct.resource.ts"
...
export class RemoteLoggingConstraintsConstruct extends Construct {
  constructor(scope: Construct, id: string, authRoleName: string, unAuthRoleName: string, logGroupName: string) {
    super(scope, id)
