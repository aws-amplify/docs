...
const ITables = Object.values(backend.data.resources.tables);

const vault = new BackupVault(backupStack, "backup-vault", {
  backupVaultName: "backup-vault",
});

const plan = new BackupPlan(backupStack, "backup-plan", {
...
plan.addSelection("plan-selection", {
  resources: ITables.map((table) => BackupResource.fromDynamoDbTable(table)),
  allowRestores: true,
});
```
