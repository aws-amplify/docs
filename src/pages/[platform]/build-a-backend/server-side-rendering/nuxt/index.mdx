...
const gqlServerClient = generateServerClient({ config: amplifyConfig });
...
export default defineNuxtPlugin({
  ...
  setup() {
    ...
    // tokens, the new tokens would be sent back to the client side via `SetCookie`
    ...
    return {
      provide: {
        ...
        // your Nuxt app here. You will have to use the APIs exported from the
