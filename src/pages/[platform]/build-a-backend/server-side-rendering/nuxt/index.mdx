...
## Start Using Amplify Gen2 in Your Nuxt 3 project

You can install Amplify Gen2 following this guide, [Manual installation](/[platform]/start/manual-installation/).
...
```ts title="plugins/01.amplify-apis.client.ts"
...
```ts title="plugins/01.amplify-apis.server.ts"
...
```ts title="plugins/02.auth-redirect.ts"
import { Amplify } from 'aws-amplify';
import config from '~/amplify_outputs.json';
