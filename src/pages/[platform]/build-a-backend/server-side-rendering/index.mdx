...
2. The library options that contain the `tokenProvider` and `credentialsProvider`
3. A callback function that contains your business logic to call Amplify categories APIs

#### Amplify Configuration Object

You can get the configuration object by passing the config object imported from the `amplifyconfiguration.json` file to the utility function `parseAmplifyConfig` (exported from `aws-amplify/utils`), or you can crate the object literal that conforms to the type `ResourceConfig` (exported from `aws-amplify`).

#### Library Options with the `tokenProvider` and `credentialsProvider`

We have mentioned above that an isolated server context may contain a user session. Amplify categories APIs will rely on the `tokenProvider` and `credentialsProvider` to function. Therefore, you will need to create and configure the providers using cookies, following the below steps.
...
When the callback function completes, the corresponding server context will be destroyed.
...
// Execute Amplify category APIs
// For example, calling `fetchAuthSession` on the server
