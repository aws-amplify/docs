...
export const meta = {
  ...
  platforms: ["javascript", "nextjs", "vue", "angular", "react", "react-native"]
};
...
- [Installed and configured Amplify libraries for Next.js](/[platform]/build-a-backend/server-side-rendering/)
...
Using an API Key for authenticating your requests, you can query the GraphQL API to get a list of all `Todo`s. To paginate over the list queries, you need to pass in a `limit` and `nextToken` on the `listTodos` query. See more at [GraphQL pagination ](/guides/api-graphql/graphql-pagination).
