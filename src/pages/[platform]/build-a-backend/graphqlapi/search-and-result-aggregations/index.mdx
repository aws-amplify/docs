...
## Aggregate values for search result (minimum, maximum, average, sum, terms, cardinality)
...
<Block name="Cardinality (cardinality)">

Provide the `cardinality` value as the `aggregate` `type` and specify the `aggregateItems` in the response field.

```graphql
query MyQuery {
  searchStudents(
    aggregates: {
      type: cardinality # Specifies that you want the "cardinality" value
      field: examsCompleted # Specifies the field for the aggregate value
      name: "cardinalityExams" # provides a name to reference in the response field
    }
    filter: { name: { wildcard: "Rene*" } }
  ) {
    aggregateItems {
...
In the example above, the response includes the count value of unique "examsCompleted" for all Students whose name starts with "Rene".
