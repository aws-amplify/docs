...
WHERE INFORMATION_SCHEMA.COLUMNS.table_schema = 'public' AND INFORMATION_SCHEMA.COLUMNS.TABLE_CATALOG = '<database-name>';
```
</Block>
</BlockSwitcher>

Generate an Amplify GraphQL API schema by running the following command, replacing the `--engine-type` value with your database engine of `mysql` or `postgres`, and the `--sql-schema` value with the path to the CSV file created in the previous step:

```sh
npx @aws-amplify/cli@sql-release api generate-schema --engine-type mysql --sql-schema schema.csv --out schema.graphql
```
