...
- An GraphQL API already set up either via the [Amplify CLI](/[platform]/build-a-backend/graphqlapi/set-up-data/), or directly via the underlying [AWS AppSync service](https://docs.aws.amazon.com/appsync/latest/devguide/designing-a-graphql-api.html).
- A frontend application set up. For instructions on setting up an app, see [Project Setup: Create your application](/[platform]/build-a-backend/storage/configure-access/)
