...
We also have the ability to target a specific bucket by passing the bucket option to the `uploadData` request. The bucket option can either be a string representing the target bucket's assigned name or an object specifying the bucket name and region as strings. 
...
Option | Type | Description |
| -- | -- | ----------- |
| bucket | String \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's assigned name or an object specifying the bucket name and region as strings.<br/><br/> Read more at [Configure additional storage buckets](/[platform]/build-a-backend/storage/set-up-storage/#configure-additional-storage-buckets) |
| contentType | String | The default content-type header value of the file when downloading it. <br/><br/> Read more at [Content-Type documentation](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type) |
