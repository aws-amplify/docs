...
We have the ability to target a specific bucket by passing in the bucket option to the `uploadData` request. The bucket option can either be a string representing the target bucket's assigned alias or an object specifying the bucket name and region as strings. See [Multi-Bucket Support](/[platform]/build-a-backend/storage/upload-files/#transfer-acceleration) for more information.
...
Option | Type | Description |
| -- | -- | ----------- |
| bucket | String \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's customer assigned alias or an object specifying the bucket name and region as strings.<br/><br/> Read more at [Multi-Bucket Support](/[platform]/build-a-backend/storage/multi-bucket) |
