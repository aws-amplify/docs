...
We also have the ability to target a specific bucket by passing the bucket option to the `uploadData` request. The bucket option can either be A string representing the target bucket's friendly name provided in Amplify backend or an object specifying the actual bucket name and region from the console. 
