...
We also have the ability to target a specific bucket by passing the bucket option to the `uploadData` request. The bucket option can either be A string representing the target bucket's assigned name in Amplify Backend or an object specifying the bucket name and region from the console.. 
