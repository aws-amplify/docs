...
If the object is located in a different bucket than the one assigned `isDefault` in your backend it can be removed by specifying the bucket with it's user defined alias or an object with `bucketName` and `region` included as strings.
...
| -- | -- | ----------- |
| bucket | String \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's customer assigned alias or an object specifying the bucket name and region as strings. |
</InlineFilter>
