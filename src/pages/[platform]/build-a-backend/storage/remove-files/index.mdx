...
function handleDeleteFolder() {
  ...
  deleteOperation = remove({ 
    ...
    options: {
      onProgress: (progress) => {
        updateProgressBar(progress.deleted?.length || 0);
      }
    }
  });
...
| onProgress | (progress: \{<br/>deleted?: \{path: string\}[];<br/>failed?: \{path: string; code: string; message: string\}[];<br/>\}) => void | Optional | Callback function for tracking folder deletion progress. Called after each batch of files is processed during folder deletion operations. |
