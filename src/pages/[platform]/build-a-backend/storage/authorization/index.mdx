...
With the following `access` definition, you can configure permissions such that auditors have readonly permissions to `media/*` while admin has full permissions.
...
export const storage = defineStorage({
  ...
  access: (allow) => ({
    'media/*': [
      allow.groups(['friends', 'admin']).to(['read', 'write'])
    ]
  })
});
```
...
A user with identity id `user123` would be able to perform read/write/delete operations on files within `media/user123/*` and would not be able to perform actions on files with any other prefix.
...
There are some rules on the types of prefixes that can be specified in the storage access definition.
