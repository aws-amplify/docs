...
export const meta = {
  ...
  description:
    'Define Authorization rules granularly for paths in the Storage bucket',
  platforms: [
    'angular',
    'javascript',
    'nextjs',
...
To grant all guest (i.e. not signed in) users of your application read access to files that start with `folder/*`, use the following `access` values.
...
To grant all authenticated (i.e. signed in) users of your application read access to files that start with `folder/*`, use the following `access` configuration.
...
With the following `access` definition, you can configure permissions such that auditors have readonly permissions to `folder/*` while admin has full permissions.
...
export const storage = defineStorage({
  name: 'myProjectFiles',
  access: (allow) => ({
    'folder/*': [allow.groups(['admin', 'auditor']).to(['read', 'write'])]
  })
});
```
...
A user with identity id "documents" would be able to perform read/write/delete operations on files within `folder/documents/*` and would not be able to perform actions on files with any other prefix.

Likewise, a user with identity ID "downloads" would be able to perform read/write/delete operation on files only within `folder/downloads/*`. In this way, each user can be granted access to a "private storage location" that is not accessible to any other user.
...
export const storage = defineStorage({
  name: 'myProjectFiles',
  access: (allow) => ({
    'folder/*': [allow.resource(demoFunction).to(['read', 'write', 'delete'])]
  })
});
```

This would grant the function `demoFunction` the ability to read write and delete files within `folder/*`.
...
2. Only one level of nesting is allowed. For example, you can define access controls on `folder/*` and `folder/documents/*` but not on `folder/documents/photos/*` because that path has 2 other prefixes.
3. Wildcards cannot conflict with the `{entity_id}` token. For example, you cannot have both `folder/*` and `folder/{entity_id}/*` defined because the wildcard in the first path conflicts with the `{entity_id}` token in the second path.
4. A path cannot be a prefix of another path with an `{entity_id}` token. For example `folder/*` and `folder/documents/{entity_id}/*` is not allowed.
...
export const storage = defineStorage({
  name: 'myProjectFiles',
  access: (allow) => ({
    'folder/*': [allow.authenticated.to(['read', 'write', 'delete'])],
    'folder/documents/*': [allow.guest.to(['read'])],
    'folder/downloads/*': [allow.authenticated.to(['read'])],
    'other/*': [
      allow.guest.to(['read']),
...
|  | folder/\* | folder/documents/\* | folder/downloads/\* | other/\* |
| --- | --- | --- | --- | --- |
| **Authenticated Users** | read, write, delete | NONE | read | read, write |
| **Guest users** | NONE | read | NONE | read |

Authenticated users have access to read, write, and delete everything under `folder/*` EXCEPT `folder/documents/*` and `folder/downloads/*`. For those subpaths, the scoped down access overrides the access granted on the parent `folder/*`
