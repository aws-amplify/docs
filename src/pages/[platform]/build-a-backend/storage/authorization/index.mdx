...
By default, all prefixes are denied to all types of users unless explicitly granted within `defineStorage` using the `access` callback as shown below.
...
The `entity_id` is a reserved token that will be replaced with the users' identifier when the file is being uploaded. You can specify the method of identification when defining access to the prefix like `allow.entity(<identification_method>).to([..])`. 
