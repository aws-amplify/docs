...
The process for cancelling a request has changed in v6. In v5, you have to maintain the promise reference returned from a Storage API call, and supply it as input to the `Storage.cancel` API. In v6, `cancel` is a function returned with the result of a Storage operation. To cancel an operation, you will call `operation.cancel()`.
