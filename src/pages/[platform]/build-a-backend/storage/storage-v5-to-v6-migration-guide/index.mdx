...
The `Storage.get` API has been separated into two API's in v6, `getURL` and `downloadData`. There are a few changes in the experience from v5 to v6 in addition to [those mentioned above](/[platform]/build-a-backend/storage/storage-v5-to-v6-migration-guide/#deprecated-options):
...
- The `path` parameter is now optional. If you want to list all files in the public folder, you do not need to provide an empty string as the `path` argument.
