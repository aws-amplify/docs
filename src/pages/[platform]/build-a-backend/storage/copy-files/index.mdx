...
## Copy across buckets / regions

Alternatively, specify a non-default bucket target by including `bucket` in both the source and destination objects. 
  ...
  try {
    const response = await copy({
      source: {
        ...
        // Alternatively, bucket: { bucketName: 'generated-primary-bucket-name', region: 'us-east-1' }
      },
      destination: {
        path: 'shared/2024/1.jpg',
        // Alternatively, path: ({identityId}) => `shared/{identityId}/1.jpg`
        bucket: {
         ...
         // Alternatively, bucket: 'secondaryStorage'
      },
    });
  } catch (error) {
    console.error('Error', err);
