  ...
  try {
    const response = await copy({
      source: {
        path: 'album/2024/1.jpg',
        // Alternatively, path: ({identityId}) => `album/{identityId}/1.jpg`
        bucket?: "userDefinedFriendlyBucketName" // Specify a target bucket using name assigned in Amplify Backend
      ...
      destination: {
        path: 'shared/2024/1.jpg',
        // Alternatively, path: ({identityId}) => `shared/{identityId}/1.jpg`
        // Specify a target using bucket name from console and associated region
        bucket: {
          bucketName: "generated-secondary-bucket-name",
          region: "us-east-2"
        }
         // Alternatively, use name assigned in Amplify Backend
...
| --- | --- | --- |
| path | string \| <br/>(\{ identityId \}) => string | A string or string returning function that represents the path in source and destination bucket to copy the object to or from |
