...
<Block name="With Key (Deprecated <= v6.x.x)">
...
try {
  const downloadResult = await downloadData({ path: "/album/2024/1.jpg" }).result;
...
try {
  const downloadResult = await downloadData({ key: "abc.txt" }).result;
  const text = await downloadResult.body.text();
  // Alternatively, you can use `downloadResult.body.blob()`
  // or `downloadResult.body.json()` get read body in Blob or JSON format.
  console.log('Succeed: ', text);
...
const downloadTask = downloadData({ path: "/album/2024/1.jpg", data: file });
downloadTask.cancel();
try {
...
```javascript
import { downloadData, isCancelError } from 'aws-amplify/storage';
const downloadTask = downloadData({ key: "abc.txt", data: file });
downloadTask.cancel();
try {
  await downloadTask.result;
} catch (error) {
...
<Callout warning>
Note: The `key` and `accessLevel` parameters are deprecated and may be removed in the next major version.
</Callout>
