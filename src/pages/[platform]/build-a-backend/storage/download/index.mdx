...
<BlockSwitcher>
<Block name="Path (Version 6.0.18)">
```javascript
...
const downloadTask = downloadData({ path: filename, data: file });
downloadTask.cancel();
try {
...
```javascript
import { downloadData, isCancelError } from 'aws-amplify/storage';
const downloadTask = downloadData({ key: filename, data: file });
downloadTask.cancel();
try {
  await downloadTask.result;
} catch (error) {
