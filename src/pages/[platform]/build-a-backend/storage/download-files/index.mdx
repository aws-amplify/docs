...
const linkToStorageFile = await getUrl({
  ...
  options: {
    ...
    bucket?: "userDefinedFriendlyBucketName" // Specify a target bucket using user defined alias or full bucket name and region
...
| -- | -- | ----------- |
| bucket | String \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's user defined alias or an object specifying the bucket name and region as strings. | [Multi-Bucket Support](/[platform]/build-a-backend/storage/multi-bucket) |
...
We have the ability to target a specific bucket by passing in the bucket option to the `downloadData` request. The bucket option can either be a string representing the target bucket's user defined alias or an object specifying the bucket name and region as strings. See [Multi-Bucket Support](/[platform]/build-a-backend/storage/upload-files/#transfer-acceleration) for more information.
