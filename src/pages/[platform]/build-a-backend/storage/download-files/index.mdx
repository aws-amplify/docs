...
{/* should this just be for js, next, react? */}
...
// or <StorageImage alt="cat" path={() => "guest/cat.jpg"} />
};
```

</InlineFilter>
{/* should this be in the inline filter? */}
...
{/* what's a presigned URL? Why do we need an expiration? */}
Get a presigned URL of a stored file and expiration of URL

<InlineFilter filters={["react", "angular", "javascript", "vue", "nextjs", "react-native"]}>

{/* should we rename getUrlResult to storageFile or something?*/}
```typescript
...
{/* use same variable name as above */}
```tsx
<a href={getUrlResult.url.toString()} target="_blank" rel="noreferrer">
  {fileName} 
</a>
```
...
{/* Is this needed? we don't have this elsewhere in our docs */}

Users can run into unexpected issues, we are giving you advance notice in documentation with links to open issues - please vote for what you need to help the team prioritize.
