...
const linkToStorageFile = await getUrl({
  ...
  options: {
    ...
    bucket?: "userDefinedFriendlyBucketName" // Specify a target bucket using assigned name or full bucket name and region
    // Alternatively, provide actual bucket name from console and associated region
    // bucket?: { bucketName: "generated-bucket-name", region: "us-east-2" }
  },
});
...
| -- | -- | ----------- |
| bucket | string \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's friendly name provided in Amplify backend or an object specifying the actual bucket name and region from the console. | [Configure additional storage buckets](/[platform]/build-a-backend/storage/set-up-storage/#configure-additional-storage-buckets) |
...
We also have the ability to target a specific bucket by passing in the bucket option to the `downloadData` request. The bucket option can either be a string representing the target bucket's assigned name or an object specifying the bucket name and region as strings. 
...
| -- | -- | ----------- | -- |
| bucket | String \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's assigned name or an object specifying the bucket name and region as strings. | [Configure additional storage buckets](/[platform]/build-a-backend/storage/set-up-storage/#configure-additional-storage-buckets) |
