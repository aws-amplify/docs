...
We also have the ability to target a specific bucket by passing in the bucket option to the `list` request. The bucket option can either be a string representing the target bucket's assigned name or an object specifying the bucket name and region as strings. 
