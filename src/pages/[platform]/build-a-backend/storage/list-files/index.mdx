...
We also have the ability to target a specific bucket by passing in the bucket option to the `list` request. The bucket option can either be A string representing the target bucket's friendly name provided in Amplify backend or an object specifying the actual bucket name and region from the console. 

See [Configure additional storage buckets](/[platform]/build-a-backend/storage/upload-files/#transfer-acceleration) for more information.
...
const result = await list({
  path: 'photos/',
  options: {
    bucket: "userDefinedFriendlyName",
    // Alternatively, { bucketName: "generated-bucket-name", region: "us-east-2" }
  }
});
```
...
| --- | --- | --- |
| bucket | String \| <br />\{ bucketName: string;<br/> region: string; \} | A string representing the target bucket's friendly name provided in Amplify backend or an object specifying the actual bucket name and region from the console. |
...
| subpathStrategy | \{ strategy: 'include' \| 'exclude',<br />delimiter?: string \} | An object representing the subpath inclusion strategy and the delimiter used to group results for exclusion. <br/><br/> Read more at [Excluding subpaths](/[platform]/build-a-backend/storage/list-files/#excluding-subpaths) |
