...
When objects are organized using a prefix you can return only those objects within the `path` you've provided, and not those within nested subpaths.

For example, given the following keys in your `path` you may want to return only the jpg objects, and not the "vacation" subpath:
