...
{/* Let's add a description of what Storage is and why you would want to use it in your app. Not sure if this is how we want to message it  */}

With Amplify Storage, you can enable file storage and management for user's content in your app. Folders within your storage instance can have different access rules. Amplify Storage is powered by Amazon Simple Storage Service (Amazon S3) buckets.
...
{/* are there any rules they need to follow for the storage name? I would guess the same rules as an S3 bucket or is this a friendly name? I personally wouldn't call a bucket for a work project "myProjectFiles" how about amplifyTeamDrive? Either way we should explain what the name is */}
...
Now when you run `npx amplify sandbox` or deploy your app with Amplify, it will configure an Amazon S3 bucket where your files will be stored. Before files can be accessed in your application, you must configure storage access rules.
...
{/* is there a reason the examples change between the two? This is using media/photos then the next is authenticated_access_only. I'm not sure this example is necessary */}
```ts title="amplify/storage/resource.ts"
export const storage = defineStorage({
  name: 'myProjectFiles',
  access: (allow) => ({
...
The access callback returns an object where each key in the object is a file prefix and each value in the object is an array of access rules that apply to that prefix. 

Refer to the following example to construct access rules for your storage bucket. [Learn more about customizing access to file prefixes](/[platform]/build-a-backend/storage/authorization/).

{/* what is an entity here? I would add an explanation or comment*/}
...
{/* I'm confused by this example - is this needed? why are they copying and pasting multiple examples? If the goal is to concepually tie to Gen 1 maybe we can do a separate "Storage in Gen 1 vs. Gen 2" guide or something - I personally feel this is confusing. */}
...
{/* We're inconsistent on pre-reqs across categories - JS doesn't have them but others do, is this intentional?  */}
...
{/* I think we should add how to publish (Git push) - does connecting with the client APIs affect whether they can use storage manager? */}
...
{/* we didn't add file uploads or downloads */}
