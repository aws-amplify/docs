...
```
After running `npx ampx sandbox` or deploying your app on Amplify, the generated `amplify_outputs.json` file will contain additional Amplify Storage metadata necessary for the client libraries to interact with your backend storage resources. 
...
```
Additional storage buckets can easily be referenced from your application code in all Amplify Storage APIs by passing the bucket option.
```ts
type StorageBucket = string | { bucketName: string; region: string }
```
The `bucket` option can either be a string representing the target bucket's user defined name or an object specifying the generated bucket name and region as strings.
...
### Manually overriding `amplify_outputs.json`

While using the Amplify Backend CLI is the easiest way to get started, existing AWS S3 resources can also easily be used with Amplify Storage. In order to configure the client side library without the CLI, you must manually create an `amplify_outputs.json` file and identify your storage resources using the template below. This client configuration file allows the Amplify Storage library to seamlessly connect to your backend resources.
