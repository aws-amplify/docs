...
To take advantage of this feature, start by creating additional storage resources in your backend using the `defineStorage` function and providing a `name` for each storage bucket. When creating new storage resources, a unique `name` attribute is required and it is recommended to assign a developer friendly value for each storage bucket to allow straightforward referencing from application code.
...
Additional storage buckets can be referenced from application code by passing the `bucket` option to Amplify Storage APIs. This can either be the target bucket's name assigned in Amplify Backend or the actual bucket name and region from the console.
...
```
Alternatively, existing storage resources can be used by creating or modifying the `amplify_outputs.json` file directly. Read more about [amplify_outputs.json](/[platform]/reference/amplify_outputs/).
