...
Amplify Storage allows you to optionally configure your backend to automatically create and manage multiple storage buckets. 

Start by creating additional storage resources in your backend. The `name` attribute is required and we recommend using a developer friendly value for each storage bucket to allow you to easily reference them from your application code later. You can define any custom access controls here as well.

Note that if you define additional storage buckets one of them must be marked with `isDefault: true`.

```ts title="amplify/storage/resource.ts"
export const primaryStorage = defineStorage({
  name: 'primaryStorage',
...
Add your additional storage resources to your backend definition.

```ts title="amplify/backend.ts"
import { defineBackend } from '@aws-amplify/backend';
import { auth } from './auth/resource';
import { primaryStorage, secondaryStorage } from './storage/resource';
...
Additional storage buckets can easily be referenced from your application code in all Amplify Storage APIs by passing the bucket option. The `bucket` option can either be a string representing the target bucket's user defined name or an object specifying the generated bucket name and region as strings.

See each Amplify Storage API page for additional examples.
...
try {
  const result = downloadData({
    ...
    options: {
      bucket: "secondaryStorage"
      // or { bucketName: "generated-secondary-bucket-name", region: "us-east-2" }
    }
  }).result;
} catch (error) {
...
### Use existing storage resources without Amplify Backend

While using the Amplify Backend is the easiest way to get started, existing storage resources can also easily be used with Amplify Storage. Pass your existing storage resource metadata to `Amplify.configure` to integrate Amplify Storage with your existing storage resource metadata.
...
Amplify.configure({
	storage: {
		bucket_name: "<your-primary-bucket-name>",
		aws_region: "<your-primary-bucket-region>",
    	// your primary bucket should be duplicated here
		buckets: [
			{
				name: "<your-primary-bucket-friendly-name>",
				bucket_name: "<your-primary-bucket-name>",
				aws_region: "<your-primary-bucket-region>",
			},
			{
...
```
Another option for integrating with existing storage resources is to create or modify the `amplify_outputs.json` file directly. Read more about [amplify_outputs.json](/[platform]/reference/amplify_outputs/).
...
/**
 * @see https://docs.amplify.aws/react/build-a-backend/ to add auth, functions, and more
 */
const backend = defineBackend({})

const storageStack = backend.createStack("ExistingStorage");

const bucket = Bucket.fromBucketAttributes(this, 'ImportedBucket', { 
  bucketArn: '<your-bucket-arn>'
})

// example showing how to add to outputs using cdk and amplify backend
// modify roles / bucket permissions
