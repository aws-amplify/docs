...
Amplify Storage allows you to optionally configure your backend to automatically create and manage multiple storage resources. 

Start by creating additional storage resources in your backend using the `defineStorage` function and providing a `name` for your storage bucket. The `name` attribute is required and we recommend assigning a developer friendly value for each storage bucket to allow you to easily reference them from your application code later. You can define any custom access controls here as well.
...
Additional storage buckets can easily be referenced from your application code in Amplify Storage by passing the bucket option. The `bucket` option can either be a string representing the target bucket's assigned name in Amplify Backend or an object specifying the bucket name and region from the console as strings.

See each Amplify Storage API page for additional usage examples.
...
try {
  const result = downloadData({
    ...
    options: {
    bucket?: "secondaryStorage" // Specify a target bucket using name assigned in Amplify Backend
    // Alternatively, provide bucket name from console and associated region
    // bucket?: { bucketName: "bucket-name-from-console", region: "us-east-2" }
    }
  }).result;
} catch (error) {
