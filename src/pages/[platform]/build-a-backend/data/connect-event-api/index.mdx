...
```kotlin
val webSocketClient: EventsWebSocketClient // Your configured EveEventsWebSocketClientntsRestClient
// List of kotlinx.serialization.json.[JsonElement, JsonPrimitive, JsonArray, JsonObject]
val jsonEvents = listOf(
  JsonObject(mapOf("some" to JsonPrimitive("data1"))),
  JsonObject(mapOf("some" to JsonPrimitive("data2")))
...
When the Flow's Corutine Scope is cancelled, the library will unsubscribe from the channel.
...
coroutineScope.launch {
  val subscription: Flow<EventsMessage> = webSocketClient.subscribe("default/channel").onCompletion {
    ...
    // Subsciption encountered an error and has been unsubscribed
...
// set flushEvents to false to immediately disconnect, discarding any pending posts to the WebSocet
