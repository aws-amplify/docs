...
final teamRequest = ModelQueries.get<Team>(Team.classType, TeamModelIdentifier(id: "YOUR_TEAM_ID"));
final teamResult = await Amplify.API.query(request: request).response;
...
final firstRequest = ModelQueries.list<Member>(Member.classType, limit: limit, where: Member.TEAM.eq(team));
...
final cartRequest = ModelQueries.get<Cart>(Cart.classType, CartModelIdentifier(id: "MY_CART_ID"));
final cartResult = await Amplify.API.query(request: request).response;
final cart = cartResult.data!;

// Do the customer call to with the id from cart
final customerRequest = ModelQueries.get<Customer>(Customer.classType, CustomerModelIdentifier(id: cart.customerId));
final customerResult = await Amplify.API.query(request: customerRequest).response;
