...
```bash title="Terminal" showLineNumbers={false}
npx amplify sandbox --config-format json-mobile --config-out-dir <path_to_android_project>/src/main/res/raw/
```
...
```bash title="Terminal" showLineNumbers={false}
npx amplify sandbox --config-format dart --config-out-dir <path_to_flutter_project>/lib/
```
...
Now let's add the necessary plugins into the Flutter application by customizing the `main()` function of the **lib/main.dart** file:
...
Start by adding a new state to track the todos and the ability to fetch the todo list when it first renders:
