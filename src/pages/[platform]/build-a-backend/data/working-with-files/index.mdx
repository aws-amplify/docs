...
// If the record has no associated file, we can return early.
guard coverArtPath = song.coverArtPath else {
...
let imagePaths = await withTaskGroup(of: String?.self) { group in
    for imageData in imagesData {
        group.addTask {
            ...
            do {
                _ = try await Amplify.Storage.uploadData(path: .fromString(path), data: imageData
                ).value
                return path
            } catch {
...
_ = try await Amplify.API.mutate(request: .update(createdAlbum)).get()
```
</InlineFilter>
...
guard let album = try await Amplify.API.query(request: .get(PhotoAlbum.self, byIdentifier: currentAlbum.id)).get() else {
    print("Album may have been deleted, no album by id: ", currentAlbum.id)
    return
}
...
class SongViewModel: ObservableObject {
    ...
    func deleteSongAndArtCover() async throws {
        ...
        // Get the song record
        guard var song = try await Amplify.API.mutate(request: .get(Song.self, byId: currentSong.id)).get() else {
            print("Error: could not retrieve the song")
            return
        }
...
class PhotoAlbumViewModel: ObservableObject {
    ...
    func test(currentAlbum: PhotoAlbum) async throws {
        // Get the album record
        guard let album = try await Amplify.API.query(request: .get(PhotoAlbum.self, byIdentifier: currentAlbum.id)).get() else {
            print("Album may have been deleted, no album by id: ", currentAlbum.id)
            return
    ...
    func removeStorageAssociationsFromAlbum() async throws {
        ...
        // Get the album record
        guard var album = try await Amplify.API.mutate(request: .get(PhotoAlbum.self, byId: currentAlbum.id)).get() else {
            print("Error: could not retrieve album")
            return
        }
