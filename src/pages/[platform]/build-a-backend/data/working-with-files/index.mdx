...
_ = Amplify.Storage.uploadData(path: .fromString("images/\(createdSong.id)-cover.png"), data: imageData)

// Update the song record with the image key
createdSong.coverArtPath = createdSong.id

_ = try await Amplify.API.mutate(request: .update(createdSong)).get()
```
</InlineFilter>
...
<InlineFilter filters={["swift"]}>
```swift title="ContentView"
let imagePath = "images/\(currentSong.id)-cover.png"

// Upload the new art image
_ = try await Amplify.Storage.uploadData(path: .fromString(imagePath), data: imageData).value

// Add the file association to the record:
currentSong.coverArtPath = imagePath

let result = try await Amplify.API.mutate(request: .update(currentSong)).get()

// Retrieve the file's signed URL:
let signedUrl = try await Amplify.Storage.getURL(key: imagePath)
```
</InlineFilter>
...
let updateResult = try await Amplify.API.mutate(request: .update(song!)).get()
```
</InlineFilter>
...
let imagePaths = await withTaskGroup(of: String?.self) { group in
    for imageData in imagesData {
        group.addTask {
            let path = "\(album.id)-\(UUID().uuidString)"
            do {
                _ = try await Amplify.Storage.uploadData(path: .fromString(path),
                                                          data: imageData
                ).value
                return path
            } catch {
...
let updateResult = try await Amplify.API.mutate(request: .update(createdAlbum)).get()
```
</InlineFilter>
...
let path = "\(currentAlbum.id)-\(UUID().uuidString)"
...
// Add new path to the existing paths
...
// Remove the file association from the record:
...
guard let imagePathsOptional = album?.imagePaths else {
    ...
    let deleteResult = try await Amplify.API.mutate(request: .delete(album!)).get()
    return
}
...
struct WorkingWithFilesApp: App {
    ...
    init() {
        _ = AWSAPIPlugin(modelRegistration: AmplifyModels())
        do {
...
class SongViewModel: ObservableObject {
    ...
    func createSong(name: String, artCover: UIImage) async throws {
        ...
        // Upload the art cover image
        _ = try await Amplify.Storage.uploadData(path: .fromString("images/\(createdSong.id)"),
                                                 data: imageData).value
    ...
    func updateArtCover(artCover: UIImage) async throws {
        ...
        // Upload the new art image
        _ = try await Amplify.Storage.uploadData(path: .fromString("images/\(currentSong.id)"),
                                                 data: imageData).value

        // Update the song record
        currentSong.coverArtPath = currentSong.id
    ...
    func removeImageAssociationFromSong() async throws {
        ...
        // Get the song record
        var song = try await Amplify.API.mutate(request: .get(Song.self, byIdentifier: currentSong.id)).get()
    ...
    func removeImageAssociationAndDeleteImage() async throws {
        ...
        guard var song = try await Amplify.API.query(request: .get(Song.self, byIdentifier: currentSong.id)).get() else {
            print("Error: could not retrieve the song")
            return
        }
    ...
    func deleteSongAndArtCover() async throws {
        ...
        // Get the song record
        guard var song = try await Amplify.API.mutate(request: .get(Song.self, byIdentifier: currentSong.id)).get() else {
            print("Error: could not retrieve the song")
            return
        }
...
class PhotoAlbumViewModel: ObservableObject {
    ...
    func addAdditionalPhotos(_ photo: UIImage) async throws {
        ...
        guard var album = try await Amplify.API.query(request: .get(PhotoAlbum.self, byIdentifier: currentAlbum.id)).get() else {
            print("Error: could not retrieve album")
            return
        }
    ...
    func refreshAlbumAndPhotos() async throws {
        ...
        guard let album = try await Amplify.API.query(
            request:
                    .get(PhotoAlbum.self,
                         byIdentifier: currentAlbum.id)).get() else {
            print("Error: could not retrieve album")
            return
        }
    ...
    func removeStorageAssociationsFromAlbum() async throws {
        ...
        // Get the album record
        guard var album = try await Amplify.API.mutate(request: .get(PhotoAlbum.self, byIdentifier: currentAlbum.id)).get() else {
            print("Error: could not retrieve album")
            return
        }
