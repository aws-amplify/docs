...
Use "userPool" as your authorization mode when if defined defined Amazon Cognito user pool-based authorization rules, such as `allow.authenticated()`, `allow.owner()`, `allow.ownerDefinedIn()`, `allow.groupsDefinedIn()`, or `allow.groups()`.
...
Use "identityPool" as your authorization mode when if defined defined Amazon Cognito identity pool-based authorization rules, such as `allow.guest()` or `allow.authenticated('identityPool')`.
...
Use "Lambda Authorizer" when you defined your own custom authorization logic via `allow.custom()`. Review [Customize your auth rules](/[platform]/build-a-backend/data/customize-authz) to learn more about how to implement your authorization protocol.
...
```swift
try await Amplify.API.query(request: .list(Todo.self, authMode: .apiKey))
```
...
```swift
try await Amplify.API.query(request: .list(Todo.self, authMode: .amazonCognitoUserPools))
```
...
```swift
try await Amplify.API.query(request: .list(Todo.self, authMode: .awsIAM))
```
...
```swift
try await Amplify.API.query(request: .list(Todo.self, authMode: .openIDConnect))
```
...
```swift
try await Amplify.API.query(request: .list(Todo.self, authMode: .function))
```
