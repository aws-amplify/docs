...
For more on the Amplify GraphQL API, see the [API documentation](/gen1/[platform]/build-a-backend/graphqlapi/set-up-graphql-api/).
...
Save the file and run `npx amplify sandbox` to deploy the changes to your backend cloud sandbox. For the purposes of this guide, we'll build a Real Estate Property listing application.
  ...
  const createMutation = useMutation({
    ...
    onMutate: async (newRealEstateProperty) => {
      ...
      if (previousRealEstateProperties) {
        queryClient.setQueryData(["realEstateProperties"], (old: Schema["RealEstateProperty"][]) => [
          ...old,
...
function App() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { data: property } = await client.models.RealEstateProperty.get({ currentRealEstatePropertyId });
      return property;
    },
  });
  // highlight-end
...
Once the backend has been provisioned, run `npx amplify generate graphql-client-code --format modelgen --model-target swift --out <path_to_swift_project>/AmplifyModels` to generate the Swift model types for the app.
