...
export const meta = {
  ...
  description:
    'Build search and aggregate queries with Amazon OpenSearch Service.',
  platforms: [
    'android',
    'angular',
    'flutter',
...
export const storage = defineStorage({
  name: "opensearch-backup-bucket-amplify-gen-2-test",
  access: allow => ({
    'public/*': [
...
```
Retrieve the `s3BucketArn` and `s3BucketName` values from the storage resource as demonstrated below. Additionally, set up an IAM role for the pipeline and allocate the roles as specified. For additional details on the necessary IAM roles, please consult the [Setting up roles and users documentation](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/pipeline-security-overview.html#pipeline-security-create).
...
Let's create the search resolver. Create a new file named `amplify/data/searchBlogResolver.ts` and paste the following code. For additional details please refer to [Amazon OpenSearch Service Resolvers](https://docs.aws.amazon.com/appsync/latest/devguide/tutorial-elasticsearch-resolvers-js.html)
