...
export const meta = {
  title: 'Connect to Amazon Translate for Translate language APIs',
  description:
...
# Connect to Amazon Translate for Translate language APIs
...
## Step 2 - Install Translate Libraries
...
## Step 3 - Configure Http Datasource and IAM Roles

 To access Amazon Translate service, you need to add translate as HttpDataSource and configure the proper IAM policy for Lambda to utilize the desired feature effectively. Update `amplify/backend.ts` file add Role policy shown below.
...
Custom resolvers work on a "request/response" basis.Define the custom resolver by creating a new file, `amplify/data/translate.ts`. 
...
In your `amplify/data/resource.ts` file, you can create a custom query and custom handler using `a.handler.custom`. Specify `TranslateDataSource` as the data source and `translate.js` as the entry point.
