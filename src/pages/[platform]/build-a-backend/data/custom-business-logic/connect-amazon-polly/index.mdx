...
Set up your project by following the instructions in the (Quickstart guide)[https://docs.amplify.aws/javascript/start/quickstart/]. 

## Step 2 - Install Polly Libraries
 To install the Amazon Polly library, run the following command in your project's root folder:
...
Customize your storage configuration to control access to different paths in your storage bucket. Create a file named `amplify/storage/resource.ts` and paste the following content:
...
## Step 4 - Configure IAM Roles
 ...
 We need to set up the appropriate IAM policy for Roles. Add Amazon Polly as a source in `amplify/backend.ts` file.
...
We need to modify the Storage resource to grant access to the `convertTextToSpeech` resource. Update the file `amplify/storage/resource.ts` as shown below.
