...
if (!updatedSong.converArtPath) return;

// Retrieve the file's signed URL:
const signedURL = await getUrl({ path: updatedSong.converArtPath });
...
// If the record has no associated file, we can return early.
if (!song?.converArtPath) return;

// Retrieve the signed URL:
const signedURL = await getUrl({ path: song.converArtPath });
...
await remove({ path: song.converArtPath });
...
function App({ signOut, user }) {
  ...
  async function removeImageFromSong() {
    ...
    try {
      ...
      setCurrentImageUrl(updatedSong.data?.converArtPath);
    } catch (error) {
