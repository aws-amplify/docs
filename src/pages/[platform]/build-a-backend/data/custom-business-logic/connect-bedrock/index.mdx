...
backend.generateHaikuFunction.resources.lambda.addToRolePolicy(
  new PolicyStatement({
    ...
    resources: [
      `arn:aws:bedrock:*::foundation-model/${process.env.MODEL_ID}`,
    ],
  })
);
```
...
In the code above, the ID for the Amazon Bedrock foundation model is set as an environment variable. We recommend setting your model id as an environment variable as it will need to be referenced in several places and you may want to use a different model. To learn more about setting environment variables in an Amplify app, please refer to our documentation on [secrets and environment variables](/[platform]/deploy-and-host/fullstack-branching/secrets-and-vars/#local-environment).

This will require creating a `.env` file in the root of your project with the following content:

```sh title=".env"
MODEL_ID=anthropic.claude-3-haiku-20240307-v1:0
```

Then, installing the `dotenv` package from npm to load the environment variables:

```sh title="Terminal"
npm install dotenv
```

For the purpose of this guide, we will use Anthropic's Claude 3 Haiku to generate content. If you want to use a different model, you can find the ID for your model of choice in the Amazon Bedrock documentation's [list of model IDs](https://docs.aws.amazon.com/bedrock/latest/userguide/model-ids.html) or the [Amazon Bedrock console](https://console.aws.amazon.com/bedrock/) and replace the value of `MODEL_ID` in your `.env` file.
