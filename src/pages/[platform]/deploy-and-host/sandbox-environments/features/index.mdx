...
The `secret()` function can only be used in specific places in your backend definition such as [configuring auth providers](/[platform]/build-a-backend/auth/add-social-provider/#configure-social-sign-in-backend) and [granting function secret access](/[platform]/build-a-backend/functions/#secret-access).
...
npx amplify sandbox --outputs-out-dir ./path/to/config --outputs-format ["json", "dart"]
```
...
```bash
npx amplify generate outputs --app-id <your-amplify-app-id> --branch main --format ["json", "dart"] out-dir ./path/to/config
```
</InlineFilter>
