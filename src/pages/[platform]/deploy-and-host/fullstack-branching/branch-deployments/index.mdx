...
```bash title="Terminal" showLineNumbers={false}
npx amplify generate outputs --app-id=<app-id> --branch main --format json-mobile --out-dir app/src/main/res/raw
```
</InlineFilter>
...
```bash title="Terminal" showLineNumbers={false}
npx amplify generate outputs --app-id=<app-id> --format=json-mobile
```
