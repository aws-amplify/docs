...
Update the build settings for the `backend` phase to run `npx amplify generate outputs --branch dev app-id $AWS_APP_ID` to generate the `amplifyconfiguration.json` file for all preview branches. After this update, any new deployed preview branches will not deploy backend resources as part of the build and instead will use the deployed backend resources from the `dev` branch.
                ...
                - |
                  ...
                  case "${AWS_BRANCH}" in
                      ...
                      pr-*)
                          ...
                          npx amplify generate outputs --branch dev --app-id $AWS_APP_ID 
                          ;;
                      *)
                          echo "Deploying to staging branch..."
                          npx amplify generate outputs --branch staging --app-id $AWS_APP_ID 
                          ;;
                  esac
                  // highlight-end
frontend:
