  ...
  branches: [
    {
      ...
      stages: [
        { name: 'staging' },
        { name: 'production', requireApproval: true, bakeTime: 30 }
      ]
    }
  ]
});
...
The `requireApproval` option pauses the pipeline before deploying to that stage, waiting for manual approval in the AWS CodePipeline console. The `bakeTime` value (in minutes) adds a waiting period after deployment to monitor for errors before proceeding.
...
export const pipeline = definePipeline({
  ...
  synth: {
    commands: [
      'npm ci',
      'npm run build',
      'npx ampx deploy --identifier $STAGE_NAME --frontend --backend'
    ],
    computeType: 'large',
    dockerEnabled: true,
    env: {
...
The `computeType` option accepts `small`, `medium`, `large`, or `xlarge` and controls the CodeBuild instance size. Enable `dockerEnabled` if your build process requires Docker (for example, building container images).
...
Deploy the pipeline itself with the `--pipeline` flag:

```bash title="Terminal"
npx ampx deploy --identifier production --pipeline
```
