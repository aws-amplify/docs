...
Before starting the migration process in this new environment, it is recommended to add test data to the categories (API, Auth, Storage) using Amplify Studio. Please ensure Studio is enabled for your Amplify app.
For Data category, you can use the [Seed data feature](https://docs.amplify.aws/gen1/react/tools/console/data/content-management/) in Amplify Studio.
...
The resource mapping JSON is an array of resource mappings, with each mapping corresponding to an unique resource you want to move from Gen1 to Gen2. Each resource mapping comprises of 2 fields (Source and Destination) which contain the stackName and the logicalId of the resource in Source and Destination stacks respectively:
