...
- [Local AWS CLI profile](/gen1/[platform]/start/getting-started/installation/) with `AdministratorAccess-Amplify` and `theAmplifyBackendDeployFullAccess` policies attached.
...
```bash title="Terminal" showLineNumbers={false}
npx @aws-amplify/migrate@0.1.0-next-3.0 to-gen-2 prepare
```
...
We will need to add the following manually to `backend.ts` file to set up the additional user pool client for your `Identity providers`. This is needed since Gen2 only supports 1 user pool client out-of-the box:
...
The add the following to `backend.ts` file; you will need to disable the `sseSpecification` override for each table configured in Gen 1. If the table does have a `sseSpecification` enabled in Gen 1, you can skip this step.

```typescript
// do for each table
// default for SSE changed to from false -> true in gen 2
// Set the gen 2 table to sseEnabled: false so that the table definitions match
...
For example, if you have a NodeJS Lambda function In Gen 1, you will see the following error in your sandbox run:
...
If you have not deployed a Amplify Gen 2 backend before, the process will redirect you the Amplify Console to bootstrap your region. Ensure you are logged with Admin credentials.
If you do experience any errors, refer to the [troubleshooting section]([platform]/build-a-backend/troubleshooting/).
...
```bash title="Terminal" showLineNumbers={false}
npx @aws-amplify/migrate@0.1.0-next-3.0 to-gen-2 execute \
...
```bash title="Terminal" showLineNumbers={false}
npx @aws-amplify/migrate@0.1.0-next-3.0 to-gen-2 revert \
...
```typescript
(backend.auth.stack.node.children.find(child => child.node.id === 'amplifyAuth') as any).domainPrefix;
```
