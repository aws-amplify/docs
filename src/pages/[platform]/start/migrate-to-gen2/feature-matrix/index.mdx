...
The simplest path is to connect your Git branch in the Amplify Console (_App Settings → Branch Settings → Add Branch_). See [branch deployments](https://docs.amplify.aws/react/deploy-and-host/amplify-hosting/branch-deployments/) for details.
...
If you need direct access to your infrastructure or want to deploy to your own AWS account, you can use [self-managed hosting](/[platform]/deploy-and-host/self-hosting/). Define your hosting configuration with `defineHosting()` and deploy using `ampx deploy` or a CI/CD pipeline you control.
...
See the [custom pipelines guide](/[platform]/deploy-and-host/amplify-hosting/custom-pipelines/) for full details.
