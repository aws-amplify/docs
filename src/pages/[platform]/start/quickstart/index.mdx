...
Create a `createTodo` function in the `MyAmplifyAppApp.swift` file with the following code:
...
Next create a `listTodos` function in the `MyAmplifyAppApp.swift` for listing to-do items:
...
```bash title="Terminal" showLineNumbers={false}
cd my-android-app
npx @aws-amplify/backend-cli graphql-client-code --format=modelgen --model-target=java --out=app/src/main/java --app-id APP-ID --branch-name main
```
...
```bash
git clone git clone https://github.com/<github-user>/amplify-backend-template.git
```
...
Now, let's go back to your Android application and test out the user isolation of the to-do items. Fetch the latest `amplify-outputs` and model files by re-running the following command in your Android Studio terminal.
...
npx @aws-amplify/backend-cli generate outputs --format=modelgen --model-target=java --out=app/src/main/java --app-id APP-ID --branch-name main
```

Also update your `amplify-outputs` file with the latest outputs information.

```bash showLineNumbers={false}
npx @aws-amplify/backend-cli generate config --out-dir=app/src/main/res/raw --app-id APP-ID --branch main
```
