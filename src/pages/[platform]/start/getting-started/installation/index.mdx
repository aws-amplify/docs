...
region = us-west-2
output = json
+ credential_process = aws configure export-credentials --profile my-dev-profile
...
Using the example above, when creating a new app or pulling an existing app, specify `my-dev-profile` as the AWS profile you'd like to use with the Amplify app.
