...
The schema generated is for a Todo app. You'll notice a `@model` directive on the `Todo` type. This directive is part of the [Amplify GraphQL API](/cli/graphql/data-modeling) category.
...
First, enable the Angular forms modules in `src/app/app.component.ts`:
