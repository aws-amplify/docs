...
Create an `amplify_outputs.json` file in your project with the configuration for your resources:
...
Create an `amplify_outputs.json` file in your Xcode project with the configuration for your resources, then configure Amplify as usual:
...
Create an `amplify_outputs.json` file in your `app/src/main/res/raw/` directory with the configuration for your resources, then configure Amplify as usual:
...
### Option 1: Manual `amplify_outputs.dart`

Generate an `amplify_outputs.dart` file from your JSON configuration:
...
val config = AmplifyOutputsData(
    auth = AmplifyOutputsData.Auth(
        ...
        userPoolClientId = "abcdef123456",
        identityPoolId = "us-east-1:11111111-2222-3333-4444-555555555555",
        passwordPolicy = AmplifyOutputsData.Auth.PasswordPolicy(
            minLength = 8,
            requireNumbers = true,
            requireLowercase = true,
            requireUppercase = true,
        ...
        oauth = AmplifyOutputsData.Auth.Oauth(
            identityProviders = listOf(
                AmplifyOutputsData.Auth.IdentityProviders.GOOGLE,
                AmplifyOutputsData.Auth.IdentityProviders.SIGN_IN_WITH_APPLE
            ),
            domain = "myapp.auth.us-east-1.amazoncognito.com",
            scopes = listOf("openid", "email", "profile"),
            redirectSignInUri = listOf("myapp://callback"),
            redirectSignOutUri = listOf("myapp://signout"),
            responseType = AmplifyOutputsData.Auth.ResponseType.Code
        ),
        standardRequiredAttributes = listOf(
            AmplifyOutputsData.AmazonCognitoStandardAttributes.EMAIL
        ),
        usernameAttributes = listOf(
            AmplifyOutputsData.Auth.UsernameAttributes.EMAIL
        ),
        userVerificationTypes = listOf(
            AmplifyOutputsData.Auth.VerificationMechanism.EMAIL
        ),
        unauthenticatedIdentitiesEnabled = true,
        mfaConfiguration = AmplifyOutputsData.Auth.MfaConfiguration.OPTIONAL,
        mfaMethods = listOf(
...
val config = AmplifyOutputsData(
    data = AmplifyOutputsData.Data(
        ...
        url = "https://abc123.appsync-api.us-east-1.amazonaws.com/graphql",
        apiKey = "da2-abcdefghijklmno",
        defaultAuthorizationType = AmplifyOutputsData.AwsAppsyncAuthorizationType.API_KEY,
...
val config = AmplifyOutputsData(
    ...
    storage = AmplifyOutputsData.Storage(
        awsRegion = "us-east-1",
        bucketName = "my-app-bucket"
    )
)
...
val config = AmplifyOutputsData(
    storage = AmplifyOutputsData.Storage(
        ...
        bucketName = "primary-bucket",
        buckets = listOf(
            AmplifyOutputsData.Storage.Bucket(
                name = "media",
...
```kotlin
val config = AmplifyOutputsData(
    analytics = AmplifyOutputsData.Analytics(
        amazonPinpoint = AmplifyOutputsData.Analytics.AmazonPinpoint(
            awsRegion = "us-east-1",
            appId = "abc123def456"
        )
    )
...
```kotlin
val config = AmplifyOutputsData(
    geo = AmplifyOutputsData.Geo(
        awsRegion = "us-east-1",
        maps = AmplifyOutputsData.Geo.Maps(
            items = mapOf(
                "myMap" to AmplifyOutputsData.AmazonLocationServiceConfig(
                    style = "VectorEsriStreets"
                )
            ),
            default = "myMap"
        ),
        searchIndices = AmplifyOutputsData.Geo.SearchIndices(
            items = listOf("myPlaceIndex"),
            default = "myPlaceIndex"
        ),
        geofenceCollections = AmplifyOutputsData.Geo.GeofenceCollections(
...
```kotlin
val config = AmplifyOutputsData(
    notifications = AmplifyOutputsData.Notifications(
        awsRegion = "us-east-1",
        amazonPinpointAppId = "abc123def456",
        channels = listOf(
            AmplifyOutputsData.AmazonPinpointChannels.FCM,
            AmplifyOutputsData.AmazonPinpointChannels.IN_APP_MESSAGING
        )
    )
)
...
val config = AmplifyOutputsData(
    ...
    data = AmplifyOutputsData.Data(
        awsRegion = "us-east-1",
        url = "https://abc123.appsync-api.us-east-1.amazonaws.com/graphql",
        defaultAuthorizationType = AmplifyOutputsData.AwsAppsyncAuthorizationType.AMAZON_COGNITO_USER_POOLS,
        authorizationTypes = listOf(
            AmplifyOutputsData.AwsAppsyncAuthorizationType.AMAZON_COGNITO_USER_POOLS,
