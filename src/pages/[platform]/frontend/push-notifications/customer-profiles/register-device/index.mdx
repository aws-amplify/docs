...
`registerDevice` is supported on React Native only. On web it throws a `PlatformNotSupportedError`, because registration depends on a push token issued by the native platform.
