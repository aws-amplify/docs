...
First, ensure you've provisioned an Amazon Location Service Map resource and configured your app using the instructions in either [Set up map](/[platform]/frontend/geo/set-up-geo/) or [Use existing resources](/[platform]/frontend/geo/existing-resources/) guide.
...
First, ensure you've provisioned an Amazon Location Service Map resource and configured your app using the instructions in either [Configure maps](/[platform]/frontend/geo/set-up-geo/) or [Use existing resources](/[platform]/frontend/geo/existing-resources/) guide.
...
The `AmplifyMapView` provides a default search field, place markers, visualization modes (map or list) and map controls. It can be used to easily embed a place picker into any app. To use the search functionality of `AmplifyMapView`, provision a search index resource using the instructions in either [Configure Location Search](/[platform]/frontend/geo/configure-location-search/) or [Use existing Amazon Location Service resources](/[platform]/frontend/geo/configure-location-search/).
