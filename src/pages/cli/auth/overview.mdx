...
This error may occur when modifying the [`breakCircularDependency`](/cli/reference/feature-flags/#breakCircularDependency) flag and push a resource that depends on the auth category. To mitigate the issue run `amplify update auth`, walkthrough the auth flow keeping the previous settings and run `amplify push` or directly run `amplify push --force`.
