...
| User Group | User needs to sign in and belong to the User Pool Group to use the REST API|
...
| User Group | create, read, update, delete | POST, GET, PUT, PATCH, DELETE | Amazon Cognito |
...
## Create REST API and restrict specific routes to specific user groups
If your app uses user-pool groups to manage different user types and would like to restrict access of specific routes to specifc user groups. We can accomplish this by the following flow: 
- Create API route. 
- Add API route handler function.
- Restrict-access to the API route to the user-group. The user-group 

>
> The following example flow assumes the existence of two user pools AdminUsers, GuestUsers for a Book store.
> The app would like to limit admin functionality like updating book records to the AdminUsers user-group, 
> while borrowing and returning books would be limited to the GuestUsers user-group. 
...
```
At the end of this command you can verify the routes and their respective user group restrictions in cli-inputs.json file at the following path.
```bash
