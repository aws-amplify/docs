...
When creating custom resources using Amplify CLI, the CLI may require additional permissions outside the Amplify managed policy, `AdministratorAccess-Amplify`.
Please refer to the Amplify [IAM Policy](/cli/reference/iam) documentation for additional information regarding the necessary permissions for Amplify CLI.
