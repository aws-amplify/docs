...
> Note: Specifying resource as ‘*’ is not recommended as best practice. This gives the Amplify function resource Administrative privileges which should be avoided.


If your Amplify resource requires access to multiple services create another block to grant access to additional services. 
...
You can specify which environment for the custom policies by adding '\$\{env\}' to the resource AWS resource ARN optionally, the '${env}' will be mapped to the current deployment environment name. 
...
On running `amplify push` commands the attributes specified in the `custom-policies.json` file will be appended to the existing IAM policy list tied to the Lambda Function's execution role.
