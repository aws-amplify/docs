...
The storage resource created by Amplify CLI has retention enabled which prevents any accidental deletion or loss of data. Hence, running `amplify remove storage` will not delete the storage resource and will need to be manually deleted on the AWS console.
