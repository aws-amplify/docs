...
| --- | --- |
| AMPLIFY_STORAGE_REGION | The region of the DynamoDB tables. |
| AMPLIFY_STORAGE_TABLES | The names of storage resources and DynamoDB tables imported for storage. |

The format of `AMPLIFY_STORAGE_TABLES` is as follows :

```json
{
  "{YOUR_STORAGE_RESOURCE_NAME_1}": "{YOUR_DYNAMODB_TABLE_NAME_1}",
  "{YOUR_STORAGE_RESOURCE_NAME_2}": "{YOUR_DYNAMODB_TABLE_NAME_2}" // If there are more than one imported tables
}
```

> Make sure to replace `{YOUR_STORAGE_RESOURCE_NAME}` and `{YOUR_DYNAMODB_TABLE_NAME}`. You can find the names of storage resources and DynamoDB tables in your `amplify/team-provider-info.json` file.