...
The CLI collects non-sensitive files of your Amplify backend and zips them and transmits the zip file to a secure location. Below are the files CLI collects, transmits and stores.
...
To help improve Amplify CLI you can opt in to automatically share your project configurations with Amplify CLI on failures. The is a project level setting and can be toggled per project. The project can be opt out by running 
...
The CLI collects two extra files including the files the configuration files collected in send report
