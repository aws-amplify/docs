...
## Configuring the CLI when there is an existing IAM user

If you've previously configured the Amplify CLI and created an IAM user, and are now installing the CLI on a new machine, it is recommended to create a new IAM user on the new machine (rather than attempt to use the previous IAM user). Having a distinct user for each machine provides the best level of visibility and control without breaking the deployment of your app.
