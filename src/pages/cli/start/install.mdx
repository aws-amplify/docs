...
> If you are using CLI version <= 10.6.2 go to [Configure CLI <= 10.6.2](#configure-the-amplify-cli-version--1062)
...
Enter a User name and click Next. You can name the user anything but we'll call it "amplify-dev".

![image](/images/cli/user-creation/user-name.png)

Select “Attach policies directly” and select “AdministratorAccess-Amplify” as the Permissions policy. Click Next.

![image](/images/cli/user-creation/user-permissions.png)

On the Review page, check that everything looks good and click Create user.
...
On the user details page, navigate to the “Security credentials”, scroll down to “Access keys” and click “Create access keys”

![image](/images/cli/user-creation/create-access-keys.png)

On the next page, select “Command Line Interface”, acknowledge the warning and click Next

![image](/images/cli/user-creation/ack-page.png)

On the next page click create access key. You’ll then see a page with the access keys for the user. Use the copy icon to copy these values to your clipboard, then return to the Amplify CLI.
...
## Configure the Amplify CLI version <= 10.6.2
