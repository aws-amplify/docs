...
```ts
    const amplifyApi = new AmplifyGraphqlApi(this, "MyNewApi", {
      ...
      authorizationModes: {
        ...
        iamConfig: {
          unauthenticatedUserRole: TK // this seems too complicated than it needs to be https://stackoverflow.com/questions/55784746/how-to-create-cognito-identitypool-with-cognito-userpool-as-one-of-the-authentic
        }
      },
    })
```
  ...
  authorizationModes: {
    ...
    lambdaConfig: {
      function: new lambda.Function(this, "MyAuthLambda", {
        code: lambda.Code.fromAsset(path.join(__dirname, "handlers/auth")), // TK verify
