export const meta = {
  title: `Custom business logic (Lambda function, HTTP, VTL)`,
  description: `Add authorization rules to your GraphQL schema to control access to your data.`,
};

Define your custom business logic in a Lambda function resolver, HTTP resolver, or VTL resolver and expose them in a GraphQL query or mutation. Extend or overwrite Amplify-generated GraphQL resolvers to optimize for your specific use cases.  

TODO: some graphics about how GraphQL resolvers work etc.
...
The `@function` directive allows you to quickly & easily configure a AWS Lambda resolvers with your GraphQL API. You can use any AWS Lambda functions that was created the Amplify CLI, AWS Lambda console, or any other tool.
...
## Overwrite Amplify-generated resolvers
...
![Resolver folders structure in Amplify project folder](/images/cli/resolver_folder_structure.png)

To overwrite an Amplify-generated resolver:
1. Find the resolver file name you want to overwrite under `build/resolvers`
...
|   ...
├── resolvers
│   └── Query.searchTodos.req.vtl # Place overwrites with the same file name
```

The example above shows how the `Query.searchTodos.req.vtl` is overwritten with a custom resolver. 
...
For example, the a resolver function file named `Mutation.createTodo.postAuth.2.req.vtl` will be slotted in right after the `Mutation.createTodo.postAuth.1.req.vtl` resolver.
