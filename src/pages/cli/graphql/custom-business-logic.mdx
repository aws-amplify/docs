...
3. Optionally, secure your custom query or mutation with [field-level auth](/cli/graphql/authorization-rules/)]
    - `@auth`on a custom query or mutation only supports static groups, iam, and apiKey. Owner and dynamic groups are currently mapped to the model generated operations since those dynamic fields are handled directly in the resolvers.
