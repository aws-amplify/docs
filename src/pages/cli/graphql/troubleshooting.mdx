...
## Generating models with improved pluralization

When you generate models with the GraphQL [`improvePluralization`](/cli/reference/feature-flags/#improvePluralization) feature flag enabled, they will be incompatible with iOS SDK versions earlier than 1.15.4 and Android SDK versions earlier than 1.28.3. This can manifest in a few different errors, an error such as `Property 'listPluralName' not found` would imply that you must update your SDK version to use improved pluralization. If you are using an up to date SDK version, you will not have a problem with unrecognized properties.
