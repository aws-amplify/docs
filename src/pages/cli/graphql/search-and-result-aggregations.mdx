...
- get aggregation values, such as total counts, sum, average, min, max
...
By default, the search result page size is 100. To customize the page size modify the `limit` parameter.

TODO FROM vs NEXT TOKEN?
...
## Aggregate values for search result (minimum, maximum, average, sum)
