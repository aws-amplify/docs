...
4. Use the following snipped to create references to the API ID and environment name
```typescript
const apiIdRef = cdk.Fn.ref(retVal.api.thing.GraphQLAPIIdOutput);
const envRef = cdk.Fn.ref('env');
```

5. Your custom resolver will need the table name in it. To build the table name, you must use the CDK references inside the template.
```VTL
