...
4. Your custom resolver will need the table name in it. To build the table name, you must instantiate the CDK references inside the template. Additionally, the template must be defined directly as an input when creating your resolver. If you create the resolver string as a template prior to passing it in as an argument, the CDK references will be evaluated to their parameter names rather than their parameter values.
```VTL
...
$util.toJson( {
  ...
  "tables": {
    "<TYPE-NAME-HERE>-${cdk.Fn.ref(retVal.api.thing.GraphQLAPIIdOutput)}-${cdk.Fn.ref("env")}": $todosArray
  }
} )
```

5. The table name is also used to get the data in the response VTL
```VTL
## [Start] ResponseTemplate. **
#if( $ctx.error )
  $util.error($ctx.error.message, $ctx.error.type)
#else
  $util.toJson($ctx.result.data.<TYPE-NAME-HERE>-${cdk.Fn.ref(retVal.api.thing.GraphQLAPIIdOutput)}-${cdk.Fn.ref("env")})
#end
