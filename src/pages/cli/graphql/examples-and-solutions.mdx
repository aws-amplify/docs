...
export class cdkStack extends cdk.Stack {
  constructor(
    ...
    const apiIdRef = cdk.Fn.ref(retVal.api["<YOUR-API-NAME>"].GraphQLAPIIdOutput);

    const tableName = "<YOUR-MODEL-NAME>" + "-" + apiIdRef + "-" + envRef;

    const requestVTL = `
      ## [Start] Initialization default values. **
      #set( $tableName = $util.dynamodb.tableName("TodoTable") )
      ...
      #set( $batchPutRequest = {} )
      $util.qr($batchPutRequest.put("version", "2018-05-29"))
      $util.qr($batchPutRequest.put("operation", "BatchPutItem"))
      $util.qr($batchPutRequest.put("tables", { "${tableName}": $todosArray }))
      $util.toJson($batchPutRequest)
    `;
