...
Sometimes it's preferred to create objects in bulk, rather than subsequently creating individual objects and waiting for all the requests to complete.

1. Define your schema with a custom mutation (the custom mutation should not be deployed to AppSync beforehand if following these steps)
```graphql
...
2. Before pushing the new custom mutation, [create a custom resource for your resolver](/cli/graphql/custom-business-logic/#vtl-resolver) and use the following code snippets as a guide to get started with your VTL

3. Use the following imports as a starting point for your custom CDK stack
```typescript
...
4. Use this snippet to get the DynamoDB table name (based on type name, API ID, and environment)
```typescript
const envName = stateManager.getCurrentEnvName();
const apiIdValue = stateManager.getCurrentMeta().api.<API-NAME-HERE>.output.GraphQLAPIIdOutput;
// Replace "Todo" with your model name 
const tableName = `Todo-${apiIdValue}-${envName}`
```

5. The table name is used in the VTL to direct the data in the request VTL
```VTL
...
$util.toJson( {
  ...
  "tables": {
    "<TABLE-NAME-HERE>": $todosArray
  }
} )
```

6. The table name is also used to get the data in the response VTL
```VTL
## [Start] ResponseTemplate. **
#if( $ctx.error )
  $util.error($ctx.error.message, $ctx.error.type)
#else
  $util.toJson($ctx.result.data.<TABLE-NAME-HERE>)
#end
## [End] ResponseTemplate. **
```

Further customization of the VTL can be made to implement authorization and/or relational capabilities. You may use the resolvers generated by the CLI as a starting point to see how some of these capabilities are implemented.
