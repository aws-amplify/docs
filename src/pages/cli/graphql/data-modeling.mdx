...
Because the `@default` directive provides a value when none is provided on creation, a non-null assertion (i.e. `String!`) is disregarded by the transformer.
...
The types for queries, mutations, and subscriptions are [found here](#type-definition-of-the-`@model`-directive) at the bottom of the page
