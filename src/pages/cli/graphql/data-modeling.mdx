...
If relational directives are used to create two of the same type of connection between the same two models, the transformer is not able to disambiguate the connections. However, with some additional specification it is doable. This avoids ambiguity by naming the connection fields directly instead of allowing the auto generated fields to have the same name and collide.
...
type Address @model {
  id: ID!
  homeIndividualID: ID @index(name: "homeIndex")
  shippingIndividualID: ID @index(name: "shipIndex")
