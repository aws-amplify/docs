...
The example above, disables the mutations, subscriptions and the `getTodo` query specifically. While allowing us to run other queries such as `listTodo`.

### Creating a custom query

We can create a custom query that enables us to retrieve a single Todo element even if the `queries: { get: null}` has been disabled.
