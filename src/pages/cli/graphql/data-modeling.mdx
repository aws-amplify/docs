...
You can define custom primary keys for relational models. The transformer will handle the foreign key fields needed by queries and mutations, which allows you to retrieve data across different relations. 

You can choose the default relations with **implicit** fields. This will result in auto generation of additional foreign key fields refering its counterpart primary key and sort key fields.

<Callout>
In "has many" relations, the foreign key fields are only generated in child type.
</Callout>
...
You can also customize the foreign key fields instead of the auto-generated ones in the schema by **explicitly** defining the `fields` parameter in relation directives.

<Callout>
The order in "fields" parameter matters. The first element should refer the primary key field and the rest are sort key fields.
</Callout>

<Callout>
When define explicit "has many" relation, the "@index" should be also defined on the child type.
</Callout>
...
```
This will result in the following AppSync schema built by transformer.
...
```
The transformer will automatically add both model's primary key and sort key fields in the joint table schema.
