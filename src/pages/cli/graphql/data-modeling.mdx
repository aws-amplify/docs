...
```graphql
type Todo @model(queries: { get: null, list: "listFor" }) {
  id: ID!
}
```

In the example above, you'll be able to run a `listForTodo` query to get a list of Todo elements. Query to get a single Item will be disabled. 

On performing a `amplify push` we can override the query by adding in.
