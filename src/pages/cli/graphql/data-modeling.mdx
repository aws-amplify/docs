...
If relational directives are used to create two of the same type of connection between the same two models, the transformer is not able to disambiguate the connections. However, by explicitly specifying the connection field names this ambiguity can be mitigated.
