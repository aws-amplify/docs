...
A secondary index consists of a "hash key" and a "sort key". Use the "hash key" to perform strict equality and the "sort key" for greater than (gt), greater than or equal to (ge), less than (lt), less than or equal to (le), equals (eq), begins with, and between operations. 
...
In the example above, you are able to query for "Customer" based on their "accountRepresentativeID":
