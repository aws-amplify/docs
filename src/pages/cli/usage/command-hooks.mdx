export const meta = {
  title: `Command Hooks`,
  description: `Execute custom scripts before, during, and after Amplify CLI commands (“amplify push”, “amplify api gql-compile”, and more). This allows you to extend Amplify’s best-practice defaults to meet your organization’s specific security guidelines and operational requirements.`,
};
...
Let's add `pre-push.js` in the `amplify/hooks` drectory with the following contents. *Note: You need to create a `amplify/hooks` folder if your Amplify project was created prior to Amplify CLI version 5.5.0*

```js
const fs = require("fs");
const parameters = JSON.parse(fs.readFileSync(0, { encoding: "utf8" }));

// Get the running Amplify CLI major verison number
...
if (currentCLIMajorVersion < MINIMUM_MAJOR_AMPLIFY_CLI_VERSION) {
  ...
  console.log('Mininum CLI version requirement met.')
  process.exit(0) 
}
```
...
Mininum CLI version requirement met.
...
Command hooks receive two paramaters, `data` and `error`. Amplify CLI passes parameters to hook scripts as a JSON string through standard input.
...
You can optionally add `hooks-config.json` file in the hooks directory to configure custom scripting runtimes or manage external dependencies.

### Adding a custom scripting runtime 
By default, Node.js and Bash are supported. To add more programming languages support, add `extensions` to a `hooks-config.json` file in the `amplify/hooks` folder.
