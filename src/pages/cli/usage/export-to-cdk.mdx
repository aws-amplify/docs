...
The export uses the current state of the Amplify backend to build, package, and generate the CloudFormation files and assets to the provided path. Read more in detail about the purpose of the files and structure generated https://quip-amazon.com/DYpUAaa3odlO/Amplify-CLI-Export-Docs#temp:C:JTdd2d3d79fe2794b87ba8d58808 section.
...
const amplifyStack = new AmplifyExportedBackend(this, "amplifyExportedBackend", {
  ...
  path: path.resolve(__dirname, 'amplify-export-<YOUR_AMPLFIY_PROJECT_NAME>')
})
```
...
Once the CDK app is deployed, run the following command to generate these client configurations for Android, iOS, Javascript, and Flutter apps.

```bash
amplify export pull --rootStackName <Amplify Root Stack Name> \
