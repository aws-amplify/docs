...
### Running GraphQL mock with custom port
You can use a `mock.json` file within the api directory of your project (ie. `<project root>/amplify/backend/api/<api name>/mock.json`) to customize the running ports for local DynamoDB and local AppSync.

Here is an example of a `mock.json` file:
...
In this example:
* The `graphqlPort` field allows you to change the local AppSync port. By default, the endpoint for local AppSync is `http://localhost:20002`
* The `port` field allows you to change the local DynamoDB port. By default, the endpoint for local DynamoDB is `http://localhost:62224`
