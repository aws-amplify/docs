...
> Note: Specifying resource as ‘*’ is not recommended as best practice. This gives the Amplify api resource Administrative privileges which should be avoided.
...
On running `amplify push` commands the attributes specified in the `custom-policies.json` file will be appended to the existing IAM policy list tied to the Fargate Task's execution role.
