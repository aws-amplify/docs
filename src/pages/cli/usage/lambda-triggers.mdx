...
const CaptchaConfirmation = (props) => {
  ...
  const handleCaptchaConfirmation = async () => {
    try {
      const res = await confirmSignIn({challengeResponse:props.authData})
  ...
  return (
    <div>
      { props.authState === 'customConfirmSignIn' && (
        <ReCAPTCHA
          sitekey="your-client-side-google-recaptcha-key"
          onChange={handleCaptchaConfirmation}
        />
      )}
    </div>
  );
...
function App() {
  ...
  return (
      <div className="App">
        ...
        <CaptchaConfirmation authState={authState} />
      </div>
  );
}
...
import { AmplifyAngularModule } from '@aws-amplify/ui-angular';
