...
> Be sure to follow all instructions for [setting up an Angular application](/start/start) with aws-amplify-angular, and [configure your Amplify instance](/lib/auth/switch-auth) to use the CUSTOM_AUTH flow.
...
Be sure to follow all instructions for [setting up an Angular application](/start/start) with aws-amplify-angular.
...
- As a part of the [GraphQL API (types with @model annotation)](/cli/graphql-transformer/model)
