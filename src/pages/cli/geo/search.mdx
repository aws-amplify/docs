...
Next, configure the access permissions for your location search index and authorize users to search for places.
You can scope permissions based on an individual user's authentication status, Cognito user pool group membership or both.
...
Select `Authorized users only` if only signed-in users are allowed to search for places.

Select `Authorized and Guest users` if both authenticated and unauthenticated users are allowed to search for places.
...
Select `Individual Groups` to scope access permissions based on [Cognito User Groups](/cli/auth/groups) if you have them already added to your project.
...
The pricing plan for Search Index is `RequestBasedUsage` by default.
