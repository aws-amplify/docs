...
<Callout>
You can check the Amplify CLI version you are using by running `amplify -v`.
If you are using Amplify CLI version `7.6.7` or older and have a Search Index added to your application with the pricing plan set to `MobileAssetTracking` or `MobileAssetManagement`,
please follow these instructions to update the pricing plan:
1. Upgrade the Amplify CLI to version `7.6.8` or later.
