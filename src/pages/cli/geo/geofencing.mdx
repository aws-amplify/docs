...
Then click on the `Add users` button that displays a window to select the users by their `username`, `email` etc. that you intend to add to the Cognito group.
...
This option allows you to choose your own fields inside `properties` of Feature object. Amplify CLI will help scan the candidate properties which exist in all feature objects and provide them as options in the walkthrough. The candidate properties are chosen based on the existance of all feature objects. The identifier field should have unique values among geofences and will be validated after being selected.
```console
? Select the property to use as the Geofence feature identifier:
  Use root-level "id" field. (Auto-assigned if missing)
