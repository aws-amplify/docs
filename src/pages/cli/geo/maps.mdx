...
Next, configure the access permissions for your Map resource and authorize users to access the map.
...
Select `Authorized users only` if only signed-in users are allowed to access the Map resource.

Select `Authorized and Guest users` if both authenticated and unauthenticated users are allowed to access the Map resource.
...
## Map Pricing Plan
The pricing plan for Map is `RequestBasedUsage` by default.
