...
When working with different environments you may want to create an environment that allow users to only access and make modifications to specific resources.

For this example lets consider a scenario where the team wants to create a environment thats allows the developers to only work with DynamoDB resources. We can use the [AWS managed](https://us-east-1.console.aws.amazon.com/iam/home#/policies/arn:aws:iam::aws:policy/AmazonDynamoDBFullAccess$serviceLevelSummary?section=permissions) policy for DynamoDB that allows users to add a DynamoDB table.

Add Permissions Boundary for the new environment, run:

```
amplify env add --permissions-boundary arn:aws:iam::aws:policy/AmazonDynamoDBFullAccess
```

We can verify if the permissions boundary have been applied by checking the AWS IAM console for the `authRole` and `unauthRole` created by Amplify CLI.

Add a DynamoDB table, run:

```
amplify add storage
```

Select `NoSQL Database` and finish the setup wizard.

Push the resources, run:

```
amplify push
```

If we create resources that are outside the scope of allowed resources on permissions boundary, for example a Cognito resource. The next `amplify push` will fail.
