...
Suppose you want to scope down permissions for Amplify CLI to only allow an environment to work with DynamoDB tables. In this case, the [DynamoDB FullAccess Managed Policy](https://us-east-1.console.aws.amazon.com/iam/home#/policies/arn:aws:iam::aws:policy/AmazonDynamoDBFullAccess$serviceLevelSummary?section=permissions) can be used as a permission boundary.

To add a Permissions Boundary for a new environment, run:

```
amplify env add --permissions-boundary arn:aws:iam::aws:policy/AmazonDynamoDBFullAccess
```

Verify if the permissions boundary have been applied by checking the AWS IAM console for the `authRole` and `unauthRole` created by Amplify CLI.
