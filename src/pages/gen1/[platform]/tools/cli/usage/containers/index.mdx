...
Amplify [libraries](/gen1/[platform]/build-a-backend/) can be used with the [Auth category](/gen1/[platform]/build-a-backend/auth/set-up-auth/) giving mobile and web applications secure connectivity and access controls to your Serverless containers. Additionally, existing GraphQL and REST services such as AWS AppSync and Amazon API Gateway can be used in the same project along with Fargate-backed APIs giving flexibility to mix and match for cost optimization and operational needs.

Note that serverless containers do incur additional costs and operational overhead, as such we recommend using AWS AppSync with the [GraphQL Transform](/gen1/[platform]/build-a-backend/graphqlapi/) as a starting point when building mobile and web apps with Amplify.
...
- [JavaScript GraphQL configuration](/gen1/[platform]/build-a-backend/graphqlapi/)
- [JavaScript REST configuration](/gen1/[platform]/build-a-backend/restapi/set-up-rest-api/#manual-setup-import-existing-rest-api)
- [Android GraphQL configuration](/gen1/[platform]/build-a-backend/graphqlapi/)
- [Android REST configuration](/gen1/[platform]/build-a-backend/restapi/set-up-rest-api/)
- [iOS GraphQL configuration](/gen1/[platform]/build-a-backend/graphqlapi/)
- [iOS REST configuration](/gen1/[platform]/build-a-backend/restapi/set-up-rest-api/)

Note that if you have enabled Authorization checks on your endpoints during `amplify add api` your clients will need to Authenticate against the Cognito User Pool configured and pass tokens. Please see the [appropriate platform guide](/gen1/[platform]/build-a-backend/auth/set-up-auth/) for adding Sign-Up and Sign-In calls to your application.
