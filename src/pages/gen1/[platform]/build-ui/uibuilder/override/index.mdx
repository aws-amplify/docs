...
**Note**: Studio supports nested collections with any GraphQL API created with Amplify, [with and without DataStore](/gen1/[platform]/tools/console/data/data-model/#datastore-and-graphql). However, these different data types have different syntax.
...
In this example, you have two collections: `AmpligramCollection`, which has a slot named `comments`, and `CommentViewCollection`. These are bound to the `Post` and `Comment` data models, which have a [one-to-many relationship](/gen1/[platform]/tools/console/data/relationships/) - therefore, each record in the `Post` data model has a field named `Comments`, whose value is an array of `Comment` records.
...
If your components are bound to a [GraphQL API using DataStore](/gen1/[platform]/tools/console/data/data-model/#datastore-and-graphql), you can use the Amplify Hub to listen to actions that are executed via [UI event handlers](/gen1/[platform]/build-ui/uibuilder/eventhandling/) and then add your custom business logic.
...
| --- | --- |
| actions:core:navigate:started | [Navigate action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#bind-ui-to-navigation-actions) started |
| actions:core:navigate:finished | [Navigate action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#bind-ui-to-navigation-actions) finished (possibly with errors) |
| actions:datastore:create:started | [DataStore create action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#create-a-record-in-database) started |
| actions:datastore:create:finished | [DataStore create action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#create-a-record-in-database) finished |
| actions:datastore:update:started | [DataStore update action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#update-a-record-from-database) started |
| actions:datastore:update:finished | [DataStore update action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#update-a-record-from-database) finished (possibly with errors) |
| actions:datastore:delete:started | [DataStore delete action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#delete-a-record-from-database) started |
| actions:datastore:delete:finished | [DataStore delete action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#delete-a-record-from-database) finished (possibly with errors) |
| actions:auth:signout:started | [SignOut action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#bind-ui-to-sign-out-actions) started |
| actions:auth:signout:finished | [SignOut action](/gen1/[platform]/build-ui/uibuilder/eventhandling/#bind-ui-to-sign-out-actions) finished (possibly with errors) |
...
**Note:** Amplify Studio now provides a new experience to build React forms. We recommend that you review [Form Builder (React)](/gen1/[platform]/build-ui/formbuilder/) first before using the data action bindings documented below.

</Callout>

Amplify Studio provides [data action bindings](/gen1/[platform]/build-ui/uibuilder/eventhandling/#bind-ui-to-create-update-or-delete-a-data-record) but if you can also self-manage the form submission states and workflows in code.
...
Amplify Studio provides [navigation action bindings](/gen1/[platform]/build-ui/uibuilder/eventhandling/#bind-ui-to-navigation-actions) but if you want to integrate with your own routing system, you can also self-manage the navigation actions.
