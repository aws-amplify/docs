...
If you have a [Connected Form](/gen1/[platform]/build-ui/formbuilder/data-binding/#connected-forms) bound to a data model with [relationships](/gen1/[platform]/tools/console/data/relationships/), Form Builder uses a special field to handle submission. The **Autocomplete** field will allow your form submitters to select one or more records from a related data model. These connected autocomplete fields have some special configuration settings.

Form builder now supports data binding to **GraphQL APIs without Conflict Resolution**. [Learn more about GraphQL and Conflict Resolution here](/gen1/[platform]/tools/console/data/data-model/#datastore-and-graphql).
...
1. [Log into your Studio application](/gen1/[platform]/tools/console/adminui/start/#logging-in-and-creating-an-app)
...
When you are satisfied with your form configuration and style, [your forms can be generated in your local project](/gen1/[platform]/build-ui/formbuilder/#render-react-form-in-your-app).
...
- You must have an [Amplify app with Studio enabled](/gen1/[platform]/tools/console/adminui/start/)
- You must have Amplify Authentication, [either through Studio](/gen1/[platform]/tools/console/auth/) or [through the CLI](/gen1/[platform]/tools/cli/commands/#auth/)
- You must have added Amplify Storage to your app, either [through Studio](/gen1/[platform]/tools/console/storage/file-storage/) or [through the CLI](/gen1/[platform]/build-a-backend/storage/configure-storage/)
...
If your File Uploader field is connected to a field in your data model, the S3 key will be written to your database via your GraphQL API. If the field is not connected to a data model, you can [handle the S3 key manually with the `onSubmit` hook](/gen1/[platform]/build-ui/formbuilder/lifecycle/#handle-form-data-submissions---onsubmit).

### File Uploader with connected forms

If a File Uploader field is bound to a data model as part of a [connected form](/gen1/[platform]/build-ui/formbuilder/data-binding/#connected-forms), a record is created or updated in the bound data model when the form is submitted.
...
- You must have a GraphQL API, deployed either [through Studio](/gen1/[platform]/tools/console/data/data-model/) or [through the CLI](/gen1/[platform]/build-a-backend/graphqlapi/).
- You must have at least one field in your data model with a type of `String`
- You must have a connected form. This can be a [default form](/gen1/[platform]/build-ui/formbuilder/#getting-started), or a [custom connected form](/gen1/[platform]/build-ui/formbuilder/data-binding/#connected-forms)

_Add File Uploader to a connected form..._

1. [Log into your Studio app](/gen1/[platform]/tools/console/adminui/start/) and select the UI Library
...
Only `String` type fields can be converted to a `StorageField` type. If the **Type** dropdown is missing `StorageField`, [check your data model](/gen1/[platform]/tools/console/data/data-model/)
...
1. [Log into your Studio app](/gen1/[platform]/tools/console/adminui/start/) and select the UI Library
2. Navigate to an unconnected form, or [create a new blank form](/gen1/[platform]/build-ui/formbuilder/#getting-started)
...
With all unconnected forms, the form submission must be configured manually. Use the `onSubmit` hook and the `fields` object to [handle submission in code](/gen1/[platform]/build-ui/formbuilder/lifecycle/#handle-form-data-submissions---onsubmit).
...
File Uploader uses the same [File level access concepts](/gen1/[platform]/build-a-backend/storage/configure-access/) as the Storage category, defaulting to `private`. If you select `private` or `protected` file level access, all files uploaded will have `{user_identity_id}` in the file path. This restricts Read and Write access, and ensures identical files uploaded by different users have unique paths.
