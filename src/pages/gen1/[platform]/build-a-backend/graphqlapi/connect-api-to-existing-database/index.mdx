...
The following content requires you to deploy the Amplify GraphQL APIs via AWS Cloud Development Kit (CDK). If you have not yet deployed an Amplify GraphQL API with AWS CDK yet, review [Set up GraphQL API](/gen1/[platform]/build-a-backend/graphqlapi/set-up-graphql-api).
...
Review [Authorization rules](/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/) to limit access to these queries and mutations based on API Key, Amazon Cognito User Pool, OpenID Connect, AWS Identity and Access Management (IAM), or a custom Lambda function.
...
This feature is currently not supported on Amplify CLI. It requires you to deploy the Amplify GraphQL APIs via AWS Cloud Development Kit (CDK). If you have not yet deployed an Amplify GraphQL API with AWS CDK yet, review [Set up GraphQL API](/gen1/[platform]/build-a-backend/graphqlapi/set-up-graphql-api).
...
type Query {
  getPostBySlug(slug: String!): [Post]
    @sql(
      statement: "SELECT * FROM posts WHERE slug = :slug LIMIT 1;"
    )
    @auth(rules: [{ allow: public }])
}
```
...
For more information on each rule please refer to our documentation on [Authorization rules](/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/).
...
For more information on field-level auth rules please refer to our documentation on [Field-level authorization rules](/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/#field-level-authorization-rules).
...
- [Create, update, and delete application data](/gen1/[platform]/build-a-backend/graphqlapi/mutate-data/)
- [Read application data](/gen1/[platform]/build-a-backend/graphqlapi/query-data/)
- [Customize Authorization Rules](/gen1/[platform]/build-a-backend/graphqlapi/customize-authorization-rules/)
