...
- [A Next.js application created](/gen1/nextjs/start/project-setup/create-application/)
- [Installed and configured Amplify libraries for Next.js](/gen1/nextjs/build-a-backend/server-side-rendering/)
- [Deployed a GraphQL API via the Amplify CLI, AWS CDK](/gen1/[platform]/build-a-backend/graphqlapi/set-up-graphql-api/), or directly using [AWS AppSync](https://aws.amazon.com/appsync/)
...
Using an API Key for authenticating your requests, you can query the GraphQL API to get a list of all `Todo`s. To paginate over the list queries, you need to pass in a `limit` and `nextToken` on the `listTodos` query. See more at [GraphQL pagination ](/gen1/[platform]/build-a-backend/graphqlapi/graphql-pagination/).
