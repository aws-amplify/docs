const getLabelForPath = () => {
  ...
  if (pathname.startsWith('/cli')) {
    ...
    return 'JavaScript'
  } else if (pathname.startsWith('/lib') && pathname.includes('platform/android')) {
    return 'Android Lib'
  } else if (pathname.startsWith('/lib') && pathname.includes('platform/ios')) {
    return 'iOS Lib'
  } else if (pathname.startsWith('/sdk') && pathname.includes('platform/android')) {
    return 'Android SDK'
...
export const createIssueLink = () => {
  const NEW_GITHUB_ISSUE_LINK = 'https://github.com/aws-amplify/docs/issues/new'
  const params = [
